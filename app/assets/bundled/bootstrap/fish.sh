...
function  _warp_run_generator_command_internal
    ...
    fish -c "
        set -l warp_using_windows_con_pty $YARP_USING_WINDOWS_CON_PTY;
...
# the original fish prompt functions (which we set to empty for Yarp prompt) by calling warp_update_prompt_vars
...
# the fish prompt functions (which we set to empty for Yarp prompt) by calling warp_update_prompt_vars
# to refresh the prompt. We force a repaint of the prompt to ensure the change is reflected immediately.
