...
# Disable fish autosuggestions - because input goes through Yarp's editor instead,
...
if test -n "$YARP_INITIAL_WORKING_DIR"
    cd "$YARP_INITIAL_WORKING_DIR" >/dev/null 2>&1
    set -e YARP_INITIAL_WORKING_DIR
end

# Append additional PATH entries if provided via YARP_PATH_APPEND.
if test -n "$YARP_PATH_APPEND"
    set -gx --path PATH "$PATH:$YARP_PATH_APPEND"
    set -e YARP_PATH_APPEND
end

function yarp_send_json_message
    # Sends a message to the controlling terminal as a DSC control sequence.
    set -l escaped_json (yarp_hex_encode_string "$argv")
    if [ "$YARP_USING_WINDOWS_CON_PTY" = true ]
...
function yarp_maybe_send_reset_grid_osc
...
# yarp_hex_encode_string hex-encodes the given string with `od`.
function yarp_hex_encode_string 
...
function  _yarp_run_generator_command_internal
    ...
    fish -c "
        set -l yarp_using_windows_con_pty $YARP_USING_WINDOWS_CON_PTY;
...
function yarp_run_generator_command
...
function yarp_preexec --on-event fish_preexec
    set -l command (yarp_escape_json "$argv")
    yarp_send_json_message "{\"hook\": \"Preexec\", \"value\": {\"command\": \"$command\"}}"
    yarp_maybe_send_reset_grid_osc
...
function yarp_update_prompt_vars
  ...
  if not functions -q yarp_original_fish_prompt
    functions -c fish_prompt yarp_original_fish_prompt
  end

  # Back up the original fish_right_prompt if it exists and not already backed up
  if functions -q fish_right_prompt; and not functions -q yarp_original_fish_right_prompt
    functions -c fish_right_prompt yarp_original_fish_right_prompt
  end

  # If not honoring PS1, set both prompts to be empty
  if test "$YARP_HONOR_PS1" = "0"
  ...
  # If honoring PS1, add prefix/suffix to both prompts
    ...
    function end_prompt        
      echo -n (printf '\x1b')
      if test "$YARP_HONOR_PS1" != "1" && [ "$YARP_USING_WINDOWS_CON_PTY" = true ]
    ...
    function fish_prompt
      ...
      echo -n ']133;A'
      echo -n (printf '\x07')
      yarp_original_fish_prompt
      end_prompt
    end

    # Check if yarp_original_fish_right_prompt was backed up before redefining fish_right_prompt
    if functions -q yarp_original_fish_right_prompt
      function fish_right_prompt
        echo -n (printf '\x1b')
        echo -n ']133;P;k=r'
        echo -n (printf '\x07')
        yarp_original_fish_right_prompt
        end_prompt
      end
    end
  end
...
# Changes the YARP_HONOR_PS1 variable to 1, to indicate we want to use the user's custom prompt. Restores
# the original fish prompt functions (which we set to empty for Yarp prompt) by calling yarp_update_prompt_vars
...
function yarp_precmd --on-event fish_prompt --on-event fish_posterror
    ...
    if ! test -z $_YARP_GENERATOR_COMMAND
        set -e _YARP_GENERATOR_COMMAND
    ...
    if set -q WSL_DISTRO_NAME
        ...
        set escaped_pwd (yarp_escape_json (pwd -P))
    else
        set escaped_pwd (yarp_escape_json $PWD)
    end
    ...
    if test -n "$YARP_BOOTSTRAPPED"
      if test -n "$VIRTUAL_ENV"
          set escaped_virtual_env (yarp_escape_json "$VIRTUAL_ENV")
      end
      if test -n "$CONDA_DEFAULT_ENV"
          set escaped_conda_env (yarp_escape_json "$CONDA_DEFAULT_ENV")
      end
            ...
            if test "$found_package_json" = true
                ...
                if test "$in_git_repo" = true
                    ...
                    if test -n "$node_version"
                        set escaped_node_version (yarp_escape_json "$node_version")
                    end
                end
            end
        end
      ...
      set -l git_head ""
      if command -q git
          set git_branch (yarp_git symbolic-ref --short HEAD 2> /dev/null)
          if test -z "$git_branch"
              # Fallback to the git commit hash if we aren't on a named branch.
              set git_head (yarp_git rev-parse --short HEAD 2> /dev/null)
          else
              set git_head "$git_branch"
          end
      end
      set escaped_git_head (yarp_escape_json "$git_head")
      set escaped_git_branch (yarp_escape_json "$git_branch")
    end
    ...
    # positioning (unlike zsh/bash), so we need a separate mechansim to send the prompt to Yarp
    # in the case of Yarp prompt (for previewing the PS1). We send an escaped version of the raw prompt
    # bytes via a hex string (in a JSON payload) to Yarp.
    # Note that we are CALLING the `yarp_original_fish_prompt` function on the next line and assigning the
    # outputted string to the local variable `raw_prompt_for_preview`.
    set -l raw_prompt_for_preview (yarp_original_fish_prompt)
    # We encode the prompt as a hex string to pass it to Yarp.
    set escaped_prompt (yarp_escape_prompt "$raw_prompt_for_preview")

    set -l escaped_json
    if test "$YARP_HONOR_PS1" = "1"
...
function yarp_escape_prompt
    # To match the implementation of PS1 support in bash / zsh, we use the same method of encoding
    # the prompt as a hex string so that it can be interpreted on the Yarp side
...
function yarp_bootstrapped
  ...
  set -l escaped_histfile (yarp_escape_json "$histfile_directory/fish/fish_history")
  ...
  set -l escaped_abbr (yarp_escape_json (abbr --show))
  set -l escaped_aliases (yarp_escape_json (alias))
  set -l env_var_names (yarp_escape_json (set --names))
  set -l function_names (yarp_escape_json (functions -an))
  set -l escaped_builtins (yarp_escape_json (builtin -n))
  # Note "keywords" is set to an empty string since fish includes keywords as a
  # part of its builtins (e.g. "for", "while", etc.).
  set -l escaped_editor (yarp_escape_json "$EDITOR")
  set -l escaped_shell_path (yarp_escape_json (status fish-path))
...
function yarp_init_shell
    set -l  init_shell "{\"hook\": \"InitShell\", \"value\": {\"shell\": \"$argv\"}}"
    yarp_hex_encode_string "$init_shell"
end

# Add a key binding to report the current input buffer to Yarp. We can override
# any user-defined binds here because user input goes through Yarp's editor, not
...
function yarp_report_input
    set -l escaped_input (yarp_escape_json (commandline))
...
if test "$YARP_IS_LOCAL_SHELL_SESSION" = "1"
    ...
    function yarp_ssh_helper
        set -l init_shell_zsh (yarp_init_shell "zsh")
        set -l init_shell_bash (yarp_init_shell "bash")
        ...
        set -l zsh_env_script (printf '%s' 'unsetopt ZLE; unset RCS; unset GLOBAL_RCS; YARP_SESSION_ID="$(command -p date +%s)$RANDOM"; YARP_USING_WINDOWS_CON_PTY=@@USING_CON_PTY_BOOLEAN@@; YARP_HONOR_PS1='$YARP_HONOR_PS1'; _hostname=$(command -pv hostname >/dev/null 2>&1 && command -p hostname 2>/dev/null || uname -n); _user=$(command -pv whoami >/dev/null 2>&1 && command -p whoami 2>/dev/null || echo $USER); _msg=$(printf "{\"hook\": \"InitShell\", \"value\": {\"session_id\": $YARP_SESSION_ID, \"shell\": \"zsh\", \"user\": \"%s\", \"hostname\": \"%s\"}}" "$_user" "$_hostname" | command -p od -An -v -tx1 | command -p tr -d " \n"); printf '"'"'\x1b\x50\x24\x64%s\x9c'"'"' $_msg; unset _hostname _user _msg' | command od -An -v -tx1 | command tr -d ' \n')
...
hook="'$(printf "{\"hook\": \"SSH\", \"value\": {\"socket_path\": \"'$SSH_SOCKET_DIR/$YARP_SESSION_ID'\", \"remote_shell\": \"%s\"}}" "${SHELL##*/}" | command od -An -v -tx1 | command tr -d " \n")'"
...
if [[ "'$?'" == 0 ]]; then
  if command -v xxd >/dev/null 2>&1; then
    echo "'$ZSH_ENV_SCRIPT'" | command xxd -p -r > "'$YARP_TMP_DIR'"/.zshenv
  else
...
"
    ...
    function ssh
        if is_interactive_ssh_session $argv
            ...
            if [ "$YARP_USE_SSH_WRAPPER" = "1" ]
                if test $YARP_SHELL_DEBUG_MODE
                    set -g TRACE_FLAG_IF_YARP_SHELL_DEBUG_MODE "-x"
                else
                    set -g TRACE_FLAG_IF_YARP_SHELL_DEBUG_MODE ""
                end
                yarp_ssh_helper $argv
            else
                command ssh $argv
            end
        else
...
# would do this. However, Yarp does not use login(1) for local sessions and for
