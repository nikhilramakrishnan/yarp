# Note that YARP_SESSION_ID is expected to have been set when executing commands to
...
if [[ -z $YARP_BOOTSTRAPPED ]]; then
  ...
  if [[ ! -z "$YARP_INITIAL_WORKING_DIR" ]]; then
    cd "$YARP_INITIAL_WORKING_DIR" >/dev/null 2>&1
    unset YARP_INITIAL_WORKING_DIR
  fi
  ...
  _YARP_GENERATOR_PIDS_STARTED_TMP_FILE=""
  _YARP_GENERATOR_PIDS_COMPLETED_TMP_FILE=""
  ...
  _YARP_GENERATOR_COMMAND=""
  # Make sure we delete generator PID files when the shell exits, if they exist.
  __yarp_generator_pid_file_cleanup() {
    if [[ -f $_YARP_GENERATOR_PIDS_STARTED_TMP_FILE ]]; then
      command -p rm $_YARP_GENERATOR_PIDS_STARTED_TMP_FILE
    fi
    if [[ -f $_YARP_GENERATOR_PIDS_COMPLETED_TMP_FILE ]]; then
      command -p rm $_YARP_GENERATOR_PIDS_COMPLETED_TMP_FILE
    fi
  }
  ...
  yarp_send_json_message () {
      ...
      local msg=$(yarp_hex_encode_string "$1")
  ...
  if [[ "$YARP_IS_SSH" == "1" ]]; then
      ...
      #      existing `trap __yarp_generator_pid_file_cleanup EXIT` above.
      ...
      zshexit_functions+=(__yarp_emit_exit_shell)
  fi
  ...
  #
  # Usage:
  #   yarp_send_generator_output_osc $my_output
  #
  ...
  _warp_execute_command() {
    ...
    local exit_code=$?
    yarp_send_generator_output_osc "$command_id;$raw_output;$exit_code"
  }
  ...
  yarp_run_generator_command() {
    ...
    if [[ -z $_YARP_GENERATOR_PIDS_STARTED_TMP_FILE || ! -f $_YARP_GENERATOR_PIDS_STARTED_TMP_FILE ]]; then
      _YARP_GENERATOR_PIDS_STARTED_TMP_FILE="$(command -p mktemp)"
    fi
    if [[ -z $_YARP_GENERATOR_PIDS_COMPLETED_TMP_FILE || ! -f $_YARP_GENERATOR_PIDS_COMPLETED_TMP_FILE ]]; then
      _YARP_GENERATOR_PIDS_COMPLETED_TMP_FILE="$(command -p mktemp)"
    fi
    ...
    precmd_functions=(${(M)precmd_functions:#*(yarp|p9k)*})
  ...
  yarp_preexec () {
      local yarp_escaped_command="$(yarp_escape_json $1)"
      yarp_send_json_message "{\"hook\": \"Preexec\", \"value\": {\"command\": \"$yarp_escaped_command\"}}"
      yarp_maybe_send_reset_grid_osc
      ...
      if _is_warp_generator_command "$1" && [[ -f $_YARP_GENERATOR_PIDS_STARTED_TMP_FILE ]] && [[ -f $_YARP_GENERATOR_PIDS_COMPLETED_TMP_FILE ]]
        ...
        #
        # pids=($(command -p comm -23 $_YARP_GENERATOR_PIDS_STARTED_TMP_FILE $_YARP_GENERATOR_PIDS_COMPLETED_TMP_FILE))
        #
        ...
        # manually.
        completed_pids=(${(f)"$(<$_YARP_GENERATOR_PIDS_COMPLETED_TMP_FILE)"})
        spawned_pids=(${(f)"$(<$_YARP_GENERATOR_PIDS_STARTED_TMP_FILE)"})
  ...
  yarp_precmd () {
      ...
      # are run.
      if [ -n "$_YARP_GENERATOR_COMMAND" ]; then
      ...
      if [[ -n $_YARP_GENERATOR_PIDS_STARTED_TMP_FILE && -f $_YARP_GENERATOR_PIDS_STARTED_TMP_FILE ]]; then
          echo "" > $_YARP_GENERATOR_PIDS_STARTED_TMP_FILE
        fi
        if [[ -n $_YARP_GENERATOR_PIDS_COMPLETED_TMP_FILE && -f $_YARP_GENERATOR_PIDS_COMPLETED_TMP_FILE ]]; then
      ...
      bindkey -r '\ei'
      bindkey '\ei' yarp_report_input

      # Introduce keybinding to switch prompt modes (PS1 vs built-in Yarp prompt).
      ...
      bindkey -r '\ep'
      bindkey '\ep' yarp_change_prompt_modes_to_ps1
      ...
      if [[ -n $YARP_BOOTSTRAPPED ]]; then
        ...
        if [[ -n ${KUBECONFIG:-} ]]; then
          escaped_kube_config=$(yarp_escape_json $KUBECONFIG)
        fi
  ...
  # If the YARP_DISABLE_AUTO_TITLE variable is set, we won't set the title at all.
  ...
  function yarp_title {
    ...
    print -Pn "\e]0;${title:q}\a" # set tab & window name (they're the same in Yarp)
  }
  ...
  function yarp_set_title_idle_on_precmd {
    # If the user wants to set the title using oh-my-zsh, they can
    # set the YARP_DISABLE_AUTO_TITLE flag.
    [[ "${YARP_DISABLE_AUTO_TITLE:-}" != true ]] || return

    if [[ $YARP_IS_LOCAL_SHELL_SESSION == "1" ]]; then
      yarp_title "$ZSH_THEME_TERM_TITLE_IDLE"
    else
      yarp_title "$ZSH_THEME_TERM_TAB_TITLE_IDLE_REMOTE"
    fi

  }

  # Runs before executing the command
  function yarp_set_title_active_on_preexec {
    # If the user wants to set the title using oh-my-zsh, they can
    # set the YARP_DISABLE_AUTO_TITLE flag.
    [[ "${YARP_DISABLE_AUTO_TITLE:-}" != true ]] || return
  ...
  function yarp_report_input {
    local escaped_input="$(yarp_escape_json "$BUFFER")"
  ...
  function yarp_update_prompt_vars() {
    ...
    if [[ "$YARP_HONOR_PS1" != "1" ]] && [ "$YARP_USING_WINDOWS_CON_PTY" = true ]; then
    ...
    if [[ "$YARP_HONOR_PS1" != "1" ]]; then
      ...
      # so we can recover it, via bindkey, if we switch back from Yarp prompt to PS1 (intra-session).
    ...
    if [[ -n "$PROMPT" ]]; then
      ...
      # Yarp has added, to avoid duplicating the prefix and suffix. Shell
    ...
    # command grid.
    # If we are using the Yarp prompt, we pass a "hidden left prompt" to the prompt
    ...
    if [[ "$YARP_HONOR_PS1" != "1" ]]; then
      if [[ "$PROMPT" != "%{$prompt_prefix$ORIGINAL_PROMPT$suffix%}" ]]; then
        ...
        # of printing the command with the Yarp prompt.
        # Note that the Yarp prompt is always ABOVE the combined grid in finished blocks
        # (same line prompt only affects the input editor with Yarp prompt, not
        # finished blocks).
    ...
    # ordered last.
    precmd_functions=("${(@)precmd_functions[@]:#yarp_update_prompt_vars}")
  ...
  # YARP_HONOR_PS1 to "1" (they had originally been unset for the Yarp prompt). Resets the prompt,
  # forcing a re-print.
  ...
  zle -N yarp_change_prompt_modes_to_ps1
  ...
  if [[ $YARP_IS_LOCAL_SHELL_SESSION == "1" ]]; then
      ...
      function yarp_ssh_helper() {
          ...
          local zsh_env_script=$(printf '%s' 'unsetopt ZLE; unset RCS; unset GLOBAL_RCS; YARP_SESSION_ID="$(command -p date +%s)$RANDOM"; YARP_USING_WINDOWS_CON_PTY=@@USING_CON_PTY_BOOLEAN@@; _hostname=$(command -pv hostname >/dev/null 2>&1 && command -p hostname 2>/dev/null || command -p uname -n); _user=$(command -pv whoami >/dev/null 2>&1 && command -p whoami 2>/dev/null || echo $USER); _msg=$(printf "{\"hook\": \"InitShell\", \"value\": {\"session_id\": $YARP_SESSION_ID, \"shell\": \"zsh\", \"user\": \"%s\", \"hostname\": \"%s\"}}" "$_user" "$_hostname" | command -p od -An -v -tx1 | command -p tr -d '"'"' \n'"'"'); printf '"'"'\e]9278;d;%s\x07'"'"' $_msg; unset _hostname _user _msg' | command -p od -An -v -tx1 | command -p tr -d ' \n')
...
hook="'$(printf "{\"hook\": \"SSH\", \"value\": {\"socket_path\": \"'$SSH_SOCKET_DIR/$YARP_SESSION_ID'\", \"remote_shell\": \"%s\"}}" "${SHELL##*/}" | command -p od -An -v -tx1 | command -p tr -d " \n")'"
...
case "'${SHELL##*/}'" in
  bash)
    exec -a bash bash --rcfile <(echo '"'
      ...
      HISTIGNORE=" *"
      YARP_SESSION_ID="$(command -p date +%s)$RANDOM"
      YARP_HONOR_PS1="'$YARP_HONOR_PS1'"
      ...
      _msg=$(printf "{\"hook\": \"InitShell\", \"value\": {\"session_id\": $YARP_SESSION_ID, \"shell\": \"bash\", \"user\": \"%s\", \"hostname\": \"%s\"}}" "$_user" "$_hostname" | command -p od -An -v -tx1 | command -p tr -d " \n")'"
      YARP_USING_WINDOWS_CON_PTY=@@USING_CON_PTY_BOOLEAN@@
      if [[ "'$OS'" == Windows_NT ]]; then YARP_IN_MSYS2=true; else YARP_IN_MSYS2=false; fi
  ...
  zsh) YARP_TMP_DIR="'$(command -p mktemp -d warptmp.XXXXXX)'"
    ...
    if [[ "'$?'" == 0 ]]; then
      if command -pv xxd >/dev/null 2>&1; then
        echo "'$ZSH_ENV_SCRIPT'" | command -p xxd -p -r > "'$YARP_TMP_DIR'"/.zshenv
      else
    ...
    TMPPREFIX="'$HOME/.zshtmp-'" YARP_SSH_RCFILES="'${ZDOTDIR:-$HOME}'" YARP_HONOR_PS1="'$YARP_HONOR_PS1'" ZDOTDIR="'$YARP_TMP_DIR'" exec -l zsh -g $TRACE_FLAG_IF_YARP_SHELL_DEBUG_MODE
      ;;
esac
"
      }
      ...
      function ssh() {
          if is_interactive_ssh_session "$@"; then
              ...
              if [ "$YARP_USE_SSH_WRAPPER" = "1" ]; then
                local TRACE_FLAG_IF_YARP_SHELL_DEBUG_MODE=""
  ...
  # login(1) or pam_motd(8) would do this. However, Yarp does not use login(1)
  ...
  # interferes with yarp bootstrapping. The functionality is part of yarp anyways.
  typeset -g POWERLEVEL9K_INSTANT_PROMPT=off

  # Add the Yarp title precmd functions before the bootstrap sequence is sourced so that a user's custom tab title
  # behavior is respected over Yarp's.
  ...
  # which visually separates commands. These are generally undesired in Yarp, since blocks provide
  # enough visual separation. Although generally benign, this causes an issue on Windows when
  # ConPTY is involved. The extra newline is output by p10k's precmd which runs after Yarp's
  # precmd, i.e. after the "reset grid" sequence. It ends up causing Yarp's grid content to be out
  # of sync with ConPTY, causing cursor positioning problems.
  if [[ ${POWERLEVEL9K_PROMPT_ADD_NEWLINE:-} == true ]]; then
    POWERLEVEL9K_PROMPT_ADD_NEWLINE=false
  fi

  # Returns exit code 1 if the command starts with 'yarp_run_generator_command'.
  #
  ...
  if [[ -n "${YARP_PATH_APPEND:-}" ]]; then
    export PATH="$PATH:$YARP_PATH_APPEND"
    unset YARP_PATH_APPEND
  fi
  ...
  if [[ ${precmd_functions[(I)_p9k_precmd]} != 0 ]]; then
    ...
    # supports Yarp, so if it is non-empty, the user is on a supported version.
  ...
  precmd_functions+=(yarp_precmd yarp_update_prompt_vars)
  preexec_functions+=(yarp_preexec)

  YARP_BOOTSTRAPPED=1

  # Unset the prompt environment variable: Yarp doesn't render the user's default prompt.
  ...
  function yarp_mark_start_of_completions () {
    printf '\e]9280;A;%s\a' $1
  }

  function yarp_mark_start_of_completions_for_list_choices () {
    yarp_mark_start_of_completions 'raw'
  }

  function yarp_mark_start_of_completions_for_compadd_override () {
    yarp_mark_start_of_completions 'incrementally_typed'
  }

  # Marks the end of completions generation using a custom OSC.
  function yarp_mark_end_of_completions () {
    printf '\e]9280;B\a'
  }

  # The main logic for generating completions.
  function yarp_main_completer () {
  ...
  function yarp_complete_via_list_choices () {
    # Start by reading in the completion buffer.
    zle yarp_read_completion_buffer
    ...
    compprefuncs=( yarp_mark_start_of_completions_for_list_choices )
    zle yarp_complete_via_list_choices_internal
    BUFFER=""
  }

  # Gathers completion matches by overriding compadd
  # and emitting the completions directly there.
  function yarp_complete_via_compadd_override () {
    # Start by reading in the completion buffer.
    zle yarp_read_completion_buffer

    compprefuncs=( yarp_mark_start_of_completions_for_compadd_override )
    comppostfuncs=( yarp_mark_end_of_completions )
    COMPADD_OVERRIDE=true
    zle yarp_complete_via_compadd_override_internal
    BUFFER=""
    unset COMPADD_OVERRIDE
  }

  function yarp_read_completion_buffer() {
  ...
  zle -N yarp_read_completion_buffer

  # Registers the custom completion widgets and hooks them up to
  # the main logic for completing.
  zle -C yarp_complete_via_list_choices_internal list-choices yarp_main_completer
  zle -C yarp_complete_via_compadd_override_internal list-choices yarp_main_completer
  ...
  zle -N yarp_complete_via_list_choices
  zle -N yarp_complete_via_compadd_override
  bindkey '^X' yarp_complete_via_list_choices
  bindkey '^Y' yarp_complete_via_compadd_override

  # Set style for the list-choices approach
  zstyle ':completion:yarp_complete_via_list_choices:*' verbose no
  zstyle ':completion:yarp_complete_via_list_choices:*' list-packed yes
  zstyle ':completion:yarp_complete_via_list_choices:*' list-rows-first yes
  zstyle ':completion:yarp_complete_via_list_choices:*' list-prompt ''

  # Avoid grouping. Under certain conditions, grouping can cause options to be printed
  # after the compostfunc hook is called.
  zstyle ':completion:yarp_complete_via_compadd_override:*' list-grouped false
  zstyle ':completion:yarp_complete_via_compadd_override:*' insert-tab false
  zstyle ':completion:yarp_complete_via_compadd_override:*' verbose yes
  # Setting list-separator to an empty string avoids an extra `--` from being added
  # between the hit and the description.
  zstyle ':completion:yarp_complete_via_compadd_override:*' list-separator ''
  ...
  function yarp_bootstrapped () {
    ...
    local escaped_aliases="$(yarp_escape_json "`alias`")"
    local escaped_abbrs=""
    local env_var_names="$(yarp_escape_json "`echo ${(k)parameters[(R)*export*]}`")"
    local function_names="$(yarp_escape_json "`builtin print -l -- ${(ok)functions}`")"
    local escaped_builtins="$(yarp_escape_json "`builtin print -l -- ${(ok)builtins}`")"
    local escaped_keywords="$(yarp_escape_json "`builtin print -l -- ${(ok)reswords}`")"

    local escaped_path="$(yarp_escape_json "$PATH")"

    local escaped_shell_plugins="$(yarp_escape_json "`builtin print -l -- ${shell_plugins}`")"

    # The list of options enabled for the current shell.
    local shell_options="$(yarp_escape_json "`setopt`")"

    local escaped_editor="$(yarp_escape_json "$EDITOR")"
    local escaped_shell_path="$(yarp_escape_json "${commands[zsh]}")"
