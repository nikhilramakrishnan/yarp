...
if [ -z "$YARP_BOOTSTRAPPED" ]; then
    ...
    yarp_preexec () {
        ...
        then
          ...
          while IFS= read -r pid; do
            completed_pids+=("$pid")
          done < $_YARP_GENERATOR_PIDS_COMPLETED_TMP_FILE
          ...
          done < $_YARP_GENERATOR_PIDS_STARTED_TMP_FILE
    ...
    yarp_precmd () {
        ...
        # are run.
        if [ ! -z  $_YARP_GENERATOR_COMMAND ]; then
        ...
        if [[ $YARP_PS1_EXPANSION_SUPPORTED  == "1" ]]; then
          ...
          deref_ps1=$(echo -e "\n" | PS1="$YARP_PS1" BASH_SILENCE_DEPRECATION_WARNING=1 "$BASH" --norc -i 2>&1 | command -p head -2 | command -p tail -1)
        fi
        ...
        if [[ -n "$YARP_BOOTSTRAPPED" ]]; then
          ...
          if command -v node > /dev/null 2>&1 && [ "$YARP_IN_MSYS2" = false ]; then
    ...
    if [[ $YARP_IS_LOCAL_SHELL_SESSION == "1" ]]; then
        ...
        function yarp_ssh_helper() {
            ...
            local zsh_env_script=$(printf '%s' 'unsetopt ZLE; unset RCS; unset GLOBAL_RCS; YARP_SESSION_ID="$(command -p date +%s)$RANDOM"; YARP_USING_WINDOWS_CON_PTY=@@USING_CON_PTY_BOOLEAN@@; YARP_HONOR_PS1='$YARP_HONOR_PS1'; _hostname=$(command -pv hostname >/dev/null 2>&1 && command -p hostname 2>/dev/null || command -p uname -n); _user=$(command -pv whoami >/dev/null 2>&1 && command -p whoami 2>/dev/null || echo $USER); _msg=$(printf "{\"hook\": \"InitShell\", \"value\": {\"session_id\": $YARP_SESSION_ID, \"shell\": \"zsh\", \"user\": \"%s\", \"hostname\": \"%s\"}}" "$_user" "$_hostname" | command -p od -An -v -tx1 | command -p tr -d '"'"' \n'"'"'); printf '"'"'\e]9278;d;%s\x07'"'"' $_msg; unset _hostname _user _msg' | command -p od -An -v -tx1 | command -p tr -d ' \n')
...
"
    ...
    unset YARP_INITIAL_HISTFILESIZE
