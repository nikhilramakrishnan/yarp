...
setopt hist_ignore_space
 ...
 _msg=$(printf "{\"hook\": \"InitShell\", \"value\": {\"session_id\": $YARP_SESSION_ID, \"shell\": \"zsh\", \"user\": \"%s\", \"hostname\": \"%s\", \"is_subshell\": true, \"wsl_name\": \"$WSL_DISTRO_NAME\"}}" "$_user" "$_hostname" | command -p od -An -v -tx1 | command -p tr -d " \n")
