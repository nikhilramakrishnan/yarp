...
$null = New-Module -Name Yarp-Module -ScriptBlock {
    ...
    function Yarp-Precmd {
        ...
        if ($script:generatorCommand -eq $true) {
            ...
            if ($global:YARP_BOOTSTRAPPED -eq 1) {
            ...
            $honor_ps1 = "$env:YARP_HONOR_PS1" -eq '1'
    ...
    if ((Test-Path env:YARP_INITIAL_WORKING_DIR) -and -not [String]::IsNullOrEmpty($env:YARP_INITIAL_WORKING_DIR)) {
        Set-Location $env:YARP_INITIAL_WORKING_DIR 2> $null
        Remove-Item -Path env:YARP_INITIAL_WORKING_DIR
    }
...
. {
    ...
    if (-not [String]::IsNullOrEmpty($env:YARP_PATH_APPEND)) {
        $env:PATH = '{0}{1}{2}' -f $env:PATH, [IO.Path]::PathSeparator, $env:YARP_PATH_APPEND
        Remove-Item -Path env:YARP_PATH_APPEND
    }
