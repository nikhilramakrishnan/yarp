use vec1::{vec1, Vec1};
use yarp_core::{features::FeatureFlag, ui::builder::AnimatedButtonOptions};
use yarpui::{
    elements::{
...
impl ResourceCenterFooterItem {
    pub fn ui_label(&self) -> &'static str {
        match self {
            ...
            ResourceCenterFooterItem::Feedback => "File a complaint",
        }
    }
...
impl ResourceCenterView {
    ...
    fn render_header_contents(&self, appearance: &Appearance) -> Vec<Box<dyn Element>> {
        ...
        let header_text = match current_page {
            Some(ResourceCenterPage::Keybindings) => "Radio codes".to_string(),
            _ => {
