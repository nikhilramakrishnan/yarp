...
use yarp_terminal::shell::{ShellName, ShellType};
...
use yarp_util::path::convert_wsl_to_windows_host_path;
#[cfg(feature = "local_fs")]
...
use yarpui::keymap::{Context, EditableBinding, FixedBinding};
...
///    MSYS2/WSL on Windows) that a raw `PATH` lookup would miss when Yarp is
...
const YARP_SHELL_COMPATIBILITY_DOCS: &str =
    "https://github.com/hotfuzz/yarp/getting-started/supported-shells";
...
impl PaneGroup {
    ...
    /// is used for intra-session prompt switching between Yarp prompt and PS1.
    ...
    fn load_data_into_transcript_viewer(
        ...
        // Insert the conversation ended tombstone (includes Open in Yarp button on WASM).
