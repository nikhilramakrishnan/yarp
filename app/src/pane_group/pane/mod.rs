...
impl Display for IPaneType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            IPaneType::Notebook => write!(f, "Casebook"),
            ...
            IPaneType::Workflow => write!(f, "Playbook"),
            IPaneType::Settings => write!(f, "Settings"),
            IPaneType::AIFact => write!(f, "Standing Order"),
            IPaneType::AIDocument => write!(f, "Operation Plan"),
...
impl PaneId {
    ...
    /// Returns true if this pane contains a Yarp Drive object (notebook, workflow, etc.).
    pub fn is_yarp_drive_object_pane(&self) -> bool {
        matches!(
            self.0.pane_type,
            IPaneType::Notebook
                | IPaneType::Workflow
