//! Support for pane contents that are shareable, like sessions and Yarp Drive objects.
//!
...
impl<P: BackingView> PaneHeader<P> {
    ...
    pub fn has_shareable_object<C: yarpui::ViewAsRef>(&self, ctx: &C) -> bool {
        self.sharing_dialog().as_ref(ctx).has_target()
    }

    pub fn has_shareable_shared_session<C: yarpui::ViewAsRef>(&self, ctx: &C) -> bool {
        self.sharing_dialog()
            .as_ref(ctx)
            .has_shared_session_target()
    }

    pub fn is_sharing_dialog_enabled<C: yarpui::ViewAsRef>(&self, ctx: &C) -> bool {
    ...
    pub fn render_sharing_controls(
        ...
        if !editability.can_edit() {
            let mut tooltip_text = String::from("Ear-only");
            if matches!(editability, ContentEditability::RequiresLogin) {
                tooltip_text.push_str(". Sign on at Sandford to log on the case");
            }
