...
#[cfg(not(target_family = "wasm"))]
use yarp_editor::render::model::AutoScrollMode;
use yarp_editor::{
...
where
    F: Fn(&mut yarpui::EventContext<'_>) + 'static,
{
    ...
    .with_tooltip_position(yarpui::ui_components::button::ButtonTooltipPosition::BelowLeft)
    .build()
    .on_click(move |ctx: &mut yarpui::EventContext<'_>, _, _| {
        on_click(ctx);
    });
...
impl CodeReviewView {
    ...
    pub fn new(
        ...
        let file_nav_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new("", NakedTheme)
                ...
                .with_tooltip("Show the case file ledger")
        ...
        let undo_action_button = ctx.add_typed_action_view(move |ctx| {
            ...
            let mut action_button = ActionButton::new("Take it back", NakedTheme)
                .with_size(ButtonSize::Small)
                .on_click(move |ctx| {
        ...
        let discard_confirm_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new("Burn the draft", DangerPrimaryTheme)
        ...
        let discard_cancel_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new("Stand down", NakedTheme).on_click(|ctx| {
        ...
        let init_project_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new("Sign on the codebase", NakedTheme)
                .with_size(ButtonSize::Small)
                .with_tooltip("Sign on the codebase indexer and YARP.md")
    ...
    fn update_file_nav_button_tooltip(&self, ctx: &mut ViewContext<Self>) {
        let tooltip = if self.file_sidebar_expanded {
            "Stash the file roster"
        } else {
            "Open the file roster"
        };
        ...
        for file in files {
            ...
            let open_in_tab_button = ctx.add_typed_action_view(move |_ctx| {
                ActionButton::new("", NakedTheme)
                    ...
                    .with_size(ButtonSize::InlineActionHeader)
                    .with_tooltip("Open the case file")
                    .on_click(move |ctx| {
            ...
            let add_context_button = ctx.add_typed_action_view(move |_ctx| {
                ActionButton::new("", NakedTheme)
                    ...
                    .with_size(ButtonSize::InlineActionHeader)
                    .with_tooltip("Pin this diff to the case")
                    .on_click(move |ctx| {
            ...
            let copy_path_button = ctx.add_typed_action_view(move |_ctx| {
                ActionButton::new("", NakedTheme)
                    .with_icon(Icon::Copy)
                    .with_size(ButtonSize::InlineActionHeader)
                    .with_tooltip("Copy the case file path")
                    .on_click(move |ctx| {
    ...
    fn render_no_changes_state(
        ...
        let mut zero_state_column = Flex::column()
            ...
            .with_child(
                Text::new("Nothing open on the desk", appearance.ui_font_family(), 16.)
                    .with_style(Properties::default().weight(Weight::Semibold))
        ...
        match result {
            ReviewSubmissionResult::Success {
                ...
                ToastStack::handle(ctx).update(ctx, |stack, ctx| {
                    let toast = DismissibleToast::default("Notes radioed to the PC.".into());
            ...
            ReviewSubmissionResult::Error => {
                ...
                let error_message = "Couldn't get the notes through to the PC.".to_string();
    ...
    pub fn render_diff_stats(stats: &DiffStats, appearance: &Appearance) -> Box<dyn Element> {
        ...
        counts_row.add_child(
            Container::new(
                ...
                .with_color(
                    yarp_core::ui::theme::Fill::Solid(internal_colors::neutral_6(theme)).into(),
                )
            ...
            column.add_child(
                Hoverable::new(file_state.sidebar_mouse_state.clone(), |mouse_state| {
                    ...
                    if mouse_state.is_hovered() {
                        container = container.with_background(yarp_core::ui::theme::Fill::Solid(
            ...
            if is_item_being_scrolled && !is_first_item_with_no_scroll {
                ...
                stack.add_positioned_child(
                    ...
                    OffsetPositioning::offset_from_parent(
                        ...
                        yarpui::elements::ParentOffsetBounds::ParentByPosition,
                        yarpui::elements::ParentAnchor::TopMiddle,
                        yarpui::elements::ChildAnchor::TopMiddle,
                    ),
                );
            }
    ...
    fn render_file_stats(&self, file: &FileDiff, appearance: &Appearance) -> Box<dyn Element> {
            ...
            row.add_children([
                ...
                Container::new(
                    Text::new("•", appearance.ui_font_family(), appearance.ui_font_size())
                        .with_style(Properties::default().weight(Weight::Bold))
                        .with_color(
                            yarp_core::ui::theme::Fill::Solid(internal_colors::neutral_6(
                                appearance.theme(),
                            ))
                            .into(),
                        )
        ...
        Container::new(row.finish())
            ...
            .with_vertical_padding(4.)
            .with_border(
                Border::all(1.).with_border_fill(yarp_core::ui::theme::Fill::Solid(
    ...
    fn insert_selection_as_context(
        ...
        if let Some(terminal_view) = self.terminal_view.as_ref().and_then(|tv| tv.upgrade(ctx)) {
            ...
            if is_long_running {
                ...
                crate::workspace::ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    let toast = crate::view_components::DismissibleToast::default(
                        "Can't pin evidence while the terminal's mid-shift.".to_string(),
                    )
                    .with_object_id(toast_id);
                    toast_stack.add_ephemeral_toast(toast, self.window_id, ctx);
                });
    ...
    fn insert_diff_hunk_as_context(
        &mut self,
        file_path: PathBuf,
        line_range: Range<yarp_editor::render::model::LineCount>,
        ctx: &mut ViewContext<Self>,
    ) {
        let Some(repo_path) = self.repo_path() else {
            return;
    ...
    fn restore_cursor_position(
        ...
        ctx: &mut yarpui::ViewContext<CodeEditorView>,
    ) {
        ...
        match mode {
            PrimaryGitActionMode::Commit => {
                ...
                self.git_primary_action_button.update(ctx, |button, ctx| {
                    ...
                    button.set_tooltip(disabled.then_some("Nothing to file"), ctx);
                    button.set_on_click(
            ...
            PrimaryGitActionMode::Publish => {
                self.git_primary_action_button.update(ctx, |button, ctx| {
                    button.set_label("Submit the report", ctx);
    ...
    fn push_or_publish_menu_item(has_upstream: bool, disabled: bool) -> MenuItem<CodeReviewAction> {
        if has_upstream {
            ...
            MenuItemFields::new("File the report")
    ...
    fn header_menu_items_new(
        ...
        if FeatureFlag::DiscardPerFileAndAllChanges.is_enabled() && has_changes {
            items.push(
                MenuItemFields::new("Burn the lot")
                    .with_icon(Icon::ReverseLeft)
...
impl BackingView for CodeReviewView {
    ...
    fn render_header_content(
        ...
        view::HeaderContent::simple("On the case file diff")
    }
