...
impl CommentListView {
        ...
        Container::new(Clipped::new(Shrinkable::new(1., header_row.finish()).finish()).finish())
            ...
            .with_border(
                Border::new(1.)
                    ...
                    .with_border_fill(yarp_core::ui::theme::Fill::Solid(neutral_4(theme))),
            )
            .finish()
    }
    ...
    fn send_button_tooltip_text(
        ...
        if let ReviewDestination::Cli(agent) = destination {
            ...
            Cow::Borrowed("PC desk has to be on the air to radio notes")
        } else if !ai_available {
            Cow::Borrowed("PC review needs rations on the books")
    ...
    fn render_send_button(&self, appearance: &Appearance, ctx: &AppContext) -> Box<dyn Element> {
        ...
        let enable_send = match &self.review_destination {
            ...
            ReviewDestination::Yarp => ai_available && has_sendable_comments,
        };
        ...
        let button = appearance
            ...
            .with_text_label("Pass it to the unit".to_string())
    ...
    fn menu_items_for_comment(
        ...
        let mut items = vec![MenuItemFields::new("Bag the text")
            .with_icon(Icon::Copy)
            .with_on_select_action(CommentListAction::CopyCommentText)
            .into_item()];

        let mut edit_item = MenuItemFields::new("Amend")
    ...
    fn render(&self, ctx: &AppContext) -> Box<dyn Element> {
        ...
        if self.is_collapsed {
            ...
            Resizable::new(self.view_state.resizable_state.clone(), panel)
                ...
                .with_dragbar_color(yarpui::elements::Fill::Solid(
                    yarpui::color::ColorU::transparent_black(),
                ))
