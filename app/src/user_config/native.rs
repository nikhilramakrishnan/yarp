...
use super::{
    launch_configs_dir, tab_configs_dir, themes_dir, workflows_dir, YarpConfigUpdateEvent,
...
impl super::YarpConfig {
    pub fn new(ctx: &mut ModelContext<Self>) -> Self {
        ...
        let _ = ctx.spawn(
            ...
            |me, user_workflows, ctx| {
                ...
                ctx.emit(YarpConfigUpdateEvent::LocalUserWorkflows);
            },
        );
    ...
    fn handle_warp_managed_paths_event(
        ...
        if FeatureFlag::TabConfigs.is_enabled() && update_touches_dir(update, &tab_configs_dir()) {
            ...
            let _ = ctx.spawn(
                ...
                |me, (configs, errors), ctx| {
                    ...
                    ctx.emit(YarpConfigUpdateEvent::TabConfigs);
                    if !errors.is_empty() {
                        ctx.emit(YarpConfigUpdateEvent::TabConfigErrors(errors));
                    }
                },
            );
        }
        ...
        {
            ctx.emit(YarpConfigUpdateEvent::Settings);
        }
    }
...
pub fn load_theme_configs(theme_path: &Path) -> YarpThemeConfig {
    let mut theme_configs = YarpThemeConfig::new();
