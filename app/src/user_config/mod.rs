...
#            - exec: code .
",
        yarp_core::paths::home_relative_path(&crate::user_config::launch_configs_dir())
    );
}
...
/// These are shipped with Yarp and user-editable (Yarp does not overwrite modifications).
#[cfg_attr(target_family = "wasm", expect(dead_code))]
pub fn default_tab_configs_dir() -> PathBuf {
    base_dir().join("default_tab_configs")
}

/// Returns whether the path points to a tab config TOML file under one of Yarp's
