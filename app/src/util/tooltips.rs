    ...
    let tooltip_element = if is_enterprise_secret_redaction_enabled {
        let tooltip_column = Flex::column()
            ...
            .with_child(
                appearance
                    .ui_builder()
                    .span("*Secrets are not sent to Yarp's server.")
                    .with_style(UiComponentStyles {
                        font_size: Some(12.),
...
/// Returns whether "Open in Yarp" should be offered for the given file path.
///
/// This checks:
...
#[cfg(feature = "local_fs")]
pub fn should_show_open_in_yarp_link(path: &Path, app: &AppContext) -> bool {
    use crate::{
...
pub fn should_show_open_in_yarp_link(_path: &std::path::Path, _app: &AppContext) -> bool {
    false
}
