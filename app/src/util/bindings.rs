...
use yarpui::keymap::{BindingId, IsBindingValid};
...
pub enum CustomAction {
    ...
    #[cfg(windows)]
    WindowsCopy,
    /// Also applies to legacy Yarp AI (toggles the panel)
    NewAgentModePane,
    /// Also applies to legacy Yarp AI (attaches the selection to the panel editor)
...
lazy_static! {
    ...
    /// know about particular Yarp specific actions, so it deals with all actions
    /// as plain isizes.  Within Yarp though we want to deal with them as the enum type.
    ...
    pub static ref MAC_PTY_NON_COMPLIANT_ACTIONS: HashSet<&'static str> = HashSet::from_iter(["terminal:yarpify_subshell", "terminal:open_block_list_context_menu_via_keybinding"]);
...
/// [`yarpui::keymap::FixedBinding`] or [`EditableBinding`].
