...
//! depends on the platform. The Yarp app must use this information to avoid rendering UI elements
//! underneath them.
...
mod windows_only {
    ...
    pub(super) use pathfinder_color::ColorU;
    pub(super) use pathfinder_geometry::vector::vec2f;
    pub(super) use yarp_core::ui::theme;
    pub(super) use yarpui::elements::{
...
/// Data the Yarp app needs to avoid rendering anything below the traffic lights.
#[derive(Clone, Debug)]
...
impl TrafficLightData {
    ...
    /// by yarpui. However, native traffic light buttons (e.g. on macOS) don't scale with zoom, so
