...
pub const DISMISSED_AI_ASSISTANT_WELCOME_KEY: &str = "DismissedYarpAIWarmWelcome";
...
pub fn user_preferences_file_path() -> PathBuf {
    yarp_core::paths::config_local_dir().join("user_preferences.json")
}

/// Returns the path to the TOML settings file.
pub fn user_preferences_toml_file_path() -> PathBuf {
    yarp_core::paths::config_local_dir().join("settings.toml")
}
