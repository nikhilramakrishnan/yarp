...
use yarp_core::{settings::Setting, ui::appearance::Appearance};
...
impl SettingsImportView {
    ...
    fn render_import_button(
        ...
        Container::new(
            button
                ...
                .with_centered_text_label("Take it on the books".to_owned())
                .build()
    ...
    fn render_reset_button(&self, appearance: &Appearance) -> Box<dyn yarpui::Element> {
        appearance
            .ui_builder()
            ...
            .with_centered_text_label("Restore standard issue".to_owned())
            .build()
            .on_click(move |ctx, _, _| {
                            ...
                            .with_child(
                                Flex::row()
                                    ...
                                    .with_cross_axis_alignment(
                                        yarpui::elements::CrossAxisAlignment::Center,
                                    )
                                    .with_main_axis_size(MainAxisSize::Max)
    ...
    fn set_theme(
        ctx: &mut yarpui::ViewContext<Self>,
    ...
    pub(crate) fn interrupt_block(&mut self, ctx: &mut yarpui::ViewContext<Self>) {
...
impl View for SettingsImportView {
    ...
    fn render(&self, app: &yarpui::AppContext) -> Box<dyn yarpui::Element> {
        ...
        const WELCOME_TEXT: &str = "Pick a kit to take on the books:";
        ...
        if display_new_session_text {
            new_session_setting_text = Container::new(
                Text::new(
                    "Some standing orders kick in once you sign on for a fresh beat.",
                    font_family,
                    font_size,
                )
                .with_color(font_color.into_solid())
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            SettingsImportAction::ImportButtonClicked => {
                ...
                // set_preferences should not fail because it is writing directly to Yarp's preferences.
