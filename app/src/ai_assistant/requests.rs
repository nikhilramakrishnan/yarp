...
/// Tracks the current request status for making Yarp AI requests against server.
...
impl Requests {
    ...
    /// Starts a Yarp AI request against the server with the given request prompt.
            ...
            move |model, (start_time, response), ctx| {
                ...
                if let RequestStatus::InFlight { request, .. } = current_request_status {
                    match response {
                            ...
                            let response = if let Some(team) = UserWorkspaces::as_ref(ctx).current_team() {
                                ...
                                if team.billing_metadata.can_upgrade_to_higher_tier_plan() {
                                    if has_admin_permissions {
                                        ...
                                        format!("Looks like rations have run out. Take it from the top {next_time}.\n\n[Top up the kit]({upgrade_url}) for more rations.")
                                    } else {
                                        format!("Looks like rations have run out. Take it from the top {next_time}.\n\nRadio a team admin to top up the kit for more rations.")
                                    }
                                } else {
                                    format!("Looks like rations have run out. Take it from the top {next_time}.")
                                }
                        ...
                        _ => {
                            let response = "Radio's down at the PC desk right now. Take it from the top in a minute.".to_owned();
                            let response_in_markdown = markdown_segments_from_text(
                                transcript_part_index,
                                TranscriptPartSubType::Answer,
                                &response,
