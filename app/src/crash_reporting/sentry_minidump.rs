...
pub fn uninit() {
    ...
    // error stopping the server, we should log it as a Sentry breadcrumb in the Yarp
...
pub fn run_server(socket_path: &Path) -> anyhow::Result<()> {
    ...
    let log_dir = yarp_core::paths::state_dir().join(yarp_core::paths::YARP_LOGS_DIR);
    let _ = std::fs::create_dir_all(&log_dir);
    let log_path = log_dir.join("yarp-minidump.log");
...
fn send_crash_report(details: Option<String>, dump: Option<minidumper::MinidumpBinary>) {
    ...
    let crash_attachment = dump.and_then(|mut dump| {
        ...
        Some(Attachment {
            buffer,
            filename: "yarp-minidump.dmp".to_string(),
...
impl MinidumpGuard {
    ...
    pub fn start() -> anyhow::Result<Self> {
        ...
        let socket_path = if cfg!(target_os = "macos") {
            ...
            yarp_core::paths::state_dir().join(socket_name)
        };
        ...
        // Ensure that the crash server process can ptrace Yarp.
        #[cfg(target_os = "linux")]
