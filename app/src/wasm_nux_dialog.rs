...
use yarpui::elements::{Align, CrossAxisAlignment, Flex};
use yarpui::ui_components::{
...
pub enum WasmNUXDialogAction {
    ...
    OpenNativeAndClose,
    /// Open the Yarp download page
    OpenDownloadDesktopAppLink,
    /// Open a link to learn more about Yarp
    LearnMore,
}
...
/// * Download Yarp if they haven't already
...
pub struct WasmNUXDialog {
    ...
    download_yarp_mouse_state: MouseStateHandle,
...
impl WasmNUXDialog {
    pub fn new() -> Self {
        Self {
            ...
            download_yarp_mouse_state: Default::default(),
    ...
    /// behavior: if Yarp is installed, redirect to it; otherwise stay on the web.
...
impl View for WasmNUXDialog {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        // 1. The user doesn't have Yarp installed - treat them as a potential new user and encourage downloading Yarp.
        // 2. The user has Yarp installed, but clicked through to the web - ask if they want to always default to web.
...
impl TypedActionView for WasmNUXDialog {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            WasmNUXDialogAction::OpenNativeAndClose => {
                // We intentionally do not set the native preference here, in case the user hasn't actually installed Yarp.
                // If they have, on subsequent loads, we'll detect that Yarp is installed and redirect to the desktop.
                ctx.emit(WasmNUXDialogEvent::Close);
            ...
            WasmNUXDialogAction::OpenDownloadDesktopAppLink => {
                ctx.open_url("https://app.yarp.dev/get_yarp");
                self.requested_download = true;
                ctx.notify();
            }
            WasmNUXDialogAction::LearnMore => {
                ctx.open_url("https://github.com/hotfuzz/yarp");
            }
        }
    }
}
