...
use onboarding::{OnboardingIntention, AI_FEATURES, YARP_DRIVE_FEATURES};
...
use yarpui::elements::{ChildAnchor, ParentAnchor, ParentOffsetBounds};
...
pub struct LoginSlideView {
    ...
    /// user has disabled Yarp Agent during onboarding (or is on the terminal
    ...
    /// `RootView` only routes Terminal-intent users here when Yarp Drive is
    /// enabled.
...
impl LoginSlideView {
    ...
    pub fn new(
        ...
        let auth_token_input = ctx.add_typed_action_view(|ctx| {
            ...
            editor.set_placeholder_text("Badge token", ctx);
            editor
        });
    ...
    fn render_browser_open_content(
        ...
        let auth_token: Box<dyn Element> = if self.show_auth_token_input {
            if editor_rendered.get() {
                ...
                Container::new(yarpui::elements::Empty::new().finish())
                    .with_padding_top(12.)
    ...
    fn render_skip_dialog(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let skip_confirm_button = self.dialog_skip_button.render(
            appearance,
            button::Params {
                content: button::Content::Label("Stand down for now".into()),
                theme: &button::themes::Primary,
                options: button::Options {
