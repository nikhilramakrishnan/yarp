...
use yarp_editor::content::anchor::Anchor;
use yarp_editor::content::edit::EditDelta;
use yarp_editor::content::find::{SearchConfig, SearchResults};
use yarp_editor::content::selection_model::BufferSelectionModel;
use yarp_editor::content::version::BufferVersion;
use yarp_editor::multiline::{AnyMultilineString, MultilineString, LF};
use yarp_editor::render::model::{AutoScrollMode, LineCount, StyleUpdateAction};
use yarp_editor::selection::TextDirection;
use yarpui::units::{IntoPixels, Pixels};
...
use yarp_editor::content::buffer::{ShouldAutoscroll, VimInsertPoint};
use yarp_editor::{
    content::{
        buffer::{
...
use yarpui::text::{point::Point, TextBuffer};
...
impl CodeEditorModel {
    ...
    pub fn set_hidden_lines(
        &mut self,
        ranges: RangeSet<yarp_editor::content::text::LineCount>,
        ctx: &mut ModelContext<Self>,
    ) {
    ...
    pub fn set_visible_line_range(
        &mut self,
        range: Range<yarp_editor::content::text::LineCount>,
        ctx: &mut ModelContext<Self>,
    ) {
    ...
    fn calculate_hidden_lines(&mut self, ctx: &mut ModelContext<Self>) {
        if let Some(context_line) = self.hide_lines_outside_of_active_diff {
            ...
            let mut visible_ranges: RangeSet<yarp_editor::content::text::LineCount> =
                RangeSet::new();
            ...
            let all_lines: Range<yarp_editor::content::text::LineCount> =
                yarp_editor::content::text::LineCount::from(0)
                    ..yarp_editor::content::text::LineCount::from(line_count);

            // Find gaps in the visible ranges
            let hidden_ranges = visible_ranges
                .gaps(&all_lines)
                .collect::<RangeSet<yarp_editor::content::text::LineCount>>();
    ...
    pub fn autocomplete_symbol(&mut self, open: char, close: char, ctx: &mut ModelContext<Self>) {
        ...
        if all_cursors {
            ...
            if let Ok(texts) = vec1::Vec1::try_from_vec(texts) {
                self.update_content(
                    |mut content, ctx| {
                        content.apply_edit(
                        yarp_editor::content::buffer::BufferEditAction::InsertForEachSelection {
                            texts: &texts,
                        },
                        yarp_editor::content::buffer::EditOrigin::UserTyped,
                        selection_model,
                        ctx,
                    );
                    },
        ...
        if let Ok(edits) = vec1::Vec1::try_from_vec(edits) {
            ...
            self.update_content(
                |mut content, ctx| {
                    content.apply_edit(
                        yarp_editor::content::buffer::BufferEditAction::InsertAtCharOffsetRanges {
                            edits: &edits,
                        },
                        yarp_editor::content::buffer::EditOrigin::UserTyped,
                        selection_model,
...
impl CoreEditorModel for CodeEditorModel {
    ...
    fn validate(&self, _ctx: &impl yarpui::ModelAsRef) {}

    // Since this is a plain text editor, there is no text styles.
    fn active_text_style(&self) -> yarp_editor::content::text::TextStyles {
        Default::default()
    }
