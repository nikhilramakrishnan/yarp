...
use crate::util::openable_file_type::{
    resolve_file_target_to_open_in_yarp, resolve_file_target_with_editor_choice,
};
use crate::{
...
impl FileTreeView {
    ...
    fn render_item_with_hover(
        ...
        let expand_icon = match expand_icon {
            Some(icon) => {
                ...
                icon.to_yarpui_icon(chevron_icon_color.into()).finish()
            }
        ...
        let icon = match render_state.icon {
            ImageOrIcon::Icon(icon) => icon.to_yarpui_icon(icon_color.into()).finish(),
    ...
    fn open_file(
        ...
        let target = if editor_layout.is_some() {
            resolve_file_target_to_open_in_yarp(path, settings, editor_layout)
        } else {
    ...
    fn context_menu_items(
            ...
            match item {
                FileTreeItem::File { .. } => {
                    ...
                    if !is_file_content_binary(&path_local) {
                        items.extend([
                            MenuItemFields::new("Crack open in a new pane")
                                .with_on_select_action(FileTreeAction::OpenInNewPane {
                                    id: id.clone(),
                                })
                                .into_item(),
                            MenuItemFields::new("Crack open in a new beat")
                ...
                FileTreeItem::DirectoryHeader { .. } => {
                    items.push(
                        MenuItemFields::new("File a new case file")
        ...
        items.extend([
            MenuItemFields::new("Bag the full path")
                .with_on_select_action(FileTreeAction::CopyPath { id: id.clone() })
                .into_item(),
            MenuItemFields::new("Bag the relative path")
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        if self.displayed_directories.is_empty() {
            if let CodingPanelEnablementState::RemoteSession { has_remote_server } = self.enablement
            {
                ...
                // Yarpification / mode 1), show a loading state — the server
