...
use yarpui::elements::{Container, Flex, ParentElement};
use yarpui::ui_components::components::{UiComponent, UiComponentStyles};
use yarpui::ui_components::text::WrappableText;
use yarpui::{AppContext, Element, Entity, ModelContext, SingletonEntity};
...
impl Items {
    ...
    pub fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        if !self.recent.is_empty() {
            flex.add_child(Self::render_section_text("Recent calls", appearance));
        ...
        if !self.suggested.is_empty() {
            flex.add_child(Self::render_section_text("BOLO", appearance));
