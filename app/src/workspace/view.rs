...
use serde_json;
use yarpui::notification::NotificationSendError;
...
use yarp_util::path::{user_friendly_path, LineAndColumnArg};
...
#[cfg(target_family = "wasm")]
use yarpui::elements::Percentage;
use yarpui::elements::{
...
use yarp_core::ui::theme::{color::internal_colors, phenomenon::PhenomenonStyle, Fill};
use yarp_core::ui::{color::coloru_with_opacity, Icon};
use yarp_editor::editor::NavigationKey;
use yarpui::keymap::Context;
use yarpui::notification::{RequestPermissionsOutcome, UserNotification};
use yarpui::platform::{
...
/// Height for all panel headers (tab bar, yarp drive, resource center, theme chooser, etc.).
...
const SESSION_CONFIG_TAB_CONFIG_CHIP_TEXT: &str = "Pull your beat kit here.";
...
pub const DEFAULT_USER_DISPLAY_NAME: &str = "Officer";
...
impl Workspace {
    ...
    fn build_worktree_sidecar_search_input(ctx: &mut ViewContext<Self>) -> ViewHandle<EditorView> {
        let editor = ctx.add_typed_action_view(|ctx| {
            ...
            editor.set_placeholder_text("Look up casebooks", ctx);
            editor
        });
    ...
    fn vertical_tabs_search_input(ctx: &mut ViewContext<Self>) -> ViewHandle<EditorView> {
        ...
        editor.update(ctx, |editor, ctx| {
            editor.set_placeholder_text("Look up beats…", ctx);
        });
        ctx.subscribe_to_view(&editor, |me, editor_view, event, ctx| match event {
            EditorEvent::Edited(_) => {
        ...
        let close_button = Hoverable::new(
            ...
            |hover_state| {
                let icon = ConstrainedBox::new(
                    icons::Icon::X
                        .to_yarpui_icon(Fill::Solid(PhenomenonStyle::modal_close_button_text()))
                        .finish(),
                )
                .with_width(16.)
                .with_height(16.)
    ...
    pub fn new(
        ...
        // Show the Yarp AI warm welcome iff the user hasn't dismissed it nor interacted with Yarp AI before.
        ...
        // Don't automatically show the Yarp AI welcome during onboarding if the block onboarding flow is being used.
    ...
    fn configure_empty_workspace(
        ...
        let open_yarp_drive = if !show_yarp_home {
            ...
            // If we can't start a terminal session to run the onboarding flow, show the Yarp Home
            // placeholder along with Yarp Drive.
            true
        };
    ...
    fn toggle_ai_assistant_panel(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        // If there is a modal open, it would sit above the Yarp AI panel and we would end up
        // focusing the Yarp AI panel _behind_ the floating modal. Instead, we opt for the normal
        // toggle behavior which will close the current modal view and then toggle Yarp AI.
        ...
        // Close any other modals that could be floating on top of the Yarp AI panel.
    ...
    /// The current focusable panels are: Yarp Drive, theme chooser, AI, and resource center (keyboard shortcuts page only)
    ...
    fn view_logs(&mut self, ctx: &mut ViewContext<Self>) {
        ctx.spawn(
            async { tokio::task::spawn_blocking(yarp_logging::create_log_bundle_zip).await },
    ...
    fn unified_new_session_menu_items(
        ...
        if is_any_ai_enabled {
            let mut agent_item = MenuItemFields::new("PC")
        ...
        if FeatureFlag::TabConfigs.is_enabled() {
            ...
            menu_items.push(
                MenuItemFields::new("New beat config")
    ...
    /// Writes the default tab config template to an unused path in `~/.yarp/tab_configs/`
    ...
    /// TOML to `~/.yarp/tab_configs/`, then opens the file in the user's editor.
    #[cfg(feature = "local_fs")]
    ...
    /// a user clicks "Update Yarp" in the top right of the tab bar.
            ...
            if let Some(version) = ChannelState::app_version() {
                ...
                match autoupdate::get_update_state(ctx) {
                    ...
                    AutoupdateStage::UnableToUpdateToNewVersion { .. } => menu_items.push(
                        MenuItemFields::new("Reissue Yarp by hand")
    ...
    pub fn open_notebook(
        ...
        // Get notebook ID to set Yarp drive index selected state
    ...
    /// Open a Yarp Drive workflow in response to an intent URL.
    ...
    /// Find an active session and pre-fill the input editor the Yarp executable with the
    /// [`yarp_cli::Command::DumpDebugInfo`] subcommand.
    fn dump_debug_info(&mut self, ctx: &mut ViewContext<Self>) {
        if let Some(exec) = std::env::current_exe()
            .ok()
            .map(|path| path.to_string_lossy().into_owned())
        {
            let command = format!("{exec} {}", yarp_cli::dump_debug_info_flag());
    ...
    /// Install the Yarp CLI by creating a symlink in /usr/local/bin
    #[cfg(target_os = "macos")]
    ...
    /// Uninstall the Yarp CLI by removing the symlink from /usr/local/bin
    #[cfg(target_os = "macos")]
    ...
    pub fn open_or_toggle_yarp_drive(
        ...
        // Telemetry and welcome tip logic is only for when the user explicitly opens Yarp Drive
    ...
    fn user_menu_items(&self, app: &AppContext) -> Vec<MenuItem<WorkspaceAction>> {
            ...
            && ChannelState::show_autoupdate_menu_items()
        {
            match autoupdate::get_update_state(app) {
                ...
                {
                    items.push(
                        MenuItemFields::new("Reissue and clock back on Yarp")
                            .with_on_select_action(WorkspaceAction::ApplyUpdate)
            ...
            Arc::new(move |_, _, appearance, _| {
                ...
                let search_icon = ConstrainedBox::new(
                    icons::Icon::SearchSmall
                        .to_yarpui_icon(theme.sub_text_color(theme.surface_2()))
                        .finish(),
                )
                .with_width(16.)
                .with_height(16.)
    ...
    fn configure_worktree_new_session_sidecar(
        ...
        self.new_session_sidecar_menu.update(ctx, |menu, view_ctx| {
            ...
            menu.set_pinned_footer_builder(move |app| {
                ...
                .with_cursor(Cursor::PointingHand)
                .on_click(|ctx: &mut yarpui::elements::EventContext, _, _| {
    ...
    fn open_repo_picker_for_tab_config_modal(
        ...
        ctx.open_file_picker(
            ...
            yarpui::platform::FilePickerConfiguration::new().folders_only(),
        );
    }
    ...
    fn maybe_generate_worktree_name(
        ...
        Some(yarp_util::worktree_names::generate_worktree_branch_name(
            &branch_refs,
        ))
    }

    /// Generates a worktree tab config TOML, writes it to `~/.yarp/tab_configs/`,
    ...
    fn handle_new_worktree_submit(
        ...
        let filename_hint = if let Some(name) = worktree_branch_name {
            ...
            let branch_refs: HashSet<&str> = branches.iter().map(|s| s.as_str()).collect();
            yarp_util::worktree_names::generate_worktree_branch_name(&branch_refs)
        };
    ...
    /// saving the materialized config to `~/.yarp/tab_configs/` first.
    ...
    fn open_worktree_in_repo(&mut self, repo_path: String, ctx: &mut ViewContext<Self>) {
        ...
        let branch_refs: HashSet<&str> = branches.iter().map(|s| s.as_str()).collect();
        let branch_name = yarp_util::worktree_names::generate_worktree_branch_name(&branch_refs);
        ...
        ctx.spawn(future, move |_workspace, conversation, ctx| {
            let Some(conversation) = conversation else {
                ...
                WorkspaceToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    let toast = DismissibleToast::error("Couldn't pull the case file.".to_owned());
    ...
    pub fn sync_window_button_visibility(&self, ctx: &mut ViewContext<Self>) {
        use yarpui::platform::mac::WindowExt;
    ...
    /// View an object in Yarp Drive and open its sharing settings.
            ...
            (true, Some(ChangelogRequestType::WindowLaunch), _) => {
                if let Some(version) = ChannelState::app_version() {
                    ...
                    if FeatureFlag::AvatarInTabBar.is_enabled() {
                        ...
                        // If resource center isn't already open and Yarp AI isn't open, then open resource center
                        if !self.current_workspace_state.is_resource_center_open
                            && !self.current_workspace_state.is_ai_assistant_panel_open
                        {
                            self.open_resource_center_main_page(ctx);
    ...
    /// automatically bootstrap AKA "yarpify" that subshell if we support it. No-op if there is
    ...
    fn update_active_session(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if let Some(terminal_handle) = pane_group_handle.as_ref(ctx).active_session_view(ctx) {
            ...
            // true for Auto SSH Yarpification (mode 1) sessions where
    ...
    fn handle_codex_modal_event(&mut self, event: &CodexModalEvent, ctx: &mut ViewContext<Self>) {
        ...
        match event {
            ...
            CodexModalEvent::UseCodex => {
                ...
                let initial_prompt = "Hello, PC Mode x Codex!".to_string();
                terminal_view.update(ctx, |terminal_view, ctx| {
    ...
    fn render_right_panel_button(
        ...
        let icon = ConstrainedBox::new(icons::Icon::Diff.to_yarpui_icon(font_color).finish())
            .with_width(16.)
            .with_height(16.)
            .finish();
        ...
        Hoverable::new(
            ...
            |mouse_state| {
                let row = Flex::row()
                    ...
                    .with_child(
                        ConstrainedBox::new(
                            icons::Icon::Search.to_yarpui_icon(text_color).finish(),
                        )
                        .with_width(16.)
                        .with_height(16.)
                        .finish(),
    ...
    fn render_tab_bar_contents(
        ...
        // Simplified mode for viewing Yarp Drive objects, shared sessions, or conversation transcripts on WASM
        #[cfg(target_family = "wasm")]
    ...
    fn render_new_session_button(&self, ctx: &AppContext) -> Box<dyn Element> {
        ...
        let new_tab_tool_tip_label_text = "New Beat".to_string();
        let new_tab_tool_tip_sublabel_text =
            keybinding_name_to_display_string(NEW_TAB_BINDING_NAME, ctx);
        let tab_configs_tool_tip_label_text = "Beat configs".to_string();
    ...
    fn render_web_anonymous_user_sign_in_button(
        ...
        let button = appearance
            ...
            .with_centered_text_label(String::from("Get sworn in"));
    ...
    fn render_offline_button(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let tool_tip_label_text = "Off the wire \u{2014} some features go dark.".to_string();
        let icon = ConstrainedBox::new(
            Container::new(
            ...
            match autoupdate::get_update_state(app) {
                    ...
                    if !self.autoupdate_unable_to_update_banner_dismissed =>
                {
                    ...
                    Some(WorkspaceBannerFields {
                        ...
                        secondary_button: None,
                        button: Some(WorkspaceBannerButtonDetails {
                            text: "Reissue Yarp by hand".to_string(),
                        ...
                        move |state| {
                            let mut container = Container::new(
                                ConstrainedBox::new(
                                    ...
                                    Icon::X.to_yarpui_icon(text_color.into()).finish(),
                                )
                                .with_width(16.)
                                .with_height(16.)
                                .finish(),
    ...
    fn render_config_panel(
        ...
        match item {
            ...
            HeaderToolbarItemKind::ToolsPanel => {
                if !pane_group.left_panel_open || yarpui::platform::is_mobile_device() {
                    return None;
                }
    ...
    fn add_toggle_setting_context_flags(&self, app: &AppContext, context: &mut Context) {
        ...
        if *yarpify_settings.use_ssh_tmux_wrapper.value() {
    ...
    fn process_updated_sync_state(&self, ctx: &mut ViewContext<Self>) {
        ...
        // If there is no active terminal (like when all Yarp windows are
    ...
    /// Opens a given URL in the desktop Yarp app if installed, or redirects to download page.
    #[cfg(target_family = "wasm")]
    fn open_link_on_desktop(&mut self, url: &Url, ctx: &mut ViewContext<Self>) {
        ...
        if !is_app_installed {
            ...
            let toast_message = format!(
                "Have Yarp installed but redirecting to download page?\nEnable Local Network Access for {} in your browser.",
                ChannelState::server_root_url()
            );
            self.toast_stack.update(ctx, |toast_stack, ctx| {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            CopyAccessTokenToClipboard => {
                ...
                let access_token =
                    yarpui::r#async::block_on(self.server_api.get_or_refresh_access_token());
            ...
            DisableTerminalInputSync => {
                ...
                self.toast_stack.update(ctx, |view, ctx| {
                    let new_toast =
                        DismissibleToast::success("Cut sync across all beats.".to_string());
                    view.add_ephemeral_toast(new_toast, ctx);
                });
                    ...
                    .get_controller_for_conversation(*conversation_id, ctx)
                {
                    ...
                    if !succesfully_exited_agent_view {
                        ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                            toast_stack.add_ephemeral_toast(
                                DismissibleToast::error(
                                    "Couldn't bin the case file. Step out of the PC view and have another go.".to_string(),
                                ),
                                window_id,
                                ctx,
                            );
...
impl View for Workspace {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        // For WASM simplified tab bar views (Yarp Drive objects, shared sessions, conversation transcripts),
        ...
        {
            let pane_group = self.active_tab_pane_group().as_ref(app);
            if yarpui::platform::wasm::is_mobile_device() && pane_group.left_panel_open {
                let scrim = Rect::new()
...
/// Idempotently sets the opencode-yarp plugin entry in `~/.config/opencode/opencode.json`.
...
fn set_opencode_yarp_plugin(new_entry: &str) -> String {
    ...
    // Remove any existing opencode-yarp entries
    plugins.retain(|entry| {
        let s = entry.as_str().unwrap_or("");
        !s.contains("opencode-yarp")
    });
