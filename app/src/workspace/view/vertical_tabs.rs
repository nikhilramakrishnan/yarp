...
use crate::drive::{cloud_object_styling::yarp_drive_icon_color, DriveObjectType};
...
use yarpui::prelude::{Align, MainAxisAlignment};
...
use yarpui::{AppContext, EntityId, SingletonEntity, ViewHandle, WindowId};
...
fn resolve_icon_with_status_variant(
    ...
    match typed {
        ...
        // Yarp Drive object types use their established index colors
...
impl TypedPane<'_> {
    ...
    fn yarp_drive_object_type(&self) -> Option<DriveObjectType> {
        typed_pane_yarp_drive_object_type(self)
    }

    fn supports_vertical_tabs_detail_sidecar(&self) -> bool {
        matches!(self, TypedPane::Terminal(_) | TypedPane::Code(_))
            || self.yarp_drive_object_type().is_some()
    }
...
fn pane_display_title_and_subtitle(
    ...
    if matches!(typed, TypedPane::Code(_)) && !title.is_empty() {
        ...
        let parent = yarp_util::path::user_friendly_path(&parent_raw, home_str).to_string();
        (filename, parent)
    } else {
        (
            if title.is_empty() {
...
fn render_git_branch_text(
    ...
    Flex::row()
        ...
        .with_spacing(2.)
        .with_child(
            ConstrainedBox::new(UiIcon::GitBranch.to_yarpui_icon(text_color).finish())
...
fn render_summary_pane_kind_icon_circle(
    ...
    let (icon_element, background): (Box<dyn Element>, ElementFill) = match kind {
        ...
        | SummaryPaneKind::Other => {
            ...
            (
                icon.to_yarpui_icon(icon_color).finish(),
                internal_colors::fg_overlay_2(theme).into(),
            )
        }
    };
...
fn render_pull_request_badge_content(label: &str, appearance: &Appearance) -> Box<dyn Element> {
    ...
    Flex::row()
        ...
        .with_spacing(4.)
        .with_child(
            ConstrainedBox::new(UiIcon::Github.to_yarpui_icon(main_text_color).finish())
                .with_width(BADGE_ICON_SIZE)
                .with_height(BADGE_ICON_SIZE)
                .finish(),
        )
        ...
        if matches!(current_mode, VerticalTabsViewMode::Expanded) {
            ...
            let pr_link_info_tooltip = if show_pr_link && pr_validation_suppressed {
                Some(ShowToggleInfoTooltip {
                    ...
                    tooltip_text: "Needs the GitHub CLI on the books and signed in",
                })
            } else {
                None
            };
    ...
    Hoverable::new(mouse_state, move |hover_state| {
            ...
            (info_mouse_state.clone(), info_tooltip_text.clone())
        {
            ...
            let info_icon = Hoverable::new(info_ms, move |info_hover| {
                let icon = ConstrainedBox::new(UiIcon::Info.to_yarpui_icon(info_color).finish())
...
fn render_yarp_drive_object_detail_section(
    props: &PaneProps<'_>,
    appearance: &Appearance,
    app: &AppContext,
) -> Box<dyn Element> {
...
fn typed_pane_yarp_drive_object_type(typed: &TypedPane<'_>) -> Option<DriveObjectType> {
    match typed {
...
fn render_detail_section(
    ...
    match &props.typed {
        ...
        | TypedPane::EnvVarCollection
        | TypedPane::AIFact
        | TypedPane::AIDocument => render_yarp_drive_object_detail_section(props, appearance, app),
        TypedPane::CodeDiff
        | TypedPane::File
        | TypedPane::Settings
        | TypedPane::EnvironmentManagement
