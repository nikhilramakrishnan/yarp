...
use yarpui::elements::PositionedElementOffsetBounds;
use yarpui::EntityId;
...
fn preferred_agent_tab_titles_default_to_title_like_text() {
    let agent_text = TerminalAgentText {
        conversation_display_title: Some("Generated Taskforce title".to_string()),
        conversation_latest_user_prompt: Some("Latest Taskforce prompt".to_string()),
    ...
    assert_eq!(
        ...
        (
            Some("Generated Taskforce title".to_string()),
...
fn preferred_agent_tab_titles_use_latest_prompt_when_enabled() {
    ...
    assert_eq!(
        ...
        (
            Some("Latest Taskforce prompt".to_string()),
...
#[test]
fn detail_sidecar_supports_terminal_code_and_yarp_drive_object_panes() {
...
fn terminal_primary_line_falls_through_to_terminal_title_when_cli_agent_has_no_plugin_data() {
    ...
    assert_eq!(line.text(), "codex - ~/yarp");
}
...
fn pane_search_fragments_prepend_custom_title_and_keep_generated_metadata() {
    ...
    assert!(search_fragments_contain_query(&fragments, "~/yarp"));
    assert!(search_fragments_contain_query(&fragments, "claude"));
}
...
fn pane_search_fragments_dedupe_custom_title_against_generated_text() {
    assert_eq!(
        ...
        vec!["Production API".to_string(), "~/yarp".to_string()]
    );
}
...
fn branch_label_display_falls_back_without_branch_icon() {
    assert_eq!(
        branch_label_display(None, "~/yarp"),
        ("~/yarp".to_string(), false)
    );
    assert_eq!(
        branch_label_display(Some(""), "~/yarp"),
        ("~/yarp".to_string(), false)
    );
    assert_eq!(
        branch_label_display(Some("main"), "~/yarp"),
        ("main".to_string(), true)
    );
}
...
fn compact_branch_subtitle_falls_back_to_working_directory_without_branch_icon() {
    assert_eq!(
        compact_branch_subtitle_display(None, Some("~/yarp")),
        Some(("~/yarp".to_string(), false))
    );
    assert_eq!(
        compact_branch_subtitle_display(Some(""), Some("~/yarp")),
        Some(("~/yarp".to_string(), false))
    );
    assert_eq!(
        compact_branch_subtitle_display(Some("main"), Some("~/yarp")),
...
fn collect_normalized_unique_summary_texts_preserves_first_seen_order() {
    assert_eq!(
        ...
        vec![
            ...
            "~/yarp-server".to_string(),
            "~/yarp-terraform".to_string(),
        ]
    );
}
...
fn format_summary_primary_labels_appends_overflow_count() {
    ...
    assert_eq!(
        ...
        Some("Claude • Taskforce • cargo • code review + 1 more".to_string())
    );
...
fn summary_search_fragments_include_hidden_overflow_values() {
    let summary = VerticalTabsSummaryData {
        ...
        working_directories: vec!["~/yarp-internal".to_string(), "~/yarp-server".to_string()],
        branch_entries: vec![
            VerticalTabsSummaryBranchEntry {
