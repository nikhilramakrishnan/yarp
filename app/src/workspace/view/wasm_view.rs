...
use crate::workspace::view::{NotebookSource, OpenYarpDriveObjectSettings, Workspace};
use crate::BlocklistAIHistoryModel;

const TRANSCRIPT_PANEL_WIDTH: f32 = 280.0;

/// Builds the FUZZ runs URL for viewing all cloud runs.
fn build_oz_runs_url() -> String {
    format!("{}/runs", ChannelState::fuzz_root_url())
}
...
impl Workspace {
    ...
    pub(super) fn build_open_in_yarp_button(
        ctx: &mut ViewContext<Self>,
    ) -> ViewHandle<ActionButton> {
        ctx.add_typed_action_view(|_ctx| {
            ActionButton::new("Open in Yarp", PrimaryTheme).on_click(move |ctx| {
                ...
                if let Some(url) = parse_current_url() {
                    ...
                    log::warn!("Could not get URL for Open in Yarp button");
                }
            })
        })
    }
    ...
    pub(super) fn build_view_cloud_runs_button(
        ...
        ctx.add_typed_action_view(|_ctx| {
            ActionButton::new("Watch all cloud beats", SecondaryTheme).on_click(move |ctx| {
