...
use yarp_core::{send_telemetry_from_ctx, ui::Icon};
...
use crate::server::telemetry::{FileTreeSource, YarpDriveSource};
...
struct MouseStateHandles {
    ...
    yarp_drive_button: MouseStateHandle,
...
pub enum LeftPanelEvent {
    ...
    ShowDeleteConfirmationDialog {
        ...
        terminal_view_id: Option<yarpui::EntityId>,
    },
}
...
pub struct LeftPanelView {
    ...
    yarp_drive_view: ViewHandle<DrivePanel>,
...
impl LeftPanelView {
    pub fn new(
        ...
        let yarp_drive_view = ctx.add_typed_action_view(DrivePanel::new);
        let conversation_list_view = ctx.add_typed_action_view(ConversationListView::new);

        ctx.subscribe_to_view(&yarp_drive_view, |_me, _, event, ctx| {
            ctx.emit(LeftPanelEvent::YarpDrive(event.clone()));
        });
        ...
        let active_view = views.first().copied().unwrap_or(ToolPanelView::YarpDrive);
        ...
        match view {
            ToolPanelView::ProjectExplorer => {
                ...
                ToolbeltButtonConfig {
                    icon: Icon::FileCopy,
                    active_icon: None,
                    tooltip_text: "Case file cabinet".to_string(),
            ...
            ToolPanelView::GlobalSearch { .. } => {
                ...
                ToolbeltButtonConfig {
                    icon: Icon::Search,
                    active_icon: None,
                    tooltip_text: "Sweep the records".to_string(),
    ...
    pub fn is_yarp_drive_active(&self) -> bool {
        self.active_view.get() == ToolPanelView::YarpDrive
    }
    ...
    pub fn yarp_drive_view(&self) -> &ViewHandle<DrivePanel> {
        &self.yarp_drive_view
    }
    ...
    pub fn focus_active_view_on_entry(&mut self, ctx: &mut ViewContext<Self>) {
        match self.active_view.get() {
            ...
            ToolPanelView::YarpDrive => {
                ctx.focus(&self.yarp_drive_view);
                self.yarp_drive_view.update(ctx, |view, ctx| {
                    view.reset_focused_index_in_yarp_drive(true, ctx);
                });
            }
            ToolPanelView::ConversationListView => {
    ...
    fn update_button_active_states(&mut self) {
        for button in &mut self.toolbelt_buttons {
            button.render_with_active_state = match &button.action {
                ...
                LeftPanelAction::YarpDrive => self.active_view.get() == ToolPanelView::YarpDrive,
...
impl LeftPanelView {
    pub fn handle_action_with_force_open(
        ...
        match action {
            ...
            LeftPanelAction::YarpDrive => {
                active_view_state::set(self, ToolPanelView::YarpDrive, ctx);
                if force_open {
                    send_telemetry_from_ctx!(
                        TelemetryEvent::YarpDriveOpened {
                            source: YarpDriveSource::ForceOpened,
                            is_code_mode_v2: true
                        },
                        ctx
                    );
...
impl View for LeftPanelView {
    ...
    fn on_focus(&mut self, focus_ctx: &FocusContext, ctx: &mut ViewContext<Self>) {
        ...
        if focus_ctx.is_self_focused() {
            match self.active_view.get() {
                ...
                ToolPanelView::YarpDrive => ctx.focus(&self.yarp_drive_view),
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let mouse_state_handles = vec![
            ...
            self.mouse_state_handles.yarp_drive_button.clone(),
        ...
        let content_area: Box<dyn Element> = match self.active_view.get() {
            ...
            ToolPanelView::YarpDrive => Shrinkable::new(
                1.0,
                Container::new(ChildView::new(&self.yarp_drive_view).finish())
                    .with_padding_left(2.)
                    .with_padding_right(2.)
                    .finish(),
            )
