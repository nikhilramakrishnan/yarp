...
use crate::{
    ...
    drive::{
        cloud_object_styling::yarp_drive_icon_color,
        ...
        CloudObjectTypeAndId, DriveObjectType, OpenYarpDriveObjectSettings,
    },
    editor::{
...
use yarp_core::{context_flag::ContextFlag, settings::Setting, ui::theme::AnsiColorIdentifier};
...
const SAVE_BUTTON_TEXT: &str = "Amend it";
...
impl WorkflowView {
    ...
    pub fn load(
        ...
        if let Some(focused_folder_id) = settings.focused_folder_id.map(SyncId::ServerId) {
            self.view_in_yarp_drive(
                YarpDriveItemId::Object(CloudObjectTypeAndId::Folder(focused_folder_id)),
                ctx,
            );
        }
    ...
    /// unnecessarily modify the backing Yarp Drive object.
        ...
        crate::workspace::ToastStack::handle(ctx).update(ctx, |stack, ctx| {
            stack.add_ephemeral_toast(
                DismissibleToast::success(if self.is_for_agent_mode {
                    "Tip-off bagged.".to_string()
                } else {
                    "Order bagged.".to_string()
                }),
                window_id,
                ctx,
            );
        ...
        if let Some((mode_text, mut edit_button)) = text_and_button {
            if matches!(editability, ContentEditability::RequiresLogin) {
                ...
                edit_button = edit_button.with_tooltip(move || {
                    ui_builder
                        .tool_tip("Sign on to amend".to_string())
                        .build()
                        .finish()
                });
            }
    ...
    fn render_alias_section(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        Flex::column()
            .with_children([
                Flex::row()
                    .with_children([
                        self.render_section_header("Call signs", appearance),
        ...
        if let Some((label, icon)) = label_and_icon {
                ...
                && !self.is_for_agent_mode
            {
                ...
                let button_with_tool_tip = appearance.ui_builder().tool_tip_on_element(
                    "Radio the PC to draft a title, descriptions, or parameters".to_string(),
    ...
    fn view_in_yarp_drive(&mut self, id: YarpDriveItemId, ctx: &mut ViewContext<Self>) {
        ctx.emit(WorkflowViewEvent::ViewInYarpDrive(id));
    }
    ...
    pub(super) fn render_trash_banner(&self, app: &AppContext) -> Option<Box<dyn Element>> {
        ...
        let text = if deleted {
            "Casework off-limits — clearance pulled"
        } else {
            "Casework filed in the incinerator"
        };
            ...
            action_row.add_child(
                Align::new(
                    appearance
                        ...
                        .with_tooltip(move || {
                            ui_builder
                                .tool_tip("Pull casework from the incinerator".to_string())
                                .build()
                                .finish()
                        })
                        .with_text_label("Pull from incinerator".to_string())
                        .build()
        ...
        row.add_child(
            Shrinkable::new(
                ...
                Container::new(render_breadcrumbs(
                    ...
                    appearance,
                    |ctx, _, breadcrumb| {
                        ctx.dispatch_typed_action(WorkflowAction::ViewInYarpDrive(
...
impl TypedActionView for WorkflowView {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            WorkflowAction::ViewInYarpDrive(id) => self.view_in_yarp_drive(*id, ctx),
...
impl BackingView for WorkflowView {
    ...
    fn pane_header_overflow_menu_items(&self, ctx: &AppContext) -> Vec<MenuItem<WorkflowAction>> {
        ...
        if self.can_open_on_desktop(ctx) {
            if let Some(link) = self.workflow_link(ctx) {
                if let Ok(url) = Url::parse(&link) {
                    menu_items.push(
                        MenuItemFields::new("Open at the desktop station")
        ...
        if space != Some(Space::Shared) {
            menu_items.push(
                MenuItemFields::new("Photocopy the file")
