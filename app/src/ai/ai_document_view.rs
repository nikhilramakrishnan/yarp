...
impl AIDocumentView {
    pub fn new(
        ...
        // Create sync button mouse state (for Yarp Drive syncing)
        ...
        let update_plan_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new("Radio over update", PrimaryTheme)
                .with_size(ButtonSize::Small)
        ...
        let restore_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new("Roll back", SecondaryTheme)
                .with_size(ButtonSize::Small)
                .on_click(|ctx| {
                    ctx.dispatch_typed_action(
                        PaneHeaderAction::<AIDocumentAction, AIDocumentAction>::CustomAction(
    ...
    fn handle_action(&mut self, action: &AIDocumentAction, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            AIDocumentAction::CopyPlanId => {
                ...
                ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    toast_stack.add_ephemeral_toast(
                        DismissibleToast::success("Case file ID bagged.".to_string()),
                        window_id,
                        ctx,
                    );
                });
...
impl BackingView for AIDocumentView {
    ...
    fn pane_header_overflow_menu_items(
        ...
        {
            menu_items.push(
                crate::menu::MenuItemFields::new("Bag the case file as markdown")
        ...
        menu_items.push(
            MenuItemFields::new("File a copy of the case ID")
