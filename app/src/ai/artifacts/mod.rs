...
pub enum Artifact {
    ...
    Plan {
        document_uid: String,
        /// None until the plan is synced to Yarp Drive.
...
impl TryFrom<yarp_graphql::ai::AIConversationArtifact> for Artifact {
    type Error = ();

    fn try_from(value: yarp_graphql::ai::AIConversationArtifact) -> Result<Self, Self::Error> {
        match value {
            yarp_graphql::ai::AIConversationArtifact::PlanArtifact(plan) => Ok(Artifact::Plan {
            ...
            yarp_graphql::ai::AIConversationArtifact::PullRequestArtifact(pr) => {
                let (repo, number) = parse_github_pr_url(&pr.url).unzip();
            ...
            yarp_graphql::ai::AIConversationArtifact::ScreenshotArtifact(screenshot) => {
            ...
            yarp_graphql::ai::AIConversationArtifact::FileArtifact(file) => Ok(Artifact::File {
            ...
            yarp_graphql::ai::AIConversationArtifact::Unknown => Err(()),
        }
    }
}
...
pub fn open_screenshot_lightbox<V: yarpui::View>(
    artifact_uids: &[String],
    ctx: &mut yarpui::ViewContext<V>,
) {
...
fn screenshot_lightbox_image_from_download_result(
    ...
    match result {
        ...
        Err(e) => {
            ...
            Some(LightboxImage {
                source: LightboxImageSource::Loading,
                description: Some("Surveillance shot's gone cold".to_string()),
            })
        }
    }
}

pub fn download_file_artifact<V: yarpui::View>(
    artifact_uid: &str,
    ctx: &mut yarpui::ViewContext<V>,
) {
    let ai_client = ServerApiProvider::handle(ctx).as_ref(ctx).get_ai_client();
...
fn open_file_download_result<V: yarpui::View>(
    artifact_uid: &str,
...
#[cfg(feature = "local_fs")]
fn open_file_download_picker<V: yarpui::View>(
    artifact: ArtifactDownloadResponse,
    ctx: &mut yarpui::ViewContext<V>,
) {
    ...
    ctx.open_save_file_picker(
        move |path_opt: Option<String>, _me: &mut V, ctx: &mut yarpui::ViewContext<V>| {
...
fn show_file_download_toast<V: yarpui::View>(
    artifact_uid: &str,
