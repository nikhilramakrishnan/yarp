...
pub(crate) fn home_config_file_path(provider: MCPProvider) -> Option<PathBuf> {
    match provider {
        MCPProvider::Yarp => yarp_core::paths::yarp_home_mcp_config_file_path(),
...
impl MCPProvider {
    ...
    pub fn home_config_path(&self) -> &'static Path {
        match self {
            MCPProvider::Yarp => Path::new(".yarp/.mcp.json"),
...
pub fn mcp_provider_from_file_path(file_path: &Path) -> Option<MCPProvider> {
    ...
    // This avoids `.mcp.json` shadowing `.yarp/.mcp.json`, for example.
...
mod tests {
    ...
    fn mcp_provider_from_file_path_recognizes_yarp_home_path() {
        ...
        {
            assert_eq!(
                mcp_provider_from_file_path(&yarp_home_mcp_config_file_path),
                Some(MCPProvider::Yarp)
            );
        }
    }
}
