...
use yarpui::ModelSpawner;
use yarpui_extras::secure_storage::AppContextExt as _;

use super::{MCPServerState, TemplatableMCPServerManager};
use {crate::ai::mcp::FileBasedMCPManager, yarpui::SingletonEntity};
...
/// updates to Yarp's secure storage via a channel.
///
...
/// runtime token auto-refreshes are written back to Yarp's secure storage.
///
...
pub async fn make_authenticated_client(
    ...
    oauth_state
        .start_authorization(&[], &redirect_uri, Some("Yarp"))
        .await?;
    ...
    let config = match auth_manager.register_client("Yarp", &redirect_uri).await {
        Ok(config) => config,
...
impl TemplatableMCPServerManager {
    ...
    pub fn handle_oauth_callback(&mut self, url: &Url) -> anyhow::Result<()> {
        ...
        yarpui::r#async::block_on(server_info.oauth_result_tx.send(result)).map_err(|_| {
...
pub(crate) fn load_credentials_from_secure_storage<T: DeserializeOwned + Default>(
    ...
    app.secure_storage()
        .read_value(key)
        .inspect_err(|err| {
            if !matches!(err, yarpui_extras::secure_storage::Error::NotFound) {
