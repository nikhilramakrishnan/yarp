...
pub const HEADER_TEXT: &str = "Loadout Editor";
...
impl ExecutionProfileEditorView {
    pub fn new(profile_id: ClientProfileId, ctx: &mut ViewContext<Self>) -> Self {
        ...
        let delete_button = ctx.add_typed_action_view(|_| {
            ActionButton::new("Strike loadout", DangerSecondaryTheme)
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            ExecutionProfileEditorViewAction::SetPlanAutoSync { enabled } => {
                AIExecutionProfilesModel::handle(ctx).update(ctx, |profiles_model, ctx| {
                    profiles_model.set_autosync_plans_to_yarp_drive(self.profile_id, *enabled, ctx);
                });
                ctx.notify();
            }
