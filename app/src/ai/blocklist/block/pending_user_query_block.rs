...
/// non-fuzz Cloud Mode run waiting for its harness CLI to start.
    ...
    pub fn new(
        ...
        let close_button = show_close_button.then(|| {
            ctx.add_typed_action_view(|_| {
                ActionButton::new("Pull the order from the queue", NakedTheme)
                    .with_icon(Icon::X)
        ...
        let send_now_button = show_send_now_button.then(|| {
            ctx.add_typed_action_view(|_| {
                ActionButton::new("Radio it in now", NakedTheme)
                    .with_icon(Icon::Play)
                    .with_size(ButtonSize::XSmall)
                    .on_click(|ctx| {
