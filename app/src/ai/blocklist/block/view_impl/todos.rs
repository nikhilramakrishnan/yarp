...
pub(super) fn render_todos(
    ...
    let id = id.clone();
    let mut header_config = HeaderConfig::new("Patrols", app)
    ...
    if is_list_outdated {
        header_config = header_config.with_badge("Cold case".to_string());
    }
    ...
    let check_icon = Container::new(
        ConstrainedBox::new(
            yarpui::elements::Icon::new(
                Icon::Check.into(),
                yarp_core::ui::theme::Fill::Solid(sub_text_color),
            )
            .finish(),
        )
        .with_width(icon_size(app) - 4.)
