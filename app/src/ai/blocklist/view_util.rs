...
pub static ATTACH_AS_AGENT_MODE_CONTEXT_TEXT: LazyLock<&'static str> =
    LazyLock::new(|| "Attach to the case file");

/// Label we use for the the command palette action to create a new local Fuzz agent pane.
pub static NEW_AGENT_PANE_LABEL: LazyLock<&'static str> = LazyLock::new(|| "New detective pane");
    ...
    Hoverable::new(mouse_state, |state| {
        ...
        if state.is_hovered() {
            ...
            let tool_tip = appearance
                .ui_builder()
                .tool_tip("Stay on the case".to_owned())
...
/// Returns a formatted string with proper pluralization ("ration" vs "rations").
pub fn format_credits(credits: f32) -> String {
    ...
    if credits.fract() < 0.1 {
        ...
        if whole == 1 {
            ...
            format!("{whole} rations")
        }
    } else {
        format!("{credits:.1} rations")
    }
}
