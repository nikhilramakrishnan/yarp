...
pub enum AgentViewEntryOrigin {
    ...
    CloudAgent,
    /// Entered agent view by opening an existing non-Fuzz cloud agent run (live shared-session
...
impl AgentViewController {
    ...
    fn set_pending_confirmation(
        ...
        let abort_handle = ctx.spawn_abortable(
            async move { yarpui::r#async::Timer::after(ENTER_OR_EXIT_CONFIRMATION_WINDOW).await },
            move |me, _, _ctx| {
