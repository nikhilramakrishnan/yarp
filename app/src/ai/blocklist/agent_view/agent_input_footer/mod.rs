...
impl AgentInputFooter {
    ...
    pub fn new(
        ...
        let update_plugin_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new("Reissue Yarp plugin", InstallPluginButtonTheme)
                .with_icon(Icon::Download)
    ...
    fn select_cli_file(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let file_picker_config = yarpui::platform::FilePickerConfiguration::new();
        ...
        ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
            toast_stack.add_ephemeral_toast(
                DismissibleToast::error(
                    "Couldn't book the plugin in automatically. \
        ...
        Fut: std::future::Future<Output = Result<(), PluginInstallError>> + Send + 'static,
    {
        ...
        let (shell_path, shell_type) = match shell_data {
            ...
            // needing the Yarp plugin to integrate with it.
    ...
    pub fn toggle_cli_voice_input(
        ...
        if let voice_input::VoiceInputToggledFrom::Key { state } = source {
            match (&self.cli_voice_input_state, state) {
                (CLIVoiceInputState::Stopped, yarpui::event::KeyState::Released) => return,
                (CLIVoiceInputState::Listening, yarpui::event::KeyState::Pressed) => return,
                _ => {}
            }
        }
...
impl TypedActionView for AgentInputFooter {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut yarpui::ViewContext<Self>) {
        match action {
            #[cfg(feature = "voice_input")]
...
impl ActionButtonTheme for AgentInputButtonTheme {
    ...
    fn font_properties(&self) -> Option<yarpui::fonts::Properties> {
        if crate::features::FeatureFlag::CloudModeInputV2.is_enabled() {
            Some(yarpui::fonts::Properties {
                weight: yarpui::fonts::Weight::Semibold,
                ..Default::default()
            })
        } else {
            None
...
/// Green-accented theme for the "Install Yarp plugin" chip.
...
async fn write_install_log(agent: CLIAgent, err: &PluginInstallError) -> Option<PathBuf> {
    let log_path = env::temp_dir().join("yarp-plugin-install.log");
    let now = chrono::Utc::now().format("%Y-%m-%d %H:%M:%S UTC");
    let contents = format!(
        "Yarp plugin installation — {agent:?}\n\
         {now}\n\
         \n\
         {log}",
        log = err.log,
