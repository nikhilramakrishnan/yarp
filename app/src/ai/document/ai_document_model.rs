...
/// The status of saving an AI Document to Yarp Drive
pub enum AIDocumentSaveStatus {
    /// Not being synced with Yarp Drive at all
    NotSaved,
    /// Is being saved to Yarp Drive, but has not finished yet
    Saving,
    /// Has been saved to Yarp Drive
    Saved,
}
...
/// Represents a document queued for creation in Yarp Drive.
#[derive(Debug, Clone)]
...
pub struct AIDocument {
    ...
    /// Set when a document is saved to Yarp Drive.
...
impl AIDocumentModel {
    ...
    fn handle_update_manager_event(
        ...
        //
        // NOTE: this handler runs for *all* Yarp Drive object creations, so we must only create the
    ...
    /// Create a document from an existing Yarp Drive notebook.
    ...
    pub fn restore_document(
        ...
        // Update the sync status of a document by checking if it exists in Yarp Drive.
