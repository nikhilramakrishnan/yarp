...
impl RuleView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let initialize_button = ctx.add_typed_action_view(|_| {
            ActionButton::new("Open the case file", NakedTheme)
                .with_icon(Icon::Plus)
    ...
    fn render_disabled_banner(&self, appearance: &Appearance) -> Box<dyn Element> {
        let mut link =
            FormattedTextFragment::hyperlink(DISABLED_BANNER_LINK_TEXT, "Settings > Taskforce");
    ...
    fn render_sync_status_icon(
        ...
        let item = ai_row.fact.to_yarp_drive_item(appearance)?;
    ...
    fn render_project_based_row(
        ...
        row.add_child(
            appearance
                ...
                .with_text_label("Pull the file".to_string())
                .build()
                .on_click(move |ctx, _, _| {
