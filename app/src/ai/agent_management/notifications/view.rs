...
use yarpui::elements::new_scrollable::{ScrollableAppearance, SingleAxisConfig};
use yarpui::elements::{
...
use yarpui::{AppContext, Entity, SingletonEntity, TypedActionView, View, ViewContext, ViewHandle};
...
pub enum NotificationMailboxViewEvent {
    NavigateToTerminal { terminal_view_id: yarpui::EntityId },
    Dismissed,
}
...
impl NotificationMailboxView {
    ...
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let mark_all_read_button = ctx.add_typed_action_view(|_| {
            ActionButton::new("Clear the briefing board", NakedTheme)
...
impl NotificationMailboxView {
    fn render_header(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let label = appearance
            .ui_builder()
            .wrappable_text("Briefings".to_string(), false)
    ...
    fn render_empty_state(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        Container::new(
            appearance
                .ui_builder()
                .wrappable_text("Briefing board's clean.".to_string(), false)
                .with_style(UiComponentStyles {
                    font_size: Some(14.),
