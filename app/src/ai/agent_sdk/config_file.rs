...
///
/// Behavior:
/// - Entries with `yarp_id` become `MCPSpec::Uuid`.
...
pub fn mcp_specs_from_mcp_servers(
    ...
    for (name, config) in mcp_servers {
        ...
        if let Some(yarp_id) = obj.get("yarp_id").and_then(Value::as_str) {
            let uuid = uuid::Uuid::parse_str(yarp_id).map_err(|_| {
                anyhow::anyhow!("MCP server '{name}' field 'yarp_id' must be a UUID")
            })?;
            uuids.push(uuid);
        } else {
