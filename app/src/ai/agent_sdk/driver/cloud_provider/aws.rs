...
use yarpui::{ModelSpawner, SingletonEntity};
...
/// AWS STS audience for Yarp Fuzz OIDC federation.
...
impl AwsCloudProvider {
    ...
    pub fn new(config: &AwsProviderConfig, run_id: &str) -> Result<Self> {
        ...
        let token_file = Builder::new()
            .prefix(&format!("fuzz_aws_oidc_{run_id}_"))
            .suffix(".token")
            .tempfile()
                ...
                .spawn(move |_, ctx| {
                    ManagedSecretManager::handle(ctx)
                        ...
                        .issue_task_identity_token(
                            yarp_managed_secrets::client::IdentityTokenOptions {
                                audience,
