...
use yarp_completer::completer::CommandExitStatus;
use yarp_core::{command::ExitCode, safe_info, safe_warn};
use yarpui::{r#async::FutureExt, ModelContext, ModelSpawner, SingletonEntity};
...
async fn prepare_environment_impl(
    ...
    if !github_repos.is_empty() {
        ...
        let should_wait_for_indexing = !matches!(
            yarp_isolation_platform::detect(),
            Some(
                yarp_isolation_platform::IsolationPlatformType::DockerSandbox
                    | yarp_isolation_platform::IsolationPlatformType::Namespace
            )
        );
