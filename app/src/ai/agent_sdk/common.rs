...
use yarp_cli::environment::{EnvironmentCreateArgs, EnvironmentUpdateArgs};
...
/// Refresh Yarp Drive before executing an operation.
pub fn refresh_yarp_drive(
    ctx: &AppContext,
) -> impl Future<Output = anyhow::Result<()>> + Send + 'static {
...
/// the harness: `Harness::Fuzz` default with a Claude conversation id is treated as a mismatch
/// and errors out.
...
impl EnvironmentChoice {
    ...
    /// Yarp Drive *must* have been synced first.
