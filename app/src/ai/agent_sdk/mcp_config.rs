...
///
/// Notes:
/// - UUID specs are coerced into `{"<uuid>": {"yarp_id": "<uuid>"}}`.
        ...
        match spec {
            MCPSpec::Uuid(uuid) => {
                ...
                insert_unique(
                    ...
                    Value::Object({
                        let mut obj = Map::new();
                        obj.insert("yarp_id".to_string(), Value::String(name));
                        obj
                    }),
                )?;
            }
...
fn validate_server_config(server_name: &str, config: &Value) -> anyhow::Result<()> {
    ...
    if has_warp_id {
        ...
        uuid::Uuid::parse_str(yarp_id).with_context(|| {
            format!("MCP server '{server_name}' field 'yarp_id' must be a UUID")
        })?;
    }
