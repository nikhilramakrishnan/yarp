...
#[test]
fn test_skill_path_from_file_path_yarp_home_skill() {
    ...
    let yarp_home_skill = yarp_home_skills_dir
        .join("my-skill")
        .join("assets")
        .join("image.png");
    let result = skill_path_from_file_path(&yarp_home_skill);
    assert_eq!(
        result,
        Some(yarp_home_skills_dir.join("my-skill").join("SKILL.md"))
    );
}
