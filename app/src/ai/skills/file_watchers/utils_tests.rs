...
#[test]
fn extract_skill_parent_directory_returns_home_dir_for_yarp_home_skill() {
    let Some(home_dir) = dirs::home_dir() else {
        eprintln!("Skipping test: home directory not available");
        return;
    };
    ...
    VirtualFS::test("find_root_skills", |dirs, mut vfs| {
        ...
        App::test((), |mut app| async move {
            ...
            let repo_handle = watcher.update(&mut app, |w, ctx| {
                w.add_directory(
                    yarp_util::standardized_path::StandardizedPath::from_local_canonicalized(&repo)
                        .unwrap(),
                    ctx,
                )
                .unwrap()
...
fn find_skill_directories_in_tree_finds_subdirectory_skills() {
    VirtualFS::test("find_subdir_skills", |dirs, mut vfs| {
        ...
        let root_yarp_skills = Entry::Directory(DirectoryEntry {
            path: yarp_util::standardized_path::StandardizedPath::try_from_local(
                &repo.join(".agents/skills"),
            )
            .unwrap(),
        ...
        let root_yarp = Entry::Directory(DirectoryEntry {
            path: yarp_util::standardized_path::StandardizedPath::try_from_local(
                &repo.join(".agents"),
            )
            .unwrap(),
            children: vec![root_yarp_skills],
            ignored: false,
            loaded: true,
        });
        ...
        let frontend_yarp_skills = Entry::Directory(DirectoryEntry {
            path: yarp_util::standardized_path::StandardizedPath::try_from_local(
        ...
        let frontend_yarp = Entry::Directory(DirectoryEntry {
            path: yarp_util::standardized_path::StandardizedPath::try_from_local(
                &repo.join("packages/frontend/.agents"),
            )
            .unwrap(),
            children: vec![frontend_yarp_skills],
            ignored: false,
            loaded: true,
        });
        ...
        let root = Entry::Directory(DirectoryEntry {
            path: yarp_util::standardized_path::StandardizedPath::try_from_local(&repo).unwrap(),
            children: vec![root_yarp, packages],
            ignored: false,
            loaded: true,
        });
...
fn find_skill_directories_in_tree_empty_repo() {
    VirtualFS::test("find_skills_empty", |dirs, mut vfs| {
        ...
        let src = Entry::Directory(DirectoryEntry {
            path: yarp_util::standardized_path::StandardizedPath::try_from_local(&repo.join("src"))
                .unwrap(),
            children: vec![],
            ignored: false,
            loaded: true,
