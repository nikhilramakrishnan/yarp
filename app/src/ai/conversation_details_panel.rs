...
impl ConversationDetailsPanel {
        ...
        match event {
            ...
            ArtifactButtonsRowEvent::CopyBranch { branch } => {
                ...
                ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    let toast = DismissibleToast::default("Branch name on the clipboard.".to_string());
    ...
    fn render_creator_section(&self, appearance: &Appearance) -> Option<Box<dyn Element>> {
        ...
        let avatar = Avatar::new(
            avatar_content,
            yarpui::ui_components::components::UiComponentStyles {
                width: Some(20.),
                height: Some(20.),
                border_radius: Some(yarpui::elements::CornerRadius::with_all(
                    yarpui::elements::Radius::Percentage(50.),
                )),
    ...
    fn render_source_section(&self, appearance: &Appearance) -> Option<Box<dyn Element>> {
        ...
        Some(self.render_simple_field("Opening call-out", trimmed, appearance))
    }
    ...
    fn render_environment_section(
        ...
        let name_text = Text::new(
            format!("Beat name: {environment_name}"),
            appearance.ui_font_family(),
            ui_font_size,
        )
        .with_color(theme.foreground().into())
    ...
    fn render_credits_with_split(
        ...
        let compute_row = self.render_cost_sub_row(
            "Patrol",
            compute,
            "Rations spent on time on patrol",
...
impl View for ConversationDetailsPanel {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        if let Some(duration) = self.data.run_time {
            ...
            content.add_child(
                Container::new(self.render_simple_field("Patrol time", &formatted, appearance))
                    .with_margin_bottom(FIELD_SPACING)
                    .finish(),
            );
        }
        ...
        if let Some(created_at) = self.data.created_at {
            ...
            content.add_child(
                Container::new(self.render_simple_field("Filed on", &formatted, appearance))
                    .with_margin_bottom(FIELD_SPACING)
                    .finish(),
            );
        }
