...
pub fn run() -> Result<()> {
    yarpui::r#async::block_on(async move {
        let executor = Arc::new(Background::default());

        // Initialize a client connection to the yarp app process.
        ...
        // Initialize logging, which internally transmits logs from this process to the yarp app
        ...
        // Send the connection address for the plugin host server back to the yarp app.
...
///
/// This assumes that all plugins are located in ~/.yarp/plugins.
fn plugin_paths() -> Vec<PathBuf> {
    const PLUGIN_PATH_SUFFIX: &str = ".yarp/plugins";
...
/// Returns `true` if the directory at the given path is directory containing JS source for a Yarp plugin.
