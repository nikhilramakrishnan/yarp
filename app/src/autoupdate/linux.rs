...
use yarp_terminal::shell::ShellType;
...
pub(super) async fn download_update_and_cleanup(
    ...
    match UpdateMethod::detect() {
        ...
        UpdateMethod::PackageManager(package_manager) => {
            log::info!("Detected that Yarp was installed using {package_manager:?}");
...
mod appimage {
    ...
    pub(super) async fn download_update_and_cleanup(
        ...
        // Move new AppImage over the one that launched the current Yarp instance.
    ...
    pub(super) fn relaunch(appimage_path: &Path) -> Result<()> {
        ...
        // autoupdate process.
        command.arg(yarp_cli::finish_update_flag());
        ...
        if let Ok(path) = std::env::var("YARP_CHANNEL_VERSIONS_PATH") {
...
mod package_manager {
    ...
    impl yarpui::Entity for AutoupdateContextBlock {
        type Event = ();
    }

    impl yarpui::View for AutoupdateContextBlock {
        ...
        fn render(&self, app: &yarpui::AppContext) -> Box<dyn yarpui::Element> {
            ...
            let mut lines = vec![
                ...
                FormattedTextLine::Line(vec![
                    FormattedTextFragment::plain_text("If you installed Yarp using "),
                    FormattedTextFragment::bold(package_manager_name),
                    FormattedTextFragment::plain_text(
                        " or a compatible tool, the pre-filled command will update Yarp for you.",
                    ),
                ]),
            ];
            ...
            if self.package_manager.needs_repository_configuration() {
                lines.push(FormattedTextLine::Line(vec![
                    FormattedTextFragment::plain_text(
                        "\nThe command below includes a one-time configuration of the Yarp package repository and PGP signing key.",
                    ),
                ]));
            }
        ...
        let base_command = match self {
            ...
            PackageManager::Pacman {
                ...
                let repo_prefix = if !is_repo_configured {
                    let cache_dir = yarp_core::paths::cache_dir();
