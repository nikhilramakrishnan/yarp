...
use yarp_core::macos::get_bundle_path;
...
fn old_executable_file_path() -> PathBuf {
    yarp_core::paths::state_dir().join(OLD_EXECUTABLE_FILE_NAME)
}
...
pub(super) fn relaunch() -> Result<()> {
    ...
    // autoupdate process.
    launch_command.push(format!(" --args {}", yarp_cli::finish_update_flag()));
...
pub async fn cleanup_all_except(preserve_update_id: Option<&str>) {
    let mut autoupdate_dir = yarp_core::paths::cache_dir();
...
fn get_download_dir(update_id: &str) -> PathBuf {
    let mut dir = yarp_core::paths::cache_dir();
    dir.push("autoupdate");
    dir.push(update_id);
    dir
}
