    ...
    F: Fn(usize, &mut EventContext, Vector2F) + 'static,
{
    ...
    let label = Text::new_inline("Beat type".to_string(), appearance.ui_font_family(), 12.)
    ...
    F: Fn(&mut EventContext, Vector2F) + 'static,
{
    ...
    let dir_display =
        yarp_util::path::user_friendly_path(&raw_path, home_dir.as_ref().and_then(|h| h.to_str()))
            .into_owned();
...
where
    F: Fn(&mut yarpui::EventContext, yarpui::geometry::vector::Vector2F) + 'static,
{
