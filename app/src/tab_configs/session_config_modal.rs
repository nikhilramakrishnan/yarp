...
use yarpui::platform::file_picker::FilePickerConfiguration;
...
pub enum SessionConfigModalAction {
    ...
    OpenDirectoryPicker,
    DirectorySelected(Result<String, yarpui::platform::file_picker::FilePickerError>),
    ToggleWorktree,
...
pub struct SessionConfigModal {
    ...
    /// defaults to Terminal behind the scenes (used when Fuzz is disabled).
...
impl SessionConfigModal {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let submit_button = ctx.add_view(|ctx| {
            ActionButton::new("Sign on with Yarp", PrimaryTheme)
    ...
    /// Reconfigures the visible session types based on whether Fuzz is available.
    /// Resets the selection to index 0 (the first available type).
    /// When Fuzz is disabled, hides the session type row entirely and defaults
