...
use crate::{
    ...
    drive::items::{env_var_collection::YarpDriveEnvVarCollection, YarpDriveItem},
...
impl StringModel for EnvVarCollection {
    ...
    fn to_yarp_drive_item(
        ...
        Some(Box::new(YarpDriveEnvVarCollection::new(
...
pub fn serialize_variables_for_shell<'s, I: IntoIterator<Item = (&'s str, &'s EnvVarValue)>>(
    ...
    match shell_type {
        // Yarp doesn't support newlines in fish so we can't use env syntax
        ShellType::Fish => {
