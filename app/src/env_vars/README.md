...
We defined `CloudEnvVarCollection` in `mod.rs`, which implements the `GenericCloudObjectType` trait. This is a mostly boilerplate implementation specifying properties such as EVCs should render in Records Locker, be linkable/exportable, etc.

The implementation of EVCs as a Records Locker object is in `app/src/drive/items/env_var_collection.rs`, where code for the Records Locker preview and click action is located.
