...
use yarpui::{r#async::executor::BackgroundTask, AppContext, SingletonEntity};
...
impl DBusServiceHost {
    ...
    fn terminate(&mut self) {
        if let Some(server_task) = self.server_task.take() {
            ...
            report_if_error!(yarpui::r#async::block_on(server_task));
        }
    }
...
impl yarpui::Entity for DBusServiceHost {
    type Event = ();
}
