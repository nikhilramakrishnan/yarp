...
pub(super) fn uri_named_pipe_name() -> String {
    format!("Yarp{:?}_URI_CHANNEL", ChannelState::channel())
}
...
fn try_create_mutex() -> Result<Option<MutexHandle>, Error> {
    ...
    // `script/windows/windows-installer.iss`, which the installer uses to detect whether Yarp is
    // running.
    let name = format!("Local\\Yarp{:?}_SingleInstance", ChannelState::channel())
        .encode_utf16()
...
/// A singleton model that is responsible for ensuring there is only one instance of Yarp running.
...
impl SingleInstanceManager {
    ...
    /// Returns whether or not this process should be treated as the main instance of Yarp.
    ///
