...
pub const AGENT_MODE_RUNNING_STEP_GROUP_NAME: &str = "Taskforce mode running";

use super::hydrate_ai_conversation_assertion;

/// Assumes that the terminal input is currently not in Taskforce input mode.
pub fn enter_agent_view() -> TestStep {
    new_step_with_default_assertions("Enter Taskforce View")
        .with_keystrokes(&["ctrl-shift-enter"])
        .add_named_assertion(
            "Assert that we are in Taskforce View and Taskforce input mode",
            move |app, window_id| {
                ...
                terminal_view.read(app, |terminal_view, app| {
                    ...
                    async_assert!(
                        ...
                        "Expected fullscreen Taskforce View + Taskforce input mode, got agent_view_state={agent_view_state:?}, is_ai_input_mode={is_ai_input_mode}"
                    )
                })
            },
        )
...
/// Assumes that the terminal input is currently in Taskforce input mode.
pub fn exit_agent_view() -> TestStep {
    new_step_with_default_assertions("Exit Taskforce View")
        .with_keystrokes(&["escape"])
        .add_named_assertion(
            "Assert that we exited Taskforce View and are not in Taskforce input mode",
            move |app, window_id| {
                let terminal_view = terminal_view(app, window_id, 0, 0);
                terminal_view.read(app, |terminal_view, app| {
                    let is_ai_input_mode = terminal_view
                    ...
                    async_assert!(
                        !is_ai_input_mode && !agent_view_state.is_active(),
                        "Expected inactive Taskforce View + non-Taskforce input mode, got agent_view_state={agent_view_state:?}, is_ai_input_mode={is_ai_input_mode}"
                    )
                })
            },
        )
...
pub fn hydrate_ai_conversation(file_name: &str) -> TestStep {
    ...
    new_step_with_default_assertions("Hydrate Taskforce conversation").add_named_assertion(
        "Assert that conversation was hydrated successfully",
        hydrate_ai_conversation_assertion(tasks),
    )
}

/// Attach the latest block in the blocklist (command + output) to the Taskforce briefing.
...
pub fn submit_ai_query_and_wait_until_done(query: &str, timeout: Duration) -> TestStep {
    submit_ai_query(query, timeout)
        .add_named_assertion(
            "Assert the Taskforce task is complete",
...
/// Submits a Taskforce briefing and waits until the task is blocked (waiting for user approval).
...
pub fn submit_ai_query_and_wait_until_blocked(query: &str, timeout: Duration) -> TestStep {
    submit_ai_query(query, timeout).add_named_assertion(
        "Assert the Taskforce task is blocked",
        assert_task_is_blocked(ConversationTarget::Active),
    )
}

// Runs a Taskforce briefing without waiting for anything.
...
pub fn submit_ai_query(query: &str, timeout: Duration) -> TestStep {
    new_step_with_default_assertions_for_pane(&format!("Enter Taskforce briefing: {query}"), 0, 0)
        .set_timeout(timeout)
...
) -> impl FnMut(&mut yarpui::App, yarpui::WindowId) -> yarpui::integration::AssertionOutcome {
    |app, window_id| {
        use crate::BlocklistAIHistoryModel;
        use yarpui::integration::AssertionOutcome;
        let terminal_view = terminal_view(app, window_id, 0, 0);
...
/// Sets the preferred Taskforce LLM. This is the base model for Taskforce and inline conversations.
pub fn set_preferred_agent_mode_llm(llm_id: &str) -> TestStep {
    let llm_id = LLMId::from(llm_id);
    TestStep::new(&format!("Set preferred Taskforce LLM to {llm_id}")).add_named_assertion(
        "Update preferred Taskforce LLM",
        move |app, window_id| {
            ...
            LLMPreferences::handle(app).update(app, |llm_preferences, ctx| {
                ...
                assert!(
                    ...
                    "LLM ID '{llm_id}' is not a valid Taskforce LLM",
                );
                llm_preferences.update_preferred_agent_mode_llm(&llm_id, terminal_view_id, ctx);
            });
            async_assert!(true, "Successfully updated preferred Taskforce LLM")
        },
    )
}
