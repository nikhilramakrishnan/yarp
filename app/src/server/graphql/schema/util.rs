...
impl From<GenericStringObjectUniqueKey>
    for yarp_graphql::generic_string_object::GenericStringObjectUniqueKey
{
    fn from(key: GenericStringObjectUniqueKey) -> Self {
        use yarp_graphql::generic_string_object::GenericStringObjectUniqueKey as GraphQLFormat;
        GraphQLFormat {
            key: key.key,
...
impl From<UniquePer> for yarp_graphql::generic_string_object::UniquePer {
    fn from(unique_per: UniquePer) -> Self {
        use yarp_graphql::generic_string_object::UniquePer as GraphQLUniquePer;
        match unique_per {
...
impl From<ObjectActionType> for yarp_graphql::object_actions::ActionType {
    fn from(action: ObjectActionType) -> Self {
        match action {
            ObjectActionType::Execute => yarp_graphql::object_actions::ActionType::Executed,
        }
    }
}
...
fn try_into_object_action_type(
    action_type: yarp_graphql::object_actions::ActionType,
) -> Result<ObjectActionType, anyhow::Error> {
    match action_type {
        yarp_graphql::object_actions::ActionType::Executed => Ok(ObjectActionType::Execute),
    }
}

/// Converts the graphql action entry (SingleAction, BundledActions) into its ObjectAction corollary.
fn try_into_object_action(
    record: &yarp_graphql::object_actions::ActionRecord,
    uid: ObjectUid,
    hashed_sqlite_id: HashedSqliteId,
) -> Result<ObjectAction, anyhow::Error> {
    match record {
        yarp_graphql::object_actions::ActionRecord::SingleAction(s) => Ok(ObjectAction {
        ...
        yarp_graphql::object_actions::ActionRecord::BundledActions(b) => Ok(ObjectAction {
        ...
        yarp_graphql::object_actions::ActionRecord::Unknown => {
...
impl TryInto<ObjectActionHistory> for yarp_graphql::object_actions::ObjectActionHistory {
