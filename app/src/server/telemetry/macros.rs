...
macro_rules! send_telemetry_sync_from_ctx {
    ($event:expr, $ctx:expr) => {
        ...
        if event.enablement_state().is_enabled() {
            let server_api =
                <$crate::server::server_api::ServerApiProvider as yarpui::SingletonEntity>::handle(
                    $ctx,
                )
                .as_ref($ctx)
                .get();
            let privacy_settings_snapshot =
                <$crate::settings::PrivacySettings as yarpui::SingletonEntity>::handle($ctx)
                    .as_ref($ctx)
                    .get_snapshot($ctx);
            let _ = $ctx.spawn(
                async move {
    ...
    ($event:expr, $app_ctx:expr) => {
        ...
        if $event.enablement_state().is_enabled() {
            let server_api =
                ...
                .get();
            let privacy_settings_snapshot =
                <$crate::settings::PrivacySettings as yarpui::SingletonEntity>::handle($app_ctx)
                    .as_ref($app_ctx)
