...
use yarp_core::channel::RudderStackDestination;
use yarpui::telemetry::Event;
...
pub fn clear_event_queue() {
    let _ = yarpui::telemetry::flush_events();
}
...
impl TelemetryApi {
    ...
    pub async fn flush_events(&self, settings_snapshot: PrivacySettingsSnapshot) -> Result<usize> {
        let events = yarpui::telemetry::flush_events();
    ...
    fn persist_events_to_telemetry_log_file(&self, events: Vec<Event>) -> Result<()> {
        let log_directory = yarp_logging::log_directory()?;
    ...
    pub async fn send_telemetry_event(
        ...
        anonymous_id: String,
        event: impl yarp_core::telemetry::TelemetryEvent,
        settings_snapshot: PrivacySettingsSnapshot,
    ) -> Result<()> {
        let event = yarpui::telemetry::create_event(
            user_id.map(|uid| uid.as_string()),
            anonymous_id,
            event.name().into(),
            event.payload(),
            event.contains_ugc(),
            yarpui::time::get_current_time(),
        );
