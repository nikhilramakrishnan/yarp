...
use yarp_graphql::{
    ...
    subscriptions::{
        get_yarp_drive_updates::GetYarpDriveUpdates, start_graphql_streaming_operation,
    },
};
...
impl ObjectClient for ServerApi {
    ...
    async fn get_yarp_drive_updates(
        ...
        let subscription = GetYarpDriveUpdates::build(());
    ...
    async fn untrash_object(&self, id: ServerId) -> Result<ObjectMetadataUpdateResult> {
        ...
        let result = match response.untrash_object {
            yarp_graphql::mutations::untrash_object::UntrashObjectResult::UntrashObjectOutput(
                output,
            ) => {
                if output.success {
