...
fn test_firebase_token_urls() -> Result<()> {
    ...
    assert_eq!(
        custom_token.proxy_url("https://staging.yarp.dev", "api_key"),
        "https://staging.yarp.dev/proxy/customToken?key=api_key"
    );
    assert_eq!(
        refresh_token.proxy_url("https://staging.yarp.dev", "api_key"),
        "https://staging.yarp.dev/proxy/token?key=api_key"
    );
    Ok(())
}
