...
pub async fn execute_command(
    ...
    // when Yarp is launched via launchd (Finder, Dock) with a minimal environment.
...
async fn capture_interactive_shell_env(
    ...
    // controlling terminal with `tcsetpgrp` [1]. If the Yarp process itself tries to read from
    // stdin (for example, some Fuzz CLI commands have interactive inputs), it may get suspended with
