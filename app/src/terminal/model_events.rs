...
impl ModelEventDispatcher {
    ...
    fn handle_terminal_model_event(&mut self, event: Event, ctx: &mut ModelContext<Self>) {
        let event_to_emit = match event {
            ...
            Event::RemoteYarpificationIsUnavailable(reason) => {
...
pub enum ModelEvent {
    ...
    /// useful for detecting when an ssh session becomes ready for yarpification.
    ...
    PromptUpdated,
    /// Emitted when the honor_ps1 state of the shell is out-of-sync with Yarp's settings.
    /// This can happen in cases such as when the user changes between PS1 and Yarp prompt inside
