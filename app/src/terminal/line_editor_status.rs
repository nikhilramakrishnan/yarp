...
use yarpui::{r#async::SpawnedFutureHandle, Entity, ModelContext, ModelHandle};
...
/// This prevents Yarp sending an escape sequence to an arbitrary running program.
///
...
pub struct LineEditorStatus {
    ...
    /// not its a Yarpified session, so to in order properly signal downstream that the line editor
    /// (for Yarpified sessions) is active, we must check if there was a corresponding precmd
...
impl LineEditorStatus {
    ...
    fn mark_line_editor_active_after_delay(
        ...
        let abort_handle = ctx.spawn_abortable(
            async move { yarpui::r#async::Timer::after(delay).await },
            |me, _, ctx| {
