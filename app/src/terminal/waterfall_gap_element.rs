...
use yarpui::elements::ZIndex;
use yarpui::event::ModifiersState;
use yarpui::units::{IntoLines, IntoPixels, Pixels};
...
pub struct WaterfallGapElement {
    ...
    origin: Option<yarpui::elements::Point>,
...
impl Element for WaterfallGapElement {
    ...
    fn paint(&mut self, origin: Vector2F, ctx: &mut yarpui::PaintContext, app: &AppContext) {
        self.origin = Some(yarpui::elements::Point::from_vec2f(
            origin,
            ctx.scene.z_index(),
        ));
    ...
    fn origin(&self) -> Option<yarpui::elements::Point> {
        self.origin
    }
    ...
    fn dispatch_event(
        ...
        if !handled {
            ...
            handled |= match event_at_z_index {
                Some(yarpui::Event::LeftMouseDown { position, .. }) => {
                    self.mouse_down(*position, ctx)
                }
                Some(yarpui::Event::RightMouseDown { position, .. }) => {
...
impl ScrollableElement for WaterfallGapElement {
    fn scroll_data(&self, app: &AppContext) -> Option<yarpui::elements::ScrollData> {
