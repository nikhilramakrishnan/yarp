...
use yarpui::assets::asset_cache::{AssetCache, AssetSource, AssetState};
use yarpui::color::ColorU;
use yarpui::elements::{Border, CornerRadius, Fill, Radius, DEFAULT_UI_LINE_HEIGHT_RATIO};
use yarpui::fonts::{FamilyId, FontId, Properties, Style, Weight};
use yarpui::geometry::rect::RectF;
use yarpui::geometry::vector::{vec2f, Vector2F};
use yarpui::image_cache::{AnimatedImageBehavior, CacheOption, FitType, Image, ImageCache};
use yarpui::platform::LineStyle;
use yarpui::text_layout::{Line, StyleAndFont, TextStyle, DEFAULT_TOP_BOTTOM_RATIO};
use yarpui::units::{IntoLines as _, Lines, Pixels};
use yarpui::{AppContext, Element, EntityId, PaintContext, Scene, SingletonEntity};
...
fn render_grid_without_ligatures<'a>(
    ...
    if FeatureFlag::ITermImages.is_enabled() {
        ...
        if !foreground_image_ids.is_empty() {
            ctx.scene.start_layer(yarpui::ClipBounds::ActiveLayer);
    ...
    for (offset, row_idx) in visible_rows.enumerate() {
        ...
        for col in 0..grid.columns() {
            ...
            // When Yarp draws the cursor (SHOW_CURSOR on), we keep the cell
