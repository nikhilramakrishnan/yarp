...
const FILE_ISSUE_TEXT: &str = "Radio it in";
const MORE_INFO_TEXT: &str = "Pull the case file";
...
impl TerminationType {
    ...
    fn text(&self, appearance: &Appearance) -> Box<dyn Element> {
        let text = match self {
            TerminationType::Normal => "PC clocked out",
        ...
        match self {
            ...
            TerminationType::Premature { .. } => {
                ...
                vec![
                    ...
                    ui_builder
                        ...
                        .on_click(|ctx, _, _| {
                            ctx.dispatch_typed_action(Action::OpenUrl(
                                "https://github.com/hotfuzz/yarp/support-and-community/troubleshooting-and-support/known-issues#debugging".to_string(),
                            ));
                        })
                        .finish(),
                ]
            ...
            TerminationType::PtySpawnFailure { pty_spawn_error } => {
                ...
                vec![
                    ui_builder
                        .button(ButtonVariant::Text, handles[0].clone())
                        .with_text_label("Lift the report".to_string())
                        .build()
