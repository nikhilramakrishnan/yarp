...
use yarp_core::ui::{appearance::Appearance, color::blend::Blend as _};
use yarpui::{
    elements::{
...
impl FileUpload {
    ...
    /// Creates an sftp command that copies a given local file into the PWD of the yarpified ssh session, if any.
    ...
    fn render_clear_upload_button(
        ...
        Container::new(
            icon_button(appearance, Icon::X, true, file.clear_button.clone())
                .with_tooltip(move || ui_builder.tool_tip("Stand the upload down".into()).build().finish())
                .build()
