...
pub enum WithinBlockBanner {
    YarpifyBanner(YarpifyBannerState),
}

impl WithinBlockBanner {
    pub fn banner_height(&self) -> f32 {
        match self.yarpify_mode() {
            Some(YarpificationMode::Ssh { .. }) => {
                BLOCK_BANNER_HEIGHT + BLOCK_BANNER_DESCRIPTION_MAX_HEIGHT
            }
            Some(YarpificationMode::Subshell { .. }) | None => BLOCK_BANNER_HEIGHT,
        }
    }

    pub fn yarpify_mode(&self) -> Option<&YarpificationMode> {
        match self {
            WithinBlockBanner::YarpifyBanner(state) => Some(&state.mode),
        }
    }
}
