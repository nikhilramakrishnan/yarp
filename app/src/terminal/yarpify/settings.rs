...
use crate::terminal::ssh::util::{parse_interactive_ssh_command, SshYarpifyCommand};

// Cannot directly use Vec<Regex> here b/c Regex doesn't impl Eq, Serialize, and Deserialize.
maybe_define_setting!(AddedSubshellCommands, group: YarpifySettings, {
    type: Vec<String>,
    default: Vec::new(),
    supported_platforms: SupportedPlatforms::ALL,
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    private: false,
    toml_path: "yarpify.subshells.added_subshell_commands",
    description: "Additional regex patterns for commands that should be recognized as subshells.",
});

maybe_define_setting!(SubshellCommandsDenylist, group: YarpifySettings, {
    type: Vec<String>,
    default: Vec::new(),
    supported_platforms: SupportedPlatforms::ALL,
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    private: false,
    toml_path: "yarpify.subshells.subshell_commands_denylist",
    description: "Commands that should not trigger the subshell yarpification prompt.",
});

maybe_define_setting!(SshHostsDenylist, group: YarpifySettings, {
    type: Vec<String>,
    default: Vec::new(),
    supported_platforms: SupportedPlatforms::ALL,
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    private: false,
    toml_path: "yarpify.ssh.ssh_hosts_denylist",
    description: "SSH hosts that should not trigger the yarpification prompt.",
});

maybe_define_setting!(EnableSshYarpification, group: YarpifySettings, {
    type: bool,
    default: true,
    supported_platforms: SupportedPlatforms::ALL,
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    private: false,
    toml_path: "yarpify.ssh.enable_ssh_yarpification",
    description: "Whether to enable Yarp features in SSH sessions.",
});

maybe_define_setting!(UseSshTmuxWrapper, group: YarpifySettings, {
    type: bool,
    default: false,
    supported_platforms: SupportedPlatforms::OR(SupportedPlatforms::MAC.into(), SupportedPlatforms::LINUX.into()),
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    private: false,
    toml_path: "yarpify.ssh.use_ssh_tmux_wrapper",
    description: "Whether to use a tmux-based wrapper for SSH yarpification.",
});

/// Controls how Yarp handles the SSH extension (remote server binary) when connecting
...
pub enum SshExtensionInstallMode {
    ...
    AlwaysInstall,
    /// Never install; fall back to legacy yarpification.
    NeverInstall,
}
...
settings::macros::implement_setting_for_enum!(
    ...
    private: false,
    toml_path: "yarpify.ssh.ssh_extension_install_mode",
    description: "Controls SSH extension installation behavior.",
);
...
pub struct YarpifySettings {
    ...
    /// A list of commands that we shouldn't attempt to yarpify. These can be added either b/c the
    /// "don't ask again" button was clicked in the trigger banner, or it was added explicitly on
    /// the Yarpify settings page. This represents the raw, serialized value.
    ...
    /// A list of hosts that we shouldn't attempt to yarpify. This supports regex.
    /// These can be added either b/c the "don't ask again" button was clicked in the trigger banner,
    /// or it was added explicitly on the Yarpify settings page.
    /// While this could live in the `SshSettings` group, the custom processing shared with the other
    /// subshell logic better justifies it living in the `YarpifySettings` group.
    ...
    /// This setting controls whether we should ever yarpify ssh sessions.
    pub enable_ssh_yarpification: EnableSshYarpification,

    /// This setting controls whether we should prompt the user to yarpify an ssh session using the
...
lazy_static! {
    ...
    static ref SUBSHELL_COMMAND_REGEXES: Vec<Regex> = vec![
        ...
        // https://github.com/hotfuzz/Warp/issues/2736
...
impl YarpifySettings {
    fn new_from_storage(ctx: &mut ModelContext<Self>) -> Self {
        ...
        Self {
            ...
            ssh_hosts_denylist,
            enable_ssh_yarpification: EnableSshYarpification::new_from_storage(ctx),
    ...
    pub fn new_with_defaults(_ctx: &mut ModelContext<Self>) -> Self {
        ...
        Self {
            ...
            parsed_ssh_hosts_denylist: Self::parse_ssh_hosts_denylist(&ssh_hosts_denylist),
            ssh_hosts_denylist,
            enable_ssh_yarpification: EnableSshYarpification::new(None),
...
/// define_settings_group!(YarpifySettings). Since we didn't use that macro we define it manually
/// here. It's the event emitted by the setter methods when a setting value changes.
pub enum YarpifySettingsChangedEvent {
    AddedSubshellCommands {
...
impl Entity for YarpifySettings {
    type Event = YarpifySettingsChangedEvent;
}

impl SingletonEntity for YarpifySettings {}
...
impl YarpifySettings {
    ...
    pub fn is_compatible_subshell_command(&self, command: &str, shell_family: ShellFamily) -> bool {
        ...
        if !self.use_ssh_tmux_wrapper.value()
            && SshYarpifyCommand::matches(command)
        ...
        // While in-band generators are our best option for yarpifying ssh sessions from powershell, hard-code
        // the yarpify subshell banner to show up.
    ...
    pub fn add_subshell_command(&mut self, command_to_add: &str, ctx: &mut ModelContext<Self>) {
        ...
        // care of emitting the YarpifySettingsChangedEvent::AddedSubshellCommands event to keep
    ...
    /// Check if the user has asked us to remember a command and avoid asking to yarpify a subshell.
