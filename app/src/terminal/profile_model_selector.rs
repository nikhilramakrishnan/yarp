...
pub fn calculate_scaled_font_size(appearance: &yarp_core::ui::appearance::Appearance) -> f32 {
...
pub fn calculate_max_profile_name_width(appearance: &yarp_core::ui::appearance::Appearance) -> f32 {
...
impl ProfileModelSelector {
    pub fn new(
        ...
        let profile_button = ctx.add_typed_action_view(|ctx| {
            ...
            .with_tooltip("Pick the playbook")
        ...
        let model_button = ctx.add_typed_action_view(|ctx| {
            ...
            .with_tooltip("Pick the officer on the case")
            .with_size(ButtonSize::UDIButton)
        });
        ...
        ctx.subscribe_to_model(&input_model, move |_me, _, event, ctx| match event {
            ...
            | BlocklistAIInputEvent::LockChanged { config } => {
                if config.is_locked && !config.input_type.is_ai() {
                    ...
                    ctx.spawn(
                        yarpui::r#async::Timer::after(NEW_MODEL_CHOICES_POPUP_DELAY),
                        |_, _, ctx| {
                            ctx.notify();
                        },
                    );
