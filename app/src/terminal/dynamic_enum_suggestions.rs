use yarp_completer::completer::{CommandOutput, GeneratorContext};
...
fn parse_dynamic_command(output: &CommandOutput) -> anyhow::Result<Vec<String>> {
    match output.status {
        yarp_completer::completer::CommandExitStatus::Success => {
        ...
        yarp_completer::completer::CommandExitStatus::Failure => {
