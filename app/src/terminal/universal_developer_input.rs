...
use yarpui::ui_components::segmented_control::{LabelConfig, TooltipConfig};
    ...
    fn tooltip_text(&self) -> String {
        match self {
            ...
            AtContextMenuDisabledReason::NoObjectsAvailable => {
                "No leads in the current context.".to_string()
            }
            #[cfg(not(target_family = "wasm"))]
            AtContextMenuDisabledReason::SshSession => "Off-limits in SSH sessions".to_string(),
            #[cfg(not(target_family = "wasm"))]
            AtContextMenuDisabledReason::Subshell => "Off-limits in subshells".to_string(),
            #[cfg(target_family = "wasm")]
...
impl View for UniversalDeveloperInputButtonBar {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn yarpui::Element> {
        ...
        let create_divider = || {
            Container::new(
                yarpui::elements::ConstrainedBox::new(
        ...
        let build_buttons = |model_selector_element: Box<dyn yarpui::Element>| {
