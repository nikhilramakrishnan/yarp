...
impl ActiveSession {
    ...
    /// Returns the `YarpAiExecutionContext` for the active session.
    pub fn ai_execution_environment(&self, app: &AppContext) -> Option<YarpAiExecutionContext> {
        self.session(app).as_ref().map(YarpAiExecutionContext::new)
    }
}
