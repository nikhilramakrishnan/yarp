...
use yarpui::{platform::OperatingSystem, Entity, ModelContext, SingletonEntity};
...
// SessionId is defined in yarp_core and re-exported here for backward compatibility.
pub use yarp_core::SessionId;
...
impl Session {
    ...
    pub fn home_dir(&self) -> Option<&str> {
        if cfg!(test) {
            return yarp_util::path::TEST_SESSION_HOME_DIR.as_deref();
        }
    ...
    pub fn windows_path_converter(&self) -> Option<fn(&str) -> String> {
        if self.is_wsl() {
            Some(yarp_util::path::convert_windows_path_to_wsl)
        } else if self.is_msys2() {
            Some(yarp_util::path::convert_windows_path_to_msys2)
        } else {
            None
        }
    }
