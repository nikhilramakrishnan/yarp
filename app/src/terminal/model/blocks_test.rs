...
use yarpui::{elements::DEFAULT_UI_LINE_HEIGHT_RATIO, App};
...
pub fn test_block_heights_combined_prompt_command_grid_yarp_prompt() {
    ...
    // Note that this test is using the Yarp prompt, hence the prompt is not included in the combined grid.
    ...
    // we have the built-in Yarp prompt, so there's padding between that prompt and the combined grid.
...
pub fn test_block_heights_combined_prompt_command_grid_ps1() {
    ...
    // We have a 2-line prompt, adding 1 extra line to the combined grid (vs 0.6 default for Yarp prompt).
...
fn test_background_blocks_finished() {
    ...
    block_list.start_active_block_for_in_band_command();
    block_list.preexec(PreexecValue {
        command: "yarp_run_generator_command abc".to_owned(),
    });
    command_finished_and_precmd(&mut block_list);
