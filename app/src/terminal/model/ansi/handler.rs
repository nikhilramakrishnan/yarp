...
use yarp_terminal::model::ansi::control_sequence_parameters::*;
...
pub trait Handler {
    ...
    /// Callback for the Yarp CommandFinished hook.
    ...
    /// Callback for the Yarp precmd hook.
    fn precmd(&mut self, _data: PrecmdValue) {}

    /// Callback for the Yarp preexec hook.
    fn preexec(&mut self, _data: PreexecValue) {}

    /// Callback for the Yarp bootstrapped hook - called once when the shell is
    /// bootstrapped
    fn bootstrapped(&mut self, _data: BootstrappedValue) {}

    /// Callback for the Yarp pre-interactive SSH session hook - called once
    ...
    /// Callback for the Yarp ssh hook - called once after successfully connecting to
    /// an SSH server
    ...
    /// Callback for the Yarp exit-shell hook — emitted by the remote shell right
    /// before it exits. Gives the Yarp client a chance to drop per-session
    ...
    /// input buffer (the reporting is itself triggered by Yarp).
    ...
    /// Callback emitted from the yarpify_ssh_session script if it's discovered
    /// that we can't yarpify the remote session.
    fn remote_yarpification_is_unavailable(&mut self, _data: YarpificationUnavailableReason) {}
    ...
    /// These allow external programs to trigger notifications in Yarp.
