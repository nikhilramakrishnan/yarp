...
/// Indicates that the following JSON-encoded message is hex-encoded for Yarp's lifecycle hooks.
...
/// Indicates that the following JSON-encoded message is unencoded for Yarp's lifecycle hooks.
/// In DCS, it is used as the final char in the DCS start sequence.
/// In OSC, it is used as the first parameter.
pub(super) const UNENCODED_JSON_MARKER: char = 'f';

/// Indicates that the following message is a ANSI-C quoted message for receiving Yarp's lifecycle
/// hooks via key-value pairs.
/// In OSC< it is used as the first parameter.
pub(super) const UNENCODED_KV_MARKER: char = 'k';

/// Enum representing all possible JSON payloads for Yarp's DCS's.
...
/// The reason that yarpification was not available when the user tried
/// to yarpify.
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all(serialize = "snake_case"))]
pub enum YarpificationUnavailableReason {
    TmuxFailed,
...
impl PrecmdValue {
    ...
    ///
    /// This relies on the assumption that the yarp_precmd shell function (responsible for writing
...
/// Received from the pty after the shell has finished executing Yarp's
/// bootstrap script.
...
/// Received from the pty when Yarp's SSH wrapper is executed, prior to
...
/// The binding is triggered when Yarp writes the `ESC-i` escape sequence to the pty.
/// Yarp usually does this after a block completes, to collect any typeahead
...
/// Received from the pty when yarp_finish_update is called at the end of an
...
/// `logout`, Ctrl-D on an empty prompt, etc.). Lets the Yarp client drop
