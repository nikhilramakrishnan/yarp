...
pub fn test_long_running_block_bottom_padding() {
    yarpui::r#async::block_on(async {
        ...
        yarpui::r#async::Timer::after(Duration::from_millis(duration)).await;
...
pub fn test_block_emits_block_completed_event_for_in_band_command() {
    ...
    let block_completed_event =
        yarpui::r#async::block_on(pin!(events_rx).find(|event| match event {
...
/// Regression test (CORE-1947): checks Yarp prompt case for is_command_empty. Specifically,
...
fn test_command_is_empty_warp_prompt() {
    ...
    // Note that we are indicating Yarp prompt, not PS1 here!
    let mut block = create_test_block_with_grids(
        block_index,
        prompt_and_command_grid,
        rprompt_grid,
