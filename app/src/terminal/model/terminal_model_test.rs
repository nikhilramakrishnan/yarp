...
use yarp_terminal::model::ansi::ClearMode;
use yarpui::text::str_to_byte_vec;
...
pub fn test_restored_empty_command_block() {
    ...
    // The mocked terminal model comes with a YarpInput block and the active block.
...
fn test_restored_blocks_on_different_host() {
    let restored_blocks = [
        SerializedBlock {
            ...
            stylized_output: str_to_byte_vec("YarpTerminal"),
            pwd: Some("/".to_owned()),
            git_head: None,
            git_branch_name: None,
            virtual_env: None,
    ...
    assert_eq!(blocks[0].output_to_string(), "YarpTerminal",);
