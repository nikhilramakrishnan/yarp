...
use yarp_terminal::model::grid::Dimensions as _;
use yarp_util::path::user_friendly_path;
use yarpui::units::{IntoLines, Lines};
use yarpui::{r#async::executor::Background, record_trace_event};
...
pub struct Block {
    ...
    /// `true` if this command block corresponds to a startup command in an fuzz environment executed
    /// in cloud mode.
...
pub enum BlockState {
    ...
    /// This block holds static content and is programmatically added to the blocklist by Yarp. An
...
impl Block {
    ...
    pub fn new(
        ...
        background_executor: Arc<Background>,
        bootstrap_stage: BootstrapStage,
        show_yarp_bootstrap_input: bool,
        show_in_band_command_blocks: bool,
        show_memory_stats: bool,
        block_index: BlockIndex,
        honor_ps1: bool,
        ...
        Block {
            ...
            event_proxy,
            bootstrap_stage,
            show_bootstrap_block: show_yarp_bootstrap_input,
    ...
    /// line prompt, we render on the same line for PS1, but not for Yarp prompt!
    ...
    /// when the shell echoes the command bytes that Yarp wrote to the PTY.
    ...
    pub fn prompt_and_command_grid_offset(&self) -> Lines {
        if self.header_grid.honor_ps1() {
            ...
            // Grid is drawn below custom Yarp prompt in finished blocks.
            self.block_banner_height()
                + self.padding_top()
                + self.prompt_height()
                + self.command_padding_top()
    ...
    pub fn prompt_and_command_height(&self) -> Lines {
        if !self.ready_to_render() {
            ...
            // Handle the case of Yarp built-in prompt with combined grid.
    ...
    pub fn top_level_command(&self, sessions: &Sessions) -> Option<String> {
        ...
        let command = yarp_completer::parsers::simple::top_level_command(
        ...
        session
            ...
            .and_then(|s| yarp_completer::parsers::simple::top_level_command(s, escape_char))
    ...
    fn wakeup_after_delay(&self) {
        ...
        self.background_executor
            .spawn(async move {
                yarpui::r#async::Timer::after(std::time::Duration::from_millis(delay_ms)).await;
...
impl ansi::Handler for Block {
    ...
    fn linefeed(&mut self) -> ScrollDelta {
        ...
        // prompt, but this is not needed in Yarp due to us visually separating
        // blocks.
        match self.header_grid.receiving_chars_for_prompt {
