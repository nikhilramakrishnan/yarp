...
use yarpui::elements::{ConstrainedBox, Container, Highlight, ParentElement, Shrinkable, Text};
use yarpui::fonts::{Properties, Weight};
use yarpui::prelude::{Align, CrossAxisAlignment, Flex, MainAxisAlignment, MainAxisSize};
use yarpui::scene::{CornerRadius, Radius};
...
impl SearchItem for InlineHistoryItem {
    ...
    fn render_icon(
        ...
        let icon = match &self.item_type {
            ...
            HistoryItemType::Command { .. } => {
                ...
                Container::new(
                    ConstrainedBox::new(Icon::Terminal.to_yarpui_icon(icon_color).finish())
            ...
            HistoryItemType::AIPrompt { .. } => {
                let icon_color = inline_styles::icon_color(appearance);
                Container::new(
                    ConstrainedBox::new(Icon::Prompt.to_yarpui_icon(icon_color).finish())
                        .with_width(icon_size)
                        .with_height(icon_size)
                        .finish(),
                )
    ...
    fn accessibility_label(&self) -> String {
        match &self.item_type {
            ...
            HistoryItemType::AIPrompt { query_text } => format!("PC prompt: {query_text}"),
        }
    }
}
