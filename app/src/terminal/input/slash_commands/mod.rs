            ...
            case if command.name == commands::CASE.name => {
                ...
                let cmd = format!(
                    "mkdir -p /tmp/yarp-radio; \
                     ...
                     if [ -n \"${{YARP_CALLSIGN:-}}\" ]; then \
                       if [ -n \"$cs_avatar\" ]; then \
                         printf '  \\033[2;38;5;244m%-10s\\033[0m %s \\033[1;38;5;%dm@%s\\033[0m \\033[2;3;38;5;%dmon case\\033[0m\\n' 'filed by' \"$cs_avatar\" \"$cs_color\" \"${{YARP_CALLSIGN}}\" \"$cs_color\"; \
                       else \
            ...
            _duty if command.name == commands::DUTY.name => {
                ...
                let cmd = if let Some(cs) = callsign {
                    if cs.eq_ignore_ascii_case("off") || cs.eq_ignore_ascii_case("clear") {
                        ...
                        let claim_color: u8 = match cs {
                            "nicholas" | "angel" => 39,
                            "frank" | "butterman.snr" => 220,
                            "danny" | "butterman" => 213,
                            "andy" | "wainwright" | "cartwright" => 208,
                            "doris" | "thatcher" => 165,
                            "tony" => 226,
                            _ => 179,
                        };
            ...
            _clock_out if command.name == commands::CLOCK_OUT.name => {
                ...
                let cmd = format!(
                    "user=\"${{USER:-unknown}}\"; \
                     ...
                     printf '\\033[1;38;5;220m🌙 OFF DUTY\\033[0m \\033[1;38;5;179m%s\\033[0m \\033[2;3;38;5;179mshift end\\033[0m\\n\\n' \"$when\"; \
                    ...
                    "user=\"${{USER:-unknown}}\"; \
                     ...
                     if [ \"$n\" -gt 0 ]; then \
                       ...
                       if [ -z \"$callsign\" ] && [ \"$for_others\" -gt 0 ]; then \
                         ...
                         printf '  \\033[2;38;5;244m%-10s\\033[0m 📨 \\033[1;38;5;179m%s\\033[0m \\033[2;3;38;5;179m— /duty <name> to read\\033[0m\\n' 'for others' \"$others_summary\"; \
                       fi; \
                     else \
                       hour=$(date +%H); \
                       case \"$hour\" in \
                ...
                if sub.eq_ignore_ascii_case("clear") {
                    let clear_cmd = "if [ -n \"$ZSH_VERSION\" ]; then setopt KSH_ARRAYS NULL_GLOB 2>/dev/null; else shopt -s nullglob 2>/dev/null; fi; \
                        ...
                        if [ \"$removed\" -eq 0 ]; then \
                          printf '\\033[1;38;5;220m📻 INBOX\\033[0m \\033[2;3;38;5;179mnothing to clear\\033[0m\\n'; \
                ...
                let cmd = "if [ -n \"$ZSH_VERSION\" ]; then setopt KSH_ARRAYS NULL_GLOB 2>/dev/null; else shopt -s nullglob 2>/dev/null; fi; \
                    ...
                    if [ ${#files[@]} -eq 0 ]; then \
                      ...
                      if [ ${#relay_q[@]} -gt 0 ]; then \
                        printf '\\033[2;38;5;240m  ▸ RELAY\\033[0m \\033[2;3;38;5;240m(%d)\\033[0m \\033[2;3;38;5;240mnot for you\\033[0m\\n' \"${#relay_q[@]}\"; \
