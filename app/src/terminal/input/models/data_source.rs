...
use yarpui::{AppContext, Element, Entity, EntityId, SingletonEntity as _};
...
impl SearchItem for ModelSearchItem {
    ...
    fn render_item(
        ...
        use yarpui::elements::{Flex, ParentElement as _};
    ...
    fn render_details(&self, app: &AppContext) -> Option<Box<dyn Element>> {
        ...
        let cost_row = if is_using_api_key {
            let manage_button = appearance
                ...
                .with_text_label("Inspect kit".to_string())
...
/// Discounts only apply when the user is billing through Yarp credits,
