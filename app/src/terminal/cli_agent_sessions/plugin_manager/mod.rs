...
/// Manages the Yarp notification plugin for a specific CLI agent.
///
...
pub(crate) trait CliAgentPluginManager: Send + Sync {
    /// The minimum plugin version required by this Yarp build.
    ...
    /// Whether the Yarp notification plugin is installed.
    ...
    /// Install the Yarp notification plugin.
    ...
    /// Update the Yarp notification plugin to the latest version.
    /// Default returns an error — only agents with `can_auto_install() == true` should override.
    ...
    /// Install the Fuzz platform plugin for this CLI agent, if one exists,
...
/// doesn't have Yarp notification plugin support.
