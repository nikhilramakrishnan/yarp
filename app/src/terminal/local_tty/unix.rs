...
fn build_host_shell_command(
    ...
    builder.env("TERM_PROGRAM", "YarpTerminal");
    ...
    if let Some(version) = ChannelState::app_version() {
        ...
        builder.env("YARP_CLIENT_VERSION", "local");
    }
    ...
    if enable_ssh_wrapper {
        builder.env("YARP_USE_SSH_WRAPPER", "1");
    } else {
        builder.env("YARP_USE_SSH_WRAPPER", "0");
    }
    ...
    if FeatureFlag::HOANotifications.is_enabled() {
        builder.env(
            "YARP_CLI_AGENT_PROTOCOL_VERSION",
            current_protocol_version().to_string(),
        );
    }

    if shell_debug_mode {
        builder.env("YARP_SHELL_DEBUG_MODE", "1");
    }
    if honor_ps1 {
        builder.env("YARP_HONOR_PS1", "1");
    } else {
        builder.env("YARP_HONOR_PS1", "0");
    }
    ...
    let path_append = extra_path_entries()
        ...
        .join(":");
    builder.env("YARP_PATH_APPEND", path_append);
    ...
    if matches!(shell_starter.shell_type(), ShellType::Bash) {
        ...
        builder.env("YARP_INITIAL_HISTFILESIZE", sentinel_value);
    }
    ...
    if let Some(start_dir) = start_dir {
        builder.env("YARP_INITIAL_WORKING_DIR", start_dir);
    }
...
fn build_docker_sandbox_command(
    ...
    // split. Many of these (e.g. `YARP_USE_SSH_WRAPPER`,
    // `SSH_SOCKET_DIR`, `HISTFILESIZE`, `YARP_IS_LOCAL_SHELL_SESSION`)
    ...
    builder.env("YARP_HONOR_PS1", if honor_ps1 { "1" } else { "0" });
    let path_append = extra_path_entries()
        .map(|p| p.to_string_lossy().into_owned())
        .join(":");
    builder.env("YARP_PATH_APPEND", path_append);
    ...
    builder.env("HISTFILESIZE", sentinel_value);
    builder.env("YARP_INITIAL_HISTFILESIZE", sentinel_value);
    // Intentionally do NOT set `YARP_INITIAL_WORKING_DIR` for sandboxes:
