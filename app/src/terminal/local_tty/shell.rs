...
/// * On macOS, this includes `$APP_PATH/Contents/Resources/bin`, in which we put a wrapper around the Yarp CLI.
...
pub fn extra_path_entries() -> impl Iterator<Item = PathBuf> {
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            ...
            if let Some(resources_path) = yarp_core::paths::bundled_resources_dir() {
...
/// executable binary for one of Yarp's supported shell types (bash, fish, zsh).
...
impl ShellStarter {
    ...
    /// used to spawn a shell process for a new top-level Yarp session. If a WSL Distribution is
...
pub struct DirectShellStarter {
    ...
    /// Arguments to be passed to the shell binary at [`shell_path`] when spawning a new Yarp
    /// session.
    args: Vec<OsString>,
}
...
impl WslShellStarter {
    ...
    pub(super) fn home_directory(&self) -> Option<PathBuf> {
        ...
        yarp_util::path::convert_wsl_to_windows_host_path(
...
fn arguments_for_session_spawning_command(
    ...
    match shell_type {
        ShellType::Zsh => {
            ...
            // begins with a space. We use this to hide Yarp bootstrap commands from the history.
            vec![
                "-c".to_owned().into(),
        ...
        ShellType::Fish => {
            ...
            vec![
                ...
                format!(
                    ...
                    // See this issue: https://github.com/hotfuzz/yarp/issues/7588
