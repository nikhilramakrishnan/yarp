...
//!
//! This module owns everything specific to running a Yarp shell inside a
...
/// Prefix for generated container names: `yarp-sandbox-<id>`.
const DOCKER_SANDBOX_NAME_PREFIX: &str = "yarp-sandbox";

/// Root directory on the host under which Docker-sandbox scratch files
/// (bash init scripts, empty workspace mount points) live.
///
/// Lives under the Yarp per-user cache directory rather than `/tmp` so:
...
fn docker_sandbox_host_root() -> PathBuf {
    yarp_core::paths::cache_dir().join("docker-sandbox")
}

/// Resolves the absolute path to the `sbx` CLI binary using the Yarp
/// process's `PATH`.
///
/// Yarp's process `PATH` is minimal and often misses user-shell-installed
/// tools (e.g. homebrew on Apple Silicon when Yarp is launched from Finder,
...
///
/// Each instance carries a unique `sandbox_id` so multiple Yarp panes can run
...
impl DockerSandboxShellStarter {
    ...
    /// Host directory where Yarp writes this sandbox's bash init script.
