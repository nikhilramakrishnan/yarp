...
use yarpui::{AppContext, ModelContext, ModelHandle, SingletonEntity, ViewHandle, WindowId};
...
impl TerminalManager {
    ...
    fn create_pty(
        ...
        let enable_ssh_wrapper = if FeatureFlag::SSHTmuxWrapper.is_enabled() {
            *YarpifySettings::as_ref(ctx)
                .enable_ssh_yarpification
                .value()
                && !*YarpifySettings::as_ref(ctx).use_ssh_tmux_wrapper.value()
        } else {
...
pub fn get_shell_starter(
    ...
    shell_starter_or_wsl_name
        .and_then(|starter| {
            yarpui::r#async::block_on(async { starter.to_shell_starter_source().await })
        })
        .map(|starter_source| {
