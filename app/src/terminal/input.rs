...
pub(super) const CLI_AGENT_RICH_INPUT_HINT_TEXT: &str = "Brief the PC on the case...";

const CLOUD_MODE_V2_HINT_TEXT: &str = "Dispatch a cloud unit";
...
pub const AI_COMMAND_SEARCH_HINT_TEXT: &str = "Type '#' to radio in for command suggestions";
...
const AGENT_MODE_AI_ENABLED_STEER_HINT_TEXT_UDI: &str = "Redirect the PC on patrol";
const AGENT_MODE_AI_ENABLED_STEER_HINT_TEXT_CLASSIC: &str =
    "Redirect the PC on patrol, or backspace to call off";
const AGENT_MODE_AI_ENABLED_FOLLOW_UP_HINT_TEXT_UDI: &str = "Radio a follow-up";
const AGENT_MODE_AI_ENABLED_FOLLOW_UP_HINT_TEXT_CLASSIC: &str =
    "Radio a follow-up, or backspace to call off";
...
pub enum CommandExecutionSource {
    /// A non-shared command execution request from Yarp AI++.
...
impl Input {
    ...
    fn agent_mode_hint_text(&mut self, app: &AppContext) -> &str {
        ...
        match (
            ...
            (InputType::AI, _) => {
                ...
                //  * `None` (no conversation, empty, passive, or untitled) => new conversation => "Yarp anything"
    ...
    /// Predicts the next action using an AI model and past context on blocks within Yarp.
    ...
    fn env_vars_command_prefix(&self, env_vars_id: &SyncId, ctx: &AppContext) -> Option<String> {
        ...
        if shell_type == ShellType::Fish {
            // Yarp currently doesn't support newlines in Fish, just prepend the vars
        ...
        match event {
                ...
                match &mode {
                    InputSuggestionsMode::CompletionSuggestions {
                                    ...
                                    && selected_item_differs_from_current_word))
                        {
                            ...
                            let _ = ctx.spawn(
                                async move {
                                    yarpui::r#async::Timer::after(Duration::from_millis(750)).await;
                            ...
                            #[cfg(feature = "local_fs")]
                            {
                                ...
                                let processed_path = self
                                    ...
                                    .and_then(|pwd| {
                                        ...
                                        let relative_path = yarp_util::path::to_relative_path(
                                            is_wsl,
                                            &absolute_path,
                                            Path::new(pwd),
                                        );
            ...
            EditorEvent::DroppedImageFiles(image_filepaths) => {
                ...
                if num_attached < image_filepaths.len() {
                    ...
                    let paths_str =
                        yarpui::clipboard_utils::escaped_paths_str(&transformed, shell_family);
    ...
    fn process_paste_event(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if yarpui::clipboard::should_insert_text_on_paste(&content) {
        ...
        let should_insert_text = if content.has_image_data() {
            ...
            let image_filepaths = yarpui::clipboard_utils::get_image_filepaths_from_paths(
