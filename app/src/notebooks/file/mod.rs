...
/// View for a read-only notebook backed by a file, rather than Yarp Drive.
...
// TODO: This should probably build on the `yarp_files` abstractions.
#[derive(Debug, Clone)]
enum SourceFile {
    Local {
...
pub fn init(app: &mut AppContext) {
    ...
    app.register_editable_bindings([
        EditableBinding::new(
            ...
            "Focus the radio mic from the case file",
    ...
    fn render_header_content(
        ...
        if self.is_markdown_file() {
            ...
            let title_text = render_pane_header_title_text(
                title,
                appearance,
                yarpui::text_layout::ClipConfig::start(),
            );
