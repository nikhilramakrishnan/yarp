...
use crate::{
    ...
    drive::{
        ...
        export::ExportManager, items::YarpDriveItemId, sharing::ShareableObject,
        CloudObjectTypeAndId, OpenYarpDriveObjectSettings,
    },
    editor::{
...
/// A view that allows viewing/execution and editing of a Yarp notebook.
...
impl NotebookView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let title = ctx.add_typed_action_view(|ctx| {
            ...
            editor.set_placeholder_text("Unfiled casebook", ctx);
            editor
        });
    ...
    fn title_from_editor(title_editor: &ViewHandle<EditorView>, app: &AppContext) -> String {
        ...
        if title.is_empty() {
            title.push_str("Unfiled casebook");
        }
        title
    }
        ...
        match event {
            ...
            ActiveNotebookDataEvent::CreatedOnServer => {
                    ...
                    .and_then(SyncId::into_server)
                {
                    self.pane_configuration.update(ctx, |pane_config, ctx| {
                        pane_config
                            .set_shareable_object(Some(ShareableObject::YarpDriveObject(id)), ctx);
                    })
                }
            }
    ...
    fn view_in_yarp_drive(&mut self, id: YarpDriveItemId, ctx: &mut ViewContext<Self>) {
        ctx.emit(NotebookEvent::ViewInYarpDrive(id));
    }
    ...
    fn overflow_menu_items(&self, ctx: &AppContext) -> Vec<MenuItem<NotebookAction>> {
        ...
        if let Some(ai_document_id) = self.active_notebook_data.as_ref(ctx).ai_document_id(ctx) {
            menu_items.push(
                MenuItemFields::new("Sign on to active beat")
                ...
                action_row.add_child(
                    Align::new(
                        appearance
                            ...
                            .with_text_label("Reinstate".to_string())
                            .build()
                            .on_click(|ctx, _, _| {
                    ...
                    Container::new(
                        Align::new(
                            appearance
                                ...
                                .with_text_label("Lift to your dossier".to_string())
                                .build()
                                .on_click(|ctx, _, _| {
            ...
            Container::new(
                Align::new(
                    appearance
                        ...
                        .with_text_label("Lift everything".to_string())
                        .build()
                        .on_click(|ctx, _, _| {
...
impl TypedActionView for NotebookView {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            NotebookAction::ViewInYarpDrive(id) => self.view_in_yarp_drive(*id, ctx),
            NotebookAction::FocusTerminalInput => {
            ...
            NotebookAction::CopyLink(link) => {
                ...
                ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    toast_stack.add_ephemeral_toast(
                        DismissibleToast::success("Frequency bagged.".to_string()),
                        window_id,
                        ctx,
                    );
                });
