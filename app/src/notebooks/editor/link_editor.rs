use yarp_editor::{editor::NavigationKey, model::RichTextEditorModel, render::model::RenderState};
use yarpui::{
    elements::{
...
impl LinkEditor {
    pub fn new(model: ModelHandle<NotebooksEditorModel>, ctx: &mut ViewContext<Self>) -> Self {
        ...
        let tag_editor = ctx.add_typed_action_view(|ctx| {
            ...
            editor.set_placeholder_text("Label on the wire", ctx);
            editor
        });
        ...
        let url_editor = ctx.add_typed_action_view(|ctx| {
            let mut editor = EditorView::single_line(editor_options.clone(), ctx);
            editor.set_placeholder_text("Wire (web or file)", ctx);
            editor
        });
...
impl View for LinkEditor {
    ...
    fn render(&self, app: &yarpui::AppContext) -> Box<dyn Element> {
        ...
        let mut link_button = appearance
            ...
            .with_centered_text_label("File the link".to_string());
