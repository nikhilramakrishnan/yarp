...
use markdown_parser::html_parser::YARP_EMBED_ATTRIBUTE_NAME;
...
use crate::{
    ...
    drive::{cloud_object_styling::yarp_drive_icon_color, DriveObjectType},
...
impl EmbeddedItem for EmbeddedWorkflow {
    ...
    fn to_rich_format(&self, app: &AppContext) -> EmbeddedItemRichFormat<'_> {
        ...
        // attributes so we could re-parse the ID and metadata when pasted into Yarp.
        ...
        EmbeddedItemRichFormat {
            ...
            html: EmbeddedItemHTMLRepresentation {
                ...
                attributes: HashMap::from([(YARP_EMBED_ATTRIBUTE_NAME, self.hashed_id())]),
            },
        }
    }
}
...
impl RenderableEmbeddedWorkflow {
    pub fn new(
        ...
        let (icon, icon_color) = if is_agent_mode_prompt {
            (
                Icon::Prompt,
                yarp_drive_icon_color(appearance, DriveObjectType::AgentModeWorkflow),
            )
        } else {
            (
                Icon::Workflow,
                yarp_drive_icon_color(appearance, DriveObjectType::Workflow),
            )
        };
...
impl RenderableBlock for RenderableEmbeddedWorkflow {
    ...
    fn paint(&mut self, model: &RenderState, ctx: &mut RenderContext, app: &AppContext) {
        ...
        ctx.paint.scene.start_layer(yarpui::ClipBounds::ActiveLayer);
    ...
    fn after_layout(&mut self, ctx: &mut yarpui::AfterLayoutContext, app: &yarpui::AppContext) {
        self.footer.after_layout(ctx, app);
    }

    fn dispatch_event(
        &mut self,
        _model: &yarp_editor::render::model::RenderState,
