...
use yarp_editor::content::buffer::{AutoScrollBehavior, BufferSelectAction, SelectionOffsets};
use yarp_editor::content::text::{BlockType, BufferBlockStyle, CodeBlockType, TextStyles};
use yarp_editor::model::{CoreEditorModel, RichTextEditorModel};
use yarp_editor::render::model::viewport::SizeInfo;
use yarp_editor::render::model::BlockItem;
use yarp_editor::render::model::RenderEvent;
use yarp_editor::selection::{TextDirection, TextUnit};
use yarpui::elements::ListIndentLevel;
use yarpui::platform::WindowStyle;
use yarpui::presenter::ChildView;
use yarpui::r#async::{block_on, FutureId};
use yarpui::text::word_boundaries::WordBoundariesPolicy;
use yarpui::{r#async::Timer, App, Entity, ModelHandle, SingletonEntity, TypedActionView};
use yarpui::{AddSingletonModel, AppContext, Element, View, ViewHandle};
...
fn test_inline_markdown() {
    App::test((), |mut app| async move {
        ...
        editor.update(&mut app, |editor, ctx| {
            ...
            editor.user_insert("[abc](https://github.com/hotfuzz/yarp", ctx);
        });

        editor.read(&app, |editor, ctx| {
            assert_eq!(
                editor.content.as_ref(ctx).debug(),
                "<text>First[abc](https://github.com/hotfuzz/yarp <b_s>bold<b_e>"
            );
        });
        ...
        editor.read(&app, |editor, ctx| {
            ...
            assert_eq!(
                editor.content.as_ref(ctx).debug(),
                "<text>First<a_https://github.com/hotfuzz/yarp>abc<a> <b_s>bold<b_e>"
            );
        });
        ...
        editor.update(&mut app, |editor, ctx| {
            ...
            assert_eq!(
                editor.content.as_ref(ctx).debug(),
                "<text>First<a_https://github.com/hotfuzz/yarp>abc<a> <b_s>bold`abc<b_e>"
            );
        ...
        editor.read(&app, |editor, ctx| {
            ...
            assert_eq!(
                editor.content.as_ref(ctx).debug(),
                "<text>First<a_https://github.com/hotfuzz/yarp>abc<a> <b_s>bold<b_e><c_s>abc<c_e>"
            );
        });
    })
}
...
fn test_pasting_link_on_selected_text() {
    App::test((), |mut app| async move {
        initialize_deps(&mut app);
        let editor = model_from_markdown("First text\nSecond line", &mut app, true);
        let clipboard_content = "https://github.com/hotfuzz/yarp";
        ...
        editor.update(&mut app, |editor, ctx| {
            ...
            assert_eq!(
                ...
                "<text><a_https://github.com/hotfuzz/yarp>First text<a>\\nSecond line"
            );
        });
    });
}
...
fn test_cut_code_block() {
    App::test((), |mut app| async move {
        ...
        model_handle.update(&mut app, |model, ctx| {
            ...
            assert_eq!(
                clipboard.html.unwrap(),
                r#"<pre><code class="language-yarp-runnable-command">command</code></pre>"#
            );
        })
    });
}
...
fn test_multiselect_pasting() {
    App::test((), |mut app| async move {
        ...
        let url_clipboard_content = "https://github.com/hotfuzz/yarp";
        ...
        editor.update(&mut app, |editor, ctx| {
            ...
            assert_eq!(
                editor.debug_buffer(ctx),
                "<text><a_https://github.com/hotfuzz/yarp>https://github.com/hotfuzz/yarp<a>\\nSecond line\\n<a_https://github.com/hotfuzz/yarp>https://github.com/hotfuzz/yarp<a><code:Shell><c_#b4fa72>code<c><text>"
            );
        });
        ...
        editor.update(&mut app, |editor, ctx| {
            ...
            assert_eq!(
                editor.debug_buffer(ctx),
                "<text>echo test<a_https://github.com/hotfuzz/yarp>https://github.com/hotfuzz/yarp<a>\\nSecond line\\n<a_https://github.com/hotfuzz/yarp>https://github.com/hotfuzz/yarp<a><code:Shell><c_#b4fa72>coecho testde<c><text>"
            );
        });
    });
}
