...
use yarp_editor::editor::EditorView;
use yarpui::{
...
fn open_notebook(
    ...
    let load_future = handle.update(app, |view, ctx| {
        view.load(notebook, &OpenYarpDriveObjectSettings::default(), ctx)
    });
...
fn test_not_eager_baton_grab_different_editor() {
    App::test((), |mut app| async move {
        ...
        let uid = "ian@yarp.dev".to_string();
        let email = "ian@yarp.dev".to_string();
...
fn test_baton_grab_editor_changed_offline() {
    App::test((), |mut app| async move {
        ...
        let other_uid = "ben@yarp.dev";
        let other_email = "ben@yarp.dev";
