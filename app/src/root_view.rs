...
use yarp_graphql::billing::StripeSubscriptionPlan;

use yarpui::elements::{
    Border, ChildAnchor, OffsetPositioning, ParentAnchor, ParentElement, ParentOffsetBounds, Stack,
};
use yarpui::rendering::OnGPUDeviceSelected;
use yarpui::{id, AddWindowOptions, DisplayId, SingletonEntity};
use yarpui::{
...
use yarpui::{FocusContext, NextNewWindowsHasThisWindowsBoundsUponClose};
        ...
        let safe_repos = self
            ...
            .filter(|repo| {
                ...
                Url::parse(repo).is_ok()
                    ...
                    || yarp_util::path::is_posix_portable_pathname(repo)
                    // Or absolute POSIX paths with portable components (e.g., /Users/me/repo)
                    || repo
                        .strip_prefix('/')
                        .is_some_and(yarp_util::path::is_posix_portable_pathname)
            })
            .join(" ");
...
///
/// This is the canonical way to open a new Yarp window - all other entrypoints should delegate to
/// it if possible.
            ...
            workspace_handle.update(ctx, |workspace, ctx| {
                workspace
                    ...
                    .update(ctx, |pane_group, ctx| {
                        pane_group.set_title("Open Up the Beat", ctx);
        ...
        None => {
            ...
            let (id, _) = ctx.add_window(
                AddWindowOptions {
                    ...
                    anchor_new_windows_from_closed_position:
                        yarpui::NextNewWindowsHasThisWindowsBoundsUponClose::No,
                    on_gpu_driver_selected: on_gpu_driver_selected_callback(),
                    window_instance: Some(ChannelState::app_id().to_string() + "-hotkey"),
                    ..Default::default()
                },
...
/// This action will show or hide all of Yarp's windows except the quake window
///
/// - If Yarp is active and has any windows, hide those windows.
/// - If Yarp is hidden, show all windows.
/// - If Yarp is active but has 0 normal windows, create a new window with a new session.
fn show_or_hide_non_quake_mode_windows(_: &(), ctx: &mut AppContext) {
    ...
    // Now there is at least one window. If a Yarp window is active, hide the app.
...
impl RootView {
    pub fn new(
        ...
        match &root_view.auth_onboarding_state {
            ...
            AuthOnboardingState::Auth(_) => {
                ...
                #[cfg(target_os = "macos")]
                {
                    use yarpui_extras::user_preferences::UserPreferences;
                    ...
                    let user_defaults = yarpui_extras::user_preferences::user_defaults::UserDefaultsPreferencesStorage::new(None);
                    if user_defaults
    ...
    pub fn open_yarp_drive_object_in_existing_window(
        ...
        if let AuthOnboardingState::Terminal(handle) = &self.auth_onboarding_state {
            ...
            log::warn!("Auth not complete before trying to open yarp drive object");
        }
        true
    }
    ...
    pub fn handle_team_intent_link_action(&mut self, _: &(), ctx: &mut ViewContext<Self>) -> bool {
        ...
        if let AuthOnboardingState::Terminal(handle) = &self.auth_onboarding_state {
            ...
            log::error!("Auth not complete before trying to open yarp drive");
        }
        ...
        match event {
            AuthManagerEvent::AuthComplete => {
                ...
                } else if let AuthOnboardingState::NeedsSsoLink { .. } = &self.auth_onboarding_state
                {
                    ...
                    // since we should err on the side of them _not_ being able to use Yarp.
                ...
                UserAuthenticationError::UserAccountDisabled(_) => {
                    cfg_if! {
                        if #[cfg(target_family = "wasm")] {
                            ...
                            // to use Yarp. Instead, they can sign in or up with a valid account.
    ...
    fn maybe_stop_active_voice_input(
        &mut self,
        key_code: &yarpui::platform::keyboard::KeyCode,
        ctx: &mut ViewContext<Self>,
    ) -> bool {
        use crate::settings::AISettings;
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        cfg_if::cfg_if! {
            if #[cfg(feature = "voice_input")] {
                use yarpui::elements::{EventHandler, DispatchEventResult};
                EventHandler::new(stack.finish())
                    .on_modifier_state_changed(|ctx, _app, key_code, key_state| {
                        if matches!(key_state, yarpui::event::KeyState::Released) {
