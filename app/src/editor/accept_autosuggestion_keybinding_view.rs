...
use yarpui::elements::{Border, ChildView, Flex, ParentElement};
use yarpui::elements::{
...
use yarpui::{Entity, TypedActionView, View, ViewHandle};
    ...
    fn render(&self, ctx: &AppContext) -> Box<dyn Element> {
        ...
        Hoverable::new(self.autosuggestion_hint_mouse_handle.clone(), |state| {
            ...
            let chevron_down = Container::new(
                ConstrainedBox::new(
                    Icon::ArrowDropDown
                        .to_yarpui_icon(Fill::Solid(font_color))
                        .finish(),
                )
            ...
            if !is_menu_open && state.is_hovered() {
                let tool_tip = appearance
                    .ui_builder()
                    .autosuggestion_tool_tip("Reassign the shortcut".into())
                    .build()
                    .finish();
                stack.add_positioned_overlay_child(
                    tool_tip,
