...
use vec1::{vec1, Vec1};
use yarp_core::{safe_error, send_telemetry_from_ctx};
use yarp_util::{path::ShellFamily, user_input::UserInput};
...
use yarpui::{elements, ViewHandle};
...
use yarpui::elements::{MouseStateHandle, Radius};
use yarpui::fonts::{FamilyId, Properties, Weight};
use yarpui::keymap::{Keystroke, PerPlatformKeystroke};
use yarpui::platform::{Cursor, FilePickerConfiguration, OperatingSystem};
...
use yarpui::{windowing, BlurContext, EntityId, FocusContext};
use yarpui::{CursorInfo, ModelContext, SingletonEntity, TypedActionView};
...
use yarpui::clipboard_utils::CLIPBOARD_IMAGE_MIME_TYPES;
...
pub type KeymapContextModifierFn = Box<dyn Fn(&mut yarpui::keymap::Context, &AppContext)>;
...
impl EditorView {
    ...
    fn drag_and_drop_files(&mut self, paths: &[UserInput<String>], ctx: &mut ViewContext<Self>) {
        ...
        let image_filepaths =
            yarpui::clipboard_utils::get_image_filepaths_from_paths(&paths_as_strings);
        ...
        let input =
            yarpui::clipboard_utils::escaped_paths_str(&transformed_paths, self.shell_family);
