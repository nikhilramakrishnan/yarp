...
use yarpui::text_layout::TextFrame;
use yarpui::windowing::WindowManager;
use yarpui::{AddSingletonModel, App, UpdateModel, UpdateView};
...
fn test_autosuggestions() -> Result<()> {
    use yarpui::text_layout::LayoutCache;
...
fn test_paste_clipboard_with_text_only_should_paste_text_normally() {
    App::test((), |mut app| async move {
        ...
        app.update(|ctx| {
            let clipboard_content = yarpui::clipboard::ClipboardContent {
...
fn test_paste_clipboard_with_image_only_should_switch_to_agent_mode() {
    App::test((), |mut app| async move {
        ...
        app.update(|ctx| {
            let png_image = yarpui::clipboard::ImageData {
...
fn test_paste_clipboard_with_unsupported_image_and_text_should_show_error() {
    App::test((), |mut app| async move {
        ...
        app.update(|ctx| {
            let bmp_image = yarpui::clipboard::ImageData {
    ...
    App::test((), |mut app| async move {
        ...
        view.update(&mut app, |view, ctx| {
            ...
            view.set_drag_drop_path_transformer(Some(Box::new(
                yarp_util::path::convert_windows_path_to_wsl,
            )));
            view.drag_and_drop_files(&paths(), ctx);
        ...
        view.update(&mut app, |view, ctx| {
            view.clear_buffer(ctx);
            view.set_drag_drop_path_transformer(Some(Box::new(
                yarp_util::path::convert_windows_path_to_msys2,
            )));
            view.drag_and_drop_files(&paths(), ctx);
