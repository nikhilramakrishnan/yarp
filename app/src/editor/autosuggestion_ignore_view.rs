...
use yarpui::{Entity, TypedActionView, View};
...
impl View for AutosuggestionIgnore {
    ...
    fn render(&self, ctx: &yarpui::AppContext) -> Box<dyn Element> {
        ...
        let line_height_ratio = appearance
            .line_height_ratio()
            .min(yarpui::elements::DEFAULT_UI_LINE_HEIGHT_RATIO);
        ...
        Hoverable::new(self.autosuggestion_ignore_mouse_handle.clone(), |state| {
            ...
            let mut ignore_button = Container::new(close_icon)
                ...
                .with_border(
                    yarpui::elements::Border::all(border_width).with_border_color(disabled_color),
                )
            ...
            if state.is_hovered() {
                let tool_tip = appearance
                    .ui_builder()
                    .autosuggestion_tool_tip("Wave the tip-off off".into())
                    .build()
                    .finish();
