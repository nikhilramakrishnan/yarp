...
use yarpui::{Entity, ModelAsRef, ModelContext, ModelHandle, SingletonEntity};
...
impl CurrentPrompt {
    ...
    ) -> (Option<yarp_completer::completer::CommandOutput>, bool) {
        ...
        Prompt::as_ref(ctx)
            ...
            .filter_map(|chip_kind| {
                ...
                if has_value && chip_kind.is_copyable() {
                    if let Some(chip) = chip_kind.to_chip() {
                        Some(
                            MenuItemFields::new(format!("Bag the {}", chip.title()))
