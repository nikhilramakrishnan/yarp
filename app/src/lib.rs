...
use yarp_cli::{agent::AgentCommand, CliCommand};
...
pub use yarp_core::errors::{report_error, report_if_error};
...
use yarp_logging::LogDestination;

// Re-export the send_telemetry_from_ctx macro at the crate root level
pub use yarp_core::send_telemetry_from_app_ctx;
pub use yarp_core::send_telemetry_from_ctx;

// Re-export the safe logging macros at the crate root level for backwards compatibility
pub use yarp_core::{safe_debug, safe_error, safe_info, safe_warn};

use crate::antivirus::AntivirusInfo;
#[cfg(feature = "local_fs")]
use yarp_files::FileModel;
use yarpui::platform::TerminationMode;
use yarpui::windowing::state::ApplicationStage;
use yarpui::{AppContext, SingletonEntity, WindowId};
...
impl LaunchMode {
    fn args(&self) -> Cow<'_, yarp_cli::AppArgs> {
        match self {
            LaunchMode::App { args, .. } => Cow::Borrowed(args),
            _ => Cow::Owned(yarp_cli::AppArgs::default()),
        }
    }
...
pub fn run() -> Result<()> {
    ...
    let args = yarp_cli::Args::from_env();
    ...
    if let Some(command) = args.command() {
        ...
        match command {
            ...
            yarp_cli::Command::Worker(yarp_cli::WorkerCommand::TerminalServer(args)) => {
            ...
            #[cfg(feature = "plugin_host")]
            yarp_cli::Command::Worker(yarp_cli::WorkerCommand::PluginHost { .. }) => {
                return crate::run_plugin_host();
            }
            #[cfg(feature = "local_tty")]
            yarp_cli::Command::Worker(yarp_cli::WorkerCommand::MinidumpServer { socket_name }) => {
                cfg_if::cfg_if! {
            ...
            #[cfg(not(target_family = "wasm"))]
            yarp_cli::Command::Worker(yarp_cli::WorkerCommand::RemoteServerProxy) => {
                return crate::remote_server::run_proxy();
            }
            #[cfg(not(target_family = "wasm"))]
            yarp_cli::Command::Worker(yarp_cli::WorkerCommand::RemoteServerDaemon) => {
                return crate::remote_server::run_daemon();
            }
            #[cfg(not(target_family = "wasm"))]
            yarp_cli::Command::Worker(yarp_cli::WorkerCommand::RipgrepSearch {
                parent,
                ignore_case,
                multiline,
                pattern,
                ...
                yarp_ripgrep::search::run_search_subprocess(
            ...
            yarp_cli::Command::Worker(worker) => {
                // Need this case to handle platforms where there are no enum variants in
                // yarp_cli::WorkerCommand, as we still need to check Command::Worker.

                // On wasm, specifically, we should fail spectacularly if we get here.
                #[cfg(target_family = "wasm")]
                panic!("Worker process not supported on WASM: {worker:?}")
            }
            yarp_cli::Command::Completions { shell } => {
                return yarp_cli::completions::generate_to_stdout(*shell);
            }
            yarp_cli::Command::CommandLine(cmd) => {
                let (is_sandboxed, computer_use_override) = match cmd.as_ref() {
                    yarp_cli::CliCommand::Agent(yarp_cli::agent::AgentCommand::Run(run_args)) => (
                        run_args.sandboxed,
            ...
            yarp_cli::Command::DumpDebugInfo => {
                return debug_dump::run();
            }
            #[cfg(not(target_family = "wasm"))]
            yarp_cli::Command::PrintTelemetryEvents => {
...
fn run_internal(mut launch_mode: LaunchMode) -> Result<()> {
    ...
    cfg_if::cfg_if! {
        if #[cfg(enable_crash_recovery)] {
            if crash_recovery::is_crash_recovery_process(launch_mode.args().as_ref()) {
                yarp_logging::init_for_crash_recovery_process()?;
            } else {
                yarp_logging::init(yarp_logging::LogConfig { is_cli, log_destination })?;
            }
        } else {
            yarp_logging::init(yarp_logging::LogConfig { is_cli, log_destination })?;
        }
    ...
    let prefs_for_public_settings: &dyn yarpui_extras::user_preferences::UserPreferences =
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        use yarpui::platform::linux::{self, AppBuilderExt};
    ...
    {
        use yarpui::platform::windows::AppBuilderExt;
        ...
        if !is_parallels_vm {
            ...
            use yarpui::platform::windows::DXCPath;
...
fn initialize_app(
    ...
    startup_toml_parse_error: Option<yarpui_extras::user_preferences::Error>,
    ...
    cfg_if::cfg_if! {
        if #[cfg(feature = "integration_tests")] {
            yarpui_extras::secure_storage::register_noop(&data_domain, ctx);
        } else if #[cfg(target_os = "linux")] {
            yarpui_extras::secure_storage::register_with_fallback(&data_domain, yarp_core::paths::state_dir(), ctx)
        } else if #[cfg(target_os = "windows")] {
            yarpui_extras::secure_storage::register_with_dir(&data_domain, yarp_core::paths::state_dir(), ctx)
        } else {
    ...
    if user_is_logged_in {
        ...
        ctx.on_first_frame_drawn(move |ctx| {
            ...
            GPUState::handle(ctx).update(ctx, |gpu_state, ctx| {
                gpu_state
                    .set_has_lower_power_gpu(yarpui::rendering::is_low_power_gpu_available(), ctx);
            });
    ...
    #[cfg(feature = "local_fs")]
    {
        ...
        if FeatureFlag::SettingsImport.is_enabled()
            && ChannelState::channel() != yarp_core::channel::Channel::Integration
        {
    ...
    if yarp_core::features::FeatureFlag::OrchestrationV2.is_enabled() {
        ctx.add_singleton_model(
    ...
    ctx.add_singleton_model(|ctx| {
        yarp_core::sync_queue::SyncQueue::<SyncTask>::new_with_rate_limit(
            &ctx.background_executor(),
...
fn app_callbacks(is_integration_test: bool) -> yarpui::platform::AppCallbacks {
    yarpui::platform::AppCallbacks {
...
fn launch(ctx: &mut yarpui::AppContext, app_state: Option<AppState>, launch_mode: LaunchMode) {
...
fn init_logging_for_unit_tests_glue() {
    ...
    yarp_logging::init_logging_for_unit_tests();
}
...
pub fn enabled_features() -> HashSet<FeatureFlag> {
    ...
    flags.extend([
        ...
        #[cfg(feature = "yarpify_footer")]
        FeatureFlag::YarpifyFooter,
