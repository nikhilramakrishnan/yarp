...
use yarp_cli::RecoveryMechanism;
...
/// Keep in sync with [`yarp_cli::AppArgs`].
...
pub fn is_crash_recovery_process(args: &yarp_cli::AppArgs) -> bool {
...
impl CrashRecoveryProcess {
    ...
    fn kill(&mut self) {
        ...
        *IS_CRASH_RECOVERY_PROCESS_RUNNING.write() = false;
        self.is_alive = false;
        yarp_logging::on_crash_recovery_process_killed();
    }
...
impl CrashRecovery {
    ...
    #[cfg(test)]
    pub fn register_for_test(app: &mut yarpui::App) {
...
#[cfg(windows)]
fn wait_for_parent_crash(args: &yarp_cli::AppArgs) {
    use windows::Win32::{
...
fn handle_parent_crash(
    ...
    yarp_logging::on_parent_process_crash();
