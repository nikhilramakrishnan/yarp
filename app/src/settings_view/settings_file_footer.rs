...
/// Size of the Taskforce mark inside the "Fix with Taskforce" button. Matches the
/// Figma spec and the workspace banner's secondary-button icon sizing.
const ALERT_OZ_ICON_SIZE: f32 = 14.;
/// Horizontal padding inside the "Open file" / "Fix with Taskforce" action buttons.
...
/// across renders per `YARP.md`; the scroll state handle serves the same
...
pub fn render_settings_error_alert(
    ...
    // `ClippedScrollable` wants `yarpui::elements::Fill` (not the theme
    ...
    let scrollable_text = ConstrainedBox::new(
        ClippedScrollable::vertical(
            ...
            text_row,
            ScrollbarWidth::Auto,
            yarpui::elements::Fill::Solid(coloru_with_opacity(text_color, 30)),
            yarpui::elements::Fill::Solid(coloru_with_opacity(text_color, 60)),
