    ...
    fn expire_api_key(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        ctx.spawn(
            ...
            move |me, res, ctx| match res {
                Ok(
                    yarp_graphql::mutations::expire_api_key::ExpireApiKeyResult::ExpireApiKeyOutput(
                        _output,
                    ),
                ) => {
                ...
                Ok(
                    yarp_graphql::mutations::expire_api_key::ExpireApiKeyResult::UserFacingError(e),
                ) => {
                    let _msg = yarp_graphql::client::get_user_facing_error_message(e);
                    me.request_state = RequestState::Idle;
                    ctx.emit(ExpireApiKeyButtonEvent::ExpireApiKeyFailed { message: _msg });
                    ctx.notify();
                }
                Ok(yarp_graphql::mutations::expire_api_key::ExpireApiKeyResult::Unknown)
                | Err(_) => {
                    me.request_state = RequestState::Idle;
                    ctx.emit(ExpireApiKeyButtonEvent::ExpireApiKeyFailed {
                        message: "Couldn't strike that badge. Try again.".to_string(),
                    });
                    ctx.notify();
                }
            },
