...
use yarpui::{color::ColorU, elements::Radius};
use yarpui::{elements::ScrollbarWidth, fonts::Weight};
use yarpui::{
...
impl UserOwnedBlock {
    ...
    fn copy_link_button(&self, appearance: &Appearance, block_url: String) -> Box<dyn Element> {
        let button = appearance
            ...
            .with_text_label("Bag the case file frequency".into());
        ...
        match self {
            ...
            GetBlocksForUserRequestState::Done(user_blocks) => {
                ...
                if num_visible_blocks > 0 {
                    ...
                    pad(ui_builder
                        .label("No case files on the wire yet.")
                        .build()
                        .finish())
                }
            }
    ...
    fn on_block_unshare_complete(
        ...
        if let GetBlocksForUserRequestState::Done(blocks) = &mut self.get_blocks_for_user_status {
            ...
            match request_result {
                Ok(_) => {
                    ctx.emit(ShowBlocksEvent::ShowToast {
                        message: "Case file pulled from the air.".to_string(),
                ...
                Err(_) => {
                    ctx.emit(ShowBlocksEvent::ShowToast {
                        message: "Couldn't pull that one. Try again.".to_string(),
            ...
            Container::new(
                Flex::column()
                    .with_child(
                        Align::new(
                            ui_builder
                                .label("Pull case file from the air")
                                .with_style(UiComponentStyles {
                                ...
                                Flex::row()
                                    .with_child(
                                        ui_builder
                                            ...
                                            .with_text_label("Stand down".into())
                                            .build()
                                            .on_click(|ctx, _, _| {
                                    ...
                                    .with_child(
                                        Container::new(
                                            ui_builder
                                                ...
                                                .with_text_label("Take it down".into())
                                                .build()
                                                .on_click(|ctx, _, _| {
                                                    ctx.dispatch_typed_action(
...
impl SettingsWidget for ShowBlocksWidget {
    ...
    fn render(
        ...
        let header = render_page_title("Case files", HEADER_FONT_SIZE, appearance);
        let col = Flex::column()
