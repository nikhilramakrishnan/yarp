...
use crate::settings::{
    ...
    AwsBedrockCredentialsEnabled, CanUseYarpCreditsWithByok, CodeSettings, CodebaseContextEnabled,
...
impl AISettingsPageView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let mcp_allowlist_dropdown = ctx.add_typed_action_view(|ctx| {
            ...
            dropdown.set_top_bar_max_width(AI_SETTINGS_DROPDOWN_WIDTH);
            dropdown.set_menu_width(AI_SETTINGS_DROPDOWN_WIDTH, ctx);
            dropdown.set_menu_header_to_static("Pick MCP servers");
            dropdown
        });
        ...
        let add_profile_button = ctx.add_typed_action_view(|_| {
            ActionButton::new("New loadout", SecondaryTheme)
                .with_icon(Icon::Plus)
    ...
    fn build_page(subpage: Option<AISubpage>, ctx: &mut ViewContext<Self>) -> PageType<Self> {
        ...
        match subpage {
            ...
            Some(AISubpage::YarpAgent) => {
                // Fuzz page: global toggle + Active AI + Input + Other
                widgets.push(Box::new(GlobalAIWidget::default()));
                if ai_settings
                    .intelligent_autosuggestions_enabled_internal
                    .is_supported_on_current_platform()
            ...
            .map(|(pattern_clone, current_agent)| {
                ctx.add_typed_action_view(move |ctx| {
                    ...
                    dropdown.set_menu_header_text_override(|label| {
                        if label == "Other" {
                            "Pick a coding officer".to_string()
                        } else {
                            label.to_string()
                        }
                    });
...
pub enum AISettingsPageAction {
    ...
    ToggleVoiceInput,
    ToggleCanUseYarpCreditsWithByok,
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            AISettingsPageAction::ToggleAgentAttribution => {
                // The updated value syncs to yarp-server automatically via
...
impl UsageWidget {
    fn render_request_usage_count(
        ...
        let request_count_label = if workspace_is_delinquent_due_to_payment_issue {
            "Suspended — bill outstanding".to_string()
        } else if is_unlimited {
            "Unrationed".to_string()
        } else {
            format!("{used}/{limit}")
        };
        ...
        let usage_header = Container::new(
            Flex::row()
                ...
                .with_child(
                    appearance
                        .ui_builder()
                        .paragraph(format!("Rations restock {formatted_next_refresh_time}"))
                        .with_style(UiComponentStyles {
        ...
        Flex::column()
            .with_child(
                render_ai_setting_toggle::<SharedBlockTitleGenerationEnabled>(
                    "Shared casebook title generation",
                    AISettingsPageAction::ToggleSharedTitleGeneration,
    ...
    fn render_git_operations_autogen_section(
        ...
        Flex::column()
            .with_child(render_ai_setting_toggle::<GitOperationsAutogenEnabled>(
                "Logbook & pull request generation",
                AISettingsPageAction::ToggleGitOperationsAutogen,
            ...
            Container::new(
                Flex::row()
                    ...
                    .with_children([
                        ...
                        appearance
                            .ui_builder()
                            .span("Show model picker on the radio mic".to_string())
                            .with_style(UiComponentStyles {
                                font_color: Some(
    ...
    fn render_voice_section(
        ...
        if ai_settings.is_voice_input_enabled(app) {
            column.add_child(render_dropdown_item(
                appearance,
                "Push-to-talk key",
                Some("Press and hold to key the mic."),
                None,
                LocalOnlyIconState::for_setting(
...
impl SettingsWidget for OtherAIWidget {
    ...
    fn render(
        ...
        column.add_child(render_ai_setting_toggle::<ShowConversationHistory>(
            "Show case file history in the tools panel",
            AISettingsPageAction::ToggleShowConversationHistory,
        ...
        if is_footer_enabled {
            ...
            if FeatureFlag::CLIAgentRichInput.is_enabled() {
                ...
                let auto_show_toggle_label = render_body_item_label::<AISettingsPageAction>(
                    "Auto show/hide rich radio mic based on PC status".into(),
                    Some(styles::header_font_color(true, app)),
                    Some(AdditionalInfo {
            ...
            let command_list = {
                ...
                list_column.add_child(
                    appearance
                        .ui_builder()
                        .span("Commands that put the duty belt on".to_string())
                        .with_style(UiComponentStyles {
                            font_size: Some(CONTENT_FONT_SIZE),
                            ..Default::default()
                        })
...
impl SettingsWidget for AgentAttributionWidget {
    ...
    fn render(
        ...
        let toggle_row = build_toggle_element(
            render_body_item_label::<AISettingsPageAction>(
                "Sign on for officer attribution".to_string(),
...
impl AwsBedrockWidget {
    fn new(ctx: &mut ViewContext<<Self as SettingsWidget>::View>) -> Self {
        ...
        let refresh_credentials_button = ctx.add_typed_action_view(|_| {
            ActionButton::new("Re-sweep", SecondaryTheme)
    ...
    fn render_aws_bedrock_section(
        ...
        let toggle_description = if is_admin_enforced {
            "Yarp loads and sends local AWS CLI credentials for Bedrock-supported models. This setting is managed by your organization.".to_string()
        } else {
            "Yarp loads and sends local AWS CLI credentials for Bedrock-supported models."
                .to_string()
        };
...
mod styles {
    use yarp_core::ui::{appearance::Appearance, theme::Fill};
