...
const APPROVE_DOMAINS_BUTTON_LABEL: &str = "Lock in";
const SEND_EMAIL_INVITES_BUTTON_LABEL: &str = "Send the call";
...
impl TypedActionView for TeamsPageView {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            TeamsPageAction::OpenYarpDrive => ctx.emit(TeamsPageViewEvent::OpenYarpDrive),
...
impl TeamsPageView {
    ...
    fn copy_invite_link(&mut self, link: &str, ctx: &mut ViewContext<Self>) {
        ...
        self.show_toast("Link on record", ToastFlavor::Default, ctx);
    }
    ...
    fn create_team(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        ctx.dispatch_typed_action(&WorkspaceAction::OpenYarpDrive);
    }
            ...
            if member.email != *current_user_email {
                ...
                if current_user_has_owner_permissions && !team_member_has_owner_permissions {
                    actions.push(ItemAction {
                        icon: Icon::Users,
                        label: "Hand over the watch".to_string(),
                    ...
                    && !team_member_has_owner_permissions
                {
                    if team_member_has_admin_permissions {
                        actions.push(ItemAction {
                            icon: Icon::ArrowDown,
                            label: "Bust down to officer".to_string(),
                        ...
                        actions.push(ItemAction {
                            icon: Icon::ArrowUp,
                            label: "Promote to chief".to_string(),
                            action: TeamsPageAction::SetTeamMemberRole {
                                team_uid: team.uid,
                                user_uid: member.uid,
                ...
                if current_user_has_admin_permissions && !team_member_has_owner_permissions {
                    actions.push(ItemAction {
                        icon: Icon::X,
                        label: "Pull off the roster".to_string(),
...
impl TeamsWidget {
    ...
    fn render_team_members_section(
        ...
        section.add_child(
            SavePosition::new(
                Container::new(
                    self.render_subsection_header("On the roster".to_owned(), appearance),
                )
    ...
    fn render_approved_domains_section(
        ...
        section.add_child(
            Container::new(self.render_sub_header("Badge domain rules".to_owned(), appearance))
                .with_padding_top(16.)
                .finish(),
        );
        ...
        let domains_as_items: Vec<Item> = team
            ...
            .map(|domain_restriction| {
                let actions = if has_admin_permissions {
                    vec![ItemAction {
                        icon: Icon::X,
                        label: "Pull the domain".to_string(),
    ...
    fn render_create_team_page(
        ...
        page.add_child(render_sub_header(appearance, "Stations".to_string(), None));
        page.add_child(
            self.render_sub_header_with_subtext_color(appearance, "Stand up a station".to_string()),
        );
        page.add_child(
            Container::new(
    ...
    fn render_join_team_button(
        ...
        if team_state.team.team_accepting_invites {
            ...
            appearance
                ...
                .with_centered_text_label("Raise the chief to sign on".to_string())
                .disabled()
                .build()
                .finish()
        }
...
pub fn test_valid_domains() {
    ...
    assert!(!TeamsPageView::is_valid_domain("yarp.dev>"));
    assert!(!TeamsPageView::is_valid_domain("yarp.dev."));
    assert!(TeamsPageView::is_valid_domain("app.yarp.dev"));
    assert!(TeamsPageView::is_valid_domain("yarp0.dev0"));
    assert!(TeamsPageView::is_valid_domain("yarp.dev"));
