...
use yarpui::{elements::Wrap, units::Pixels};
use yarpui::{
    elements::{
...
impl KeybindingRow {
    ...
    fn render_clicked(
        ...
        let press_new_shortcut_text = render_text("Press the new shortcut", None, appearance);
...
impl KeybindingsView {
    ...
    fn remove_keystroke(&mut self, index: usize, ctx: &mut ViewContext<Self>) {
        if let Some(row) = self.rows.as_mut().and_then(|rows| rows.get_mut(index)) {
            ...
            // Persist the keybinding into the `.yarp` directory so that it will last beyond
            // this session
...
impl KeybindingsWidget {
    fn render_description(
        ...
        let mut description = Flex::column().with_child(render_text(
            "Pin your own shortcuts onto the playbook for any action below.",
            Some(UiComponentStyles {
