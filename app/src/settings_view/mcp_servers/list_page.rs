...
use yarp_core::ui::{appearance::AppearanceEvent, theme::color::internal_colors, Icon};
use yarpui::{
    elements::{
...
impl MCPServersListPageView {
        ...
        match update {
            MCPServerUpdate::CloudTemplate { .. } => {
                ...
                ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    let toast = DismissibleToast::success(String::from("MCP server re-issued."));
    ...
    fn file_based_root_chip_text(root_path: &PathBuf) -> Option<String> {
        ...
        // If the path is the Yarp data directory (e.g. ~/.yarp or ~/.yarp_dev), set the text to
        ...
        if root_path == &crate::yarp_managed_paths_watcher::yarp_data_dir() {
            return Some("global".to_string());
        }
