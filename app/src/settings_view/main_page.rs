...
use yarp_core::{channel::ChannelState, context_flag::ContextFlag};
use yarpui::{
...
use yarpui::{fonts::Weight, keymap::ContextPredicate};
use yarpui::{
    ...
    fn render(
        ...
        let account_info = if view.auth_state.is_anonymous_or_logged_out() {
            ...
            let profile_image_source = view.auth_state.user_photo_url().map(|url| {
                asset_cache::url_source_with_persistence(url, &yarp_core::paths::cache_dir())
            });
    ...
    fn render(
        ...
        let label_info = AdditionalInfo {
            ...
            on_click_action: Some(MainPageAction::OpenUrl(
                "https://github.com/hotfuzz/yarp/terminal/more-features/settings-sync".into(),
            )),
            secondary_text: None,
