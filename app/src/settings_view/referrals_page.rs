...
const LINK_BUTTON_TEXT: &str = "Bag the frequency";
...
impl EmailValidationError {
    ...
    fn ui_message(&self) -> String {
        match self {
            EmailValidationError::Empty => "Need an email on the form.".to_owned(),
            EmailValidationError::Invalid(invalid_email) => {
                format!("This email won't pass the desk: {invalid_email}")
            }
        }
    }
}
