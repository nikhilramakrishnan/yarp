...
use super::{
    ...
    index::{yarp_drive_section_header_position_id, DriveIndexAction, DriveIndexSection},
...
pub trait YarpDriveItem {
    ...
    fn icon(&self, appearance: &Appearance, color: Option<Fill>) -> Option<Box<dyn Element>> {
        ...
        let icon_fill = color.unwrap_or(yarp_drive_icon_color(appearance, object_type).into());
        Some(Icon::from(object_type).to_yarpui_icon(icon_fill).finish())
    }
...
impl YarpDriveItemId {
    pub fn drive_row_position_id(&self) -> String {
        match self {
            ...
            Self::Space(space) => {
                yarp_drive_section_header_position_id(&DriveIndexSection::Space(*space))
            }
            Self::Trash => "Trash".to_string(),
        }
    }
}
/// This uniquely identifies an item in Yarp Drive index
/// Includes spaces (which CloudObjectTypeAndId does not entail)
#[derive(Debug, Clone, PartialEq, Eq, Copy)]
pub enum YarpDriveItemId {
    AIFactCollection,
    MCPServerCollection,
...
impl Clone for Box<dyn YarpDriveItem> {
    fn clone(&self) -> Self {
        self.clone_box()
    }
}
