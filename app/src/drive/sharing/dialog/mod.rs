...
use yarpui::ui_components::button::{ButtonVariant, TextAndIcon, TextAndIconAlignment};
...
impl SharingDialog {
    ...
    /// The Yarp Drive server ID for the target object. `None` if the target is not a Yarp Drive
    /// object or AI conversation.
    fn target_cloud_object_id(&self, app: &AppContext) -> Option<ServerId> {
        match self.target.as_ref() {
            Some(ShareableObject::YarpDriveObject(id)) => Some(*id),
    ...
    /// The targeted Yarp Drive object, or `None` if the target is not a known Yarp Drive object.
    ...
    fn targeted_object_name(&self, app: &AppContext) -> String {
        self.target
            ...
            .and_then(|target| match target {
                ShareableObject::YarpDriveObject(server_id) => CloudModel::as_ref(app)
    ...
    pub fn editability(&self, app: &AppContext) -> ContentEditability {
        match self.target.as_ref() {
            ...
            Some(ShareableObject::YarpDriveObject(id)) => {
    ...
    /// panes and the Yarp Drive index).
    ...
    fn owner(&self, app: &AppContext) -> Option<Subject> {
        match self.target.as_ref()? {
            ShareableObject::YarpDriveObject(id) => {
    ...
    pub fn copy_link(&self, ctx: &mut ViewContext<Self>) {
        if let Some(url) = self.target.as_ref().and_then(|target| target.link(ctx)) {
            let event = match self.target {
                ...
                Some(ShareableObject::YarpDriveObject(_))
            ...
            ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                let toast =
                    DismissibleToast::default(format!("Frequency to {object_name} bagged."));
    ...
    fn reset_guest_menu(&mut self, guest_index: usize, ctx: &mut ViewContext<Self>) {
        if let Some(guest) = self.guest_states.get(guest_index) {
            ...
            self.guest_menu.update(ctx, |menu, ctx| {
                ...
                // (team removal is supported for YarpDrive objects and AI conversations, but not sessions)
    ...
    fn remove_targeted_guest(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        match &self.target {
            Some(ShareableObject::YarpDriveObject(object_id)) => {
    ...
    fn render_invite_form(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        ...
        let mut invite_button = appearance
            ...
            .with_centered_text_label("Enlist them".into())
        ...
        // For Yarp Drive targets, we can't update permissions while there's a pending change.
        if self
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            SharingDialogAction::SetLinkPermissions(access_level) => {
                self.set_open_menu(OpenMenuState::None, ctx);
                if let Some(ShareableObject::YarpDriveObject(id)) = self.target.as_ref() {
