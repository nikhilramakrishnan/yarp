...
## Problem
When users compose prompts for CLI agents through Yarp's rich input (Ctrl-G or the Compose button), they cannot browse or insert saved prompts. The normal Yarp agent input supports prompts, but the CLI agent input does not. Users must manually type or paste prompt content.
...
## Non-goals
- Changing how prompts work in the normal Yarp agent input.
...
### Trigger
User selects a prompt from the prompts menu, same as in normal Yarp input.
