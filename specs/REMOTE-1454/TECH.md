...
- `yarp_cli::agent::Harness` (`crates/yarp_cli/src/agent.rs:120-131`) — `Oz`, `Claude`, `Gemini`.
## Current state
...
`is_cloud_agent_pre_first_exchange` is a view-level helper that returns `true` while the active cloud-agent conversation has `exchange_count() == 0`. It gates: the "Setting up environment" yarping indicator, the pre-harness input-box hiding, remote-input suppression in `shared_session/viewer/terminal_manager.rs`, and `maybe_insert_setup_command_blocks`. For non-oz there are no exchanges, so this stays `true` forever.
