...
- `crates/editor/src/render/element/image.rs:13-49` — markdown render block that constructs `yarpui::elements::Image` from the resolved asset source.
...
- The markdown render element constructs a generic `yarpui::elements::Image` with `.contain()` and no animation start time (`crates/editor/src/render/element/image.rs:42-49`).
...
## Proposed changes
- Add an explicit animated-image behavior to `yarpui::elements::Image`, with two modes:
...
- Leave explicit animated callers unchanged. `app/src/resource_center/section_views/changelog_section.rs:152` should continue to opt into animation, and the `yarpui` animated-image example should still exercise the full-animation path.
...
graph TD
  ...
  C --> D[BlockItem::Image]
  D --> E[RenderableImage]
  E --> F[yarpui::elements::Image with FirstFramePreview]
...
- Add `yarpui_core` unit tests with small animated GIF and animated WebP fixtures:
...
- Run the usual repo validation before implementation is considered done:
  - `cargo fmt`
  - targeted Rust tests for the touched editor / yarpui_core modules
