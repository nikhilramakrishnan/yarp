...
2. Yarp writes the default template to `~/.yarp/tab_configs/my_tab_config.toml` and opens it in the user's configured editor.
...
- Oz generates a valid `.toml` file and writes it to `~/.yarp/tab_configs/`.
...
- Yarp opens a new tab with the specified pane layout, running any configured commands.
...
- `worktree_name_autogenerated` (optional, bool, default false): when `true`, Yarp auto-generates the worktree branch name instead of prompting the user. The app detects worktree configs by scanning `commands` for `git worktree`.
...
8. The `tab-configs` bundled skill appears in `/skills` in Yarp.
