...
Linear: [APP-4067](https://linear.app/yarpdotdev/issue/APP-4067/enable-gemini-notifications)
Figma: none provided

## Summary

Add install and update chip support for the Gemini CLI Yarp plugin (`gemini-yarp`), matching the existing Claude Code chip UX. Enable the listener so Yarp processes structured notifications from Gemini sessions (session status, permission requests, task completion).

## Problem

The `gemini-yarp` extension already exists and uses the same structured OSC 777 notification protocol as Claude Code (`yarp://cli-agent` title, JSON body with `agent: "gemini"`). However, Yarp currently ignores these notifications because:
...
- Changing the gemini-yarp plugin itself (it's maintained separately in `yarpdotdev/gemini-cli-yarp`).
...
- **Hooks**: Defined in `hooks/hooks.json` within the extension directory (not in the manifest). The gemini-yarp plugin uses `SessionStart`, `AfterAgent`, `Notification`, `BeforeAgent`, and `AfterTool` hooks.
...
- Clicking triggers auto-install via `gemini extensions install https://github.com/yarpdotdev/gemini-cli-yarp --consent`.
...
- Title: "Install Yarp Plugin for Gemini CLI"
- Subtitle: "Run the following command, then restart Gemini CLI."
- Steps:
  1. "Install the Yarp extension" — command: `gemini extensions install https://github.com/yarpdotdev/gemini-cli-yarp --consent`
...
- Clicking triggers auto-update via `gemini extensions update gemini-yarp`.
  - The extension name is `gemini-yarp` (the installed directory name under `~/.gemini/extensions/`).
- On success, a toast confirms the update and tells the user to restart Gemini.
- On failure, a toast appears with an error, and the user can click the info (ⓘ) button to open manual instructions in a split pane.

**Manual update instructions (split pane):**
- Title: "Update Yarp Plugin for Gemini CLI"
- Subtitle: "Run the following command, then restart Gemini CLI."
- Steps:
  1. "Update the Yarp extension" — command: `gemini extensions update gemini-yarp`
...
1. **Filesystem check**: `~/.gemini/extensions/gemini-yarp/gemini-extension.json` — read the `version` field to determine if the plugin is installed and whether it's outdated. Symlinks are followed (supports `gemini extensions link`).
...
1. Starting a Gemini CLI session in Yarp when the plugin is not installed shows a green install chip.
...
5. Clicking the update chip runs `gemini extensions update gemini-yarp` and shows a success/failure toast.
