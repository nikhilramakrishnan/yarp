...
Linear: [APP-3704](https://linear.app/yarpdotdev/issue/APP-3704/save-current-tab-as-a-new-config)
...
Add a "Save as new config" item to the tab right-click context menu that snapshots the current tab's pane layout (splits, working directories, focus, and color) and writes it as a new tab config TOML file in `~/.yarp/tab_configs/`.
...
1. Yarp snapshots the tab's pane tree: split directions, each pane's current working directory, focus state, and tab color.
2. Yarp writes a new `.toml` file to `~/.yarp/tab_configs/` with an auto-generated filename (`my_tab_config.toml`, or `my_tab_config_1.toml` if the name is taken).
3. Yarp opens the file in the user's configured editor (same behavior as "Create new tab config...").
