...
## Summary
The code-review panel's git operations buttons (Commit, Push, Publish, Create PR, Commit and push, Commit and create PR) must succeed regardless of how Yarp was launched. When opened from Finder/Dock, Yarp inherits launchd's minimal `PATH`, which excludes Homebrew / MacPorts / Nix / asdf / etc. Pushes to LFS-enabled repos fail (LFS `pre-push` hook can't find `git-lfs`), and PR creation fails when `gh` isn't on `/usr/bin`.
## Behavior
...
5. First git op per Yarp session may pay a one-time shell-capture cost (<1s typically; bounded by `.zshrc` load). Absorbed by the existing loading state; no new UI. Subsequent ops reuse the cache.
...
## Validation
- Launch Yarp from Finder with Homebrew-only `git-lfs`. On a LFS repo, Commit and push → succeeds.
...
- Launch Yarp from a terminal → all git ops behave as today.
