...
"User" is not limited to the end user of the Yarp app. It means whoever consumes the surface being designed:

- For UI / UX features: the human using Yarp.
...
14. Sharing or exporting a block (Yarp Drive, share link, save as file) preserves the original markdown source, not the rendered form.

15. Theming: table borders, header backgrounds, alternating row shading (if any), and link/code styles all come from the active Yarp theme. No hard-coded colors.
