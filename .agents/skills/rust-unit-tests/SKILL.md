...
description: Write, improve, and run Rust unit tests in the yarp Rust codebase.
---

# Rust Unit Tests in yarp
...
- Prefer `yarpui::App::test` for deterministic unit tests around views/models.
- Initialize app models once, then mutate via `update` and assert via `read`.

```rust
use yarpui::App;
// In app crate tests prefer `crate::test_util::...`; from other crates use `yarp::test_util::...`.
...
```rust
use yarp::features::FeatureFlag; // or `use crate::features::FeatureFlag;` inside the app crate
