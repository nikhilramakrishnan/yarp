...
/// Identity Federation to authenticate to GCP as a particular Fuzz agent run.
///
...
pub struct GcpCredentials {
    ...
    /// Temporary file where Yarp OIDC tokens are cached.
...
impl GcpCredentials {
    ...
    pub fn federated(
        ...
        let cred_config_json =
            generate_gcp_credential_config(task_id, config, &fuzz_binary_path, output_file.path())?;
        safe_debug!(
...
fn generate_gcp_credential_config(
    ...
    let fuzz_binary_display = fuzz_binary_path.display().to_string();
    ...
    if fuzz_binary_display.contains(' ') {
        return Err(PrepareGcpCredentialsError::InvalidBinaryPath {
            path: fuzz_binary_path.to_path_buf(),
        });
    }
    if task_id.contains(' ') {
    ...
    let mut command = format!("{fuzz_binary_display} federate issue-gcp-token --run-id {task_id}");
