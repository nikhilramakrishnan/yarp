...
fn wss_becomes_https_and_strips_path() {
    let got = derive_http_origin_from_ws_url("wss://rtc.app.yarp.dev/graphql/v2");
    assert_eq!(got.as_deref(), Some("https://rtc.app.yarp.dev"));
}
...
fn unparseable_input_returns_none() {
    ...
    assert!(derive_http_origin_from_ws_url("https://app.yarp.dev").is_none());
}
