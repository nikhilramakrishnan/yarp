...
pub fn init_and_register_preferences(ctx: &mut AppContext) {
    ctx.add_singleton_model(move |_| {
        crate::PublicPreferences::new(Box::<
            yarpui_extras::user_preferences::in_memory::InMemoryPreferences,
        >::default())
    });
    ctx.add_singleton_model(move |_| {
...
impl EventListener {
    fn new(ctx: &mut yarpui::ModelContext<Self>) -> Self {
...
impl yarpui::Entity for EventListener {
    type Event = ();
}
...
fn test_set_value_raises_changed_event_no_save() {
    yarpui::App::test((), |mut app| async move {
