...
fn test_parse_link_in_tag() {
    ...
    let source = "a [https://google.com](https://yarp.dev) link";
    assert_eq!(
        test_parse_markdown(source),
        vec![FormattedTextLine::Line(vec![
            FormattedTextFragment::plain_text("a "),
            FormattedTextFragment::hyperlink("https://google.com", "https://yarp.dev"),
            FormattedTextFragment::plain_text(" link")
        ])]
    );
}
...
fn test_parse_escape_in_style() {
    let source = "Some *styled \\* escaped* [te\\]xt\\^](https://yarp.dev)";
    assert_eq!(
        test_parse_markdown(source),
        vec![FormattedTextLine::Line(vec![
            ...
            FormattedTextFragment::plain_text(" "),
            FormattedTextFragment::hyperlink("te]xt^", "https://yarp.dev")
        ])]
    );
}
...
fn test_parse_inline_link() {
    assert_eq!(
        parse_all("[basic](https://yarp.dev)", parse_inline),
...
fn test_parse_table_with_links() {
    ...
    if let FormattedTextLine::Table(table) = &result[0] {
        ...
        assert!(matches!(
            ...
            Some(Hyperlink::Url(url)) if url == "https://yarp.dev"
        ));
    } else {
        panic!("Expected table");
    }
