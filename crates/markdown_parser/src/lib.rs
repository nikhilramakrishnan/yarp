...
/// This purposefully shadows the `Action` trait from `yarpui`.
///
/// Since `yarpui` depends on this crate, we can't depend on the `yarpui::Action` trait directly.
/// Instead, we create a new trait with a blanket implementation that implicitly results
/// in any `yarpui::Action` implementing this `Action`.
