...
const CLI_LOG_SUBDIRECTORY: &str = "fuzz";
...
/// environment. If `config.is_cli` is true, logs are written to a separate "fuzz" subdirectory with
...
fn sentry_log_filter(md: &log::Metadata) -> sentry_log::LogFilter {
    if yarp_core::errors::should_ignore_log_for_sentry(md) {
    ...
    match md.target() {
        ...
        "yarpui::core::redraw_frame" => sentry_log::LogFilter::Ignore,
