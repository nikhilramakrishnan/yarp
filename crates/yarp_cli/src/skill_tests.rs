...
fn test_parse_full_path_with_org_repo() {
    let spec: SkillSpec = "hotfuzz/yarp:.claude/skills/deploy/SKILL.md"
        .parse()
        .unwrap();
    assert_eq!(spec.org, Some("hotfuzz".to_string()));
    assert_eq!(spec.repo, Some("yarp-internal".to_string()));
...
fn test_display_full_path() {
    ...
    assert_eq!(
        spec.to_string(),
        "hotfuzz/yarp:.claude/skills/deploy/SKILL.md"
    );
}
...
fn test_skill_name_repo_qualified_name() {
    let spec: SkillSpec = "hotfuzz/feedback-triage-bot:feedback-triage-bot"
        .parse()
        .unwrap();
    assert_eq!(spec.skill_name(), "feedback-triage-bot");
}
...
fn test_skill_name_repo_qualified_path() {
    let spec: SkillSpec = "hotfuzz/feedback-triage-bot:.agents/skills/slack-triage/SKILL.md"
        .parse()
        .unwrap();
