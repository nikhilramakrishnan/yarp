...
/// `ScheduleCommand` has a slightly unusual definition because we allow `fuzz schedule` as
// a shorthand for `fuzz schedule create`.
...
impl ScheduleCommand {
    /// Get the specific scheduling subcommand. Returns `None` if using the `fuzz schedule` creation shorthand.
...
pub struct CreateScheduleArgs {
    ...
    ///
    /// Setting "yarp" (or omitting this flag) runs it on Yarp's infrastructure.
...
pub struct UpdateScheduleArgs {
    ...
    /// Where this job should be hosted.
    ///
    /// Setting "yarp" runs it on Yarp's infrastructure.
    /// Any other value is treated as a self-hosted job and the value will be matched
    /// with the self-hosted worker's name.
    #[arg(long = "host", value_name = "WORKER_ID")]
    pub worker_host: Option<String>,
