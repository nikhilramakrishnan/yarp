...
use yarpui::integration::{self, PersistedDataMap, TestStep};
use yarpui::integration::{TestDriver, TestSetupUtils};
use yarpui::{App, WindowId};
use yarpui_extras::user_preferences::file_backed::FileBackedUserPreferences;
...
impl Builder {
    ...
    pub fn build(self, test_name: &str, create_temp_dir_for_test: bool) -> TestDriver {
        ...
        let inner = inner.with_setup(move |utils| {
            ...
            // filesystem
            utils.set_env(yarp::keyboard::DISABLE_SAVE_ENV_VAR, Some("true"));
        ...
        let file_path = yarp::settings::user_preferences_file_path();
