...
use yarp::{terminal::find::TerminalFindModel, util::bindings::CustomAction, AgentModeEntrypoint};
...
use yarp::integration_testing::terminal::assert_focused_editor_in_tab;
use yarp::integration_testing::{
    settings::assert_theme_chooser_contains,
    tab::{assert_pane_title, assert_tab_title},
};
use yarp::settings::CtrlTabBehavior;
use yarp::terminal::keys_settings::KeysSettings;
use yarp::terminal::{
    model::{blocks::BlockHeightSummary, terminal_model::BlockIndex},
    view::TerminalViewState,
};
use yarp::workflows::categories::CategoriesView;
use yarp::{
...
use yarp::terminal::view::ALIAS_EXPANSION_BANNER_SEEN_KEY;
use yarp::{
...
use yarp::{settings_view::SettingsAction, terminal::block_list_viewport::ScrollLines};
use yarp::{
...
pub fn test_suggestions_menu_positioning() -> Builder {
    new_builder()
        ...
        .with_step(
            new_step_with_default_assertions("Open Yarp Drive")
                        ...
                        workspace
                            ...
                            .read(app, |pane_group, ctx| {
                                pane_group
                                    ...
                                    .as_ref(ctx)
                                    .as_any()
                                    .downcast_ref::<yarp::terminal::local_tty::TerminalManager>()
...
precmd_functions+=(_p9k_precmd)
        ...
        // If the user then switches back to the Yarp prompt, we should close the banner.
        .with_step(
...
/// Default auto title. We test that Yarp's auto title is used and verify that that
...
"#,
            ...
            write_rc_files_for_test(
                &dir,
                r#"
YARP_DISABLE_AUTO_TITLE="true"
"#,
                [ShellRcType::Zsh],
            );
        })
        .with_step(wait_until_bootstrapped_single_pane_for_tab(0))
        // If Yarp title is disabled, we don't set the DISABLE_AUTO_TITLE env variable
        .with_step(execute_command_for_single_terminal_in_tab(
            0,
            "echo $DISABLE_AUTO_TITLE".to_string(),
            ExpectedExitStatus::Success,
...
// the workspace view, but we DO force yarp drive open to show the dialog, so we can look for that
...
pub fn test_context_chips_prompt_at_bootstrap() -> Builder {
    new_builder()
        ...
        .with_step(
            new_step_with_default_assertions("Check Yarp prompt")
