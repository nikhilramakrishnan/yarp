# Integration tests in Yarp
This is a short guide into writing integration tests in Yarp.
...
```sh
for i in {0..100}; do
    YARPUI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS=1 RUST_BACKTRACE=full YARP_SHELL_PATH=/bin/bash cargo run -p integration -- test_simple_example
...
* You can copy over a yarp.sqlite file from ~/Library/Application\ Support/{yarp, dev.yarp.Yarp-(Dev|Preview|Stable)} directly
* You may want to sanitize some info that is specific to you (i.e. cwd https://staging.yarp.dev/block/FNBafyVtxvjmdNIx6HxUM5)


### How to run integration tests?
To run a specific integration test you can use:
```
  YARPUI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS="1" cargo run --bin integration -- test_simple_example
```

The `YARPUI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS="1"` will force the new terminal window to open, which helps a lot when iterating on your integration test implementation!

### Known issues / limitations
* To determine (from the `TestStep`) which shell is used for the test, you can try checking `YARP_SHELL_PATH` environment variable (that works within the CI on github) or check the passwd for the user (for local runs).
* Similarly you can run the test with a specific shell by setting the `YARP_SHELL_PATH` and then running the test. Note that if you're running with fish, you also need to pass in `--features fish_shell` until that feature flag is removed. For example: `YARP_SHELL_PATH=/usr/local/bin/fish`, then `cargo run --bin integration --features fish_shell -- test_simple_example`
