...
impl SelectedSettings {
    ...
    pub fn is_yarp_drive_enabled(&self) -> bool {
        match self {
            SelectedSettings::AgentDrivenDevelopment {
                ui_customization, ..
            } => ui_customization
                .as_ref()
                .map(|ui| ui.show_yarp_drive)
                .unwrap_or(true),
            SelectedSettings::Terminal {
                ui_customization, ..
            } => ui_customization
...
impl OnboardingStateModel {
    ...
    pub(crate) fn set_tools_panel_enabled(&mut self, enabled: bool, ctx: &mut ModelContext<Self>) {
        ...
        self.ui_customization.show_yarp_drive = enabled;
        ctx.notify();
    }
    ...
    pub(crate) fn set_show_yarp_drive(&mut self, value: bool, ctx: &mut ModelContext<Self>) {
        if self.ui_customization.show_yarp_drive == value {
            return;
        }
        send_telemetry_from_ctx!(
            OnboardingEvent::SettingChanged {
                setting: "yarp_drive".to_string(),
                value: value.to_string(),
            },
            ctx
        );
        self.ui_customization.show_yarp_drive = value;
        ctx.notify();
    }
