...
impl IntentionSlide {
    ...
    fn render_header(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let logo = ConstrainedBox::new(Icon::YarpLogoLight.to_yarpui_icon(logo_fill).finish())
            .with_width(64.)
            .with_height(64.)
            .finish();
    ...
    fn render_agent_card(
        ...
        let header_row = {
            let label = appearance
                .ui_builder()
                .paragraph("Work cases with the Taskforce")
                .with_style(UiComponentStyles {
                    font_size: Some(16.),
            ...
            for (i, icon) in [Icon::Fuzz, Icon::ClaudeLogo, Icon::OpenAILogo]
                .iter()
                .enumerate()
            {
                let el = ConstrainedBox::new(icon.to_yarpui_icon(icon_fill).finish())
                    .with_width(16.)
                    .with_height(16.)
                    .finish();
                icon_row = if i == 0 {
        ...
        let description = FormattedTextElement::from_str(
            "Detective-grade officer assistance for fast investigation. Includes:",
            appearance.ui_font_family(),
            14.,
        )
        ...
        let checklist = {
            ...
            let mut col = Flex::column()
                ...
                .with_cross_axis_alignment(CrossAxisAlignment::Start);
            for &item in items {
                let icon_el = ConstrainedBox::new(Icon::Check.to_yarpui_icon(check_fill).finish())
                    .with_width(16.)
                    .with_height(16.)
                    .finish();
    ...
    fn render_terminal_card(
        ...
        let badge = {
            let badge_text = appearance
                .ui_builder()
                .paragraph("No Taskforce features")
                .with_style(UiComponentStyles {
                    font_size: Some(12.),
                    font_weight: Some(Weight::Semibold),
                    font_color: Some(text_color),
