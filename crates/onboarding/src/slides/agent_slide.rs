...
impl button::Theme for UpgradeButtonTheme {
    ...
    fn text_color(
        &self,
        background: Option<yarp_core::ui::theme::Fill>,
        appearance: &Appearance,
    ) -> ColorU {
        let bg = background
...
pub struct AgentSlide {
    onboarding_state: yarpui::ModelHandle<OnboardingStateModel>,
...
impl AgentSlide {
            ...
            match event {
                ...
                OnboardingStateEvent::AuthStateChanged => {
                        ...
                        && me.last_auth_state != OnboardingAuthState::PayingUser
                    {
                        ...
                        let _ = ctx.spawn(
                            yarpui::r#async::Timer::after(PLAN_ACTIVATED_TOAST_DURATION),
                            |me: &mut Self, _, ctx| {
    ...
    fn render_bottom_nav(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let step_index = 2;
        let step_count = if yarp_core::features::FeatureFlag::OpenYarpNewSettingsModes.is_enabled()
        {
            5
        } else {
            4
...
impl View for AgentSlide {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        stack.add_child(
            yarpui::elements::Align::new(bottom_overlay)
                .bottom_center()
                .finish(),
        );
        stack.finish()
