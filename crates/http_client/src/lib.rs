...
impl Client {
    ...
    /// For example, app.yarp.dev --> app.yarp.dev.
    ...
    fn include_yarp_http_headers<U: IntoUrl + Clone>(url: U) -> bool {
        url.into_url().is_ok_and(|url| {
            url.host_str().is_some_and(|dest_host| {
                ...
                // If the request is going to our server, the destination host should be "app.yarp.dev" or
                // "staging.yarp.dev". The window hostname should also return the same.
