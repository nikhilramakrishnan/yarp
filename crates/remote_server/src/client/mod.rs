...
use yarpui::r#async::{executor, FutureExt as _};
...
use yarpui::r#async::TransportStream;
...
/// so that `kill_on_drop` fires when teardown occurs. In Yarp this is
...
impl RemoteServerClient {
    ...
    /// session's (in Yarp, on the `RemoteServerManager`'s
