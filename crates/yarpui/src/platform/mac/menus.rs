...
extern "C" {
    ...
    fn make_yarp_custom_menu_item(ctx: *mut c_void) -> id;
...
fn resolve_standard_action(action: StandardAction) -> StandardMenuItemProperties {
    ...
    match action {
        ...
        StandardAction::Quit => make("Quit Yarp", "terminate:", cmd, "q"),
        StandardAction::Hide => make("Hide Yarp", "hide:", cmd, "h"),
...
unsafe fn make_menu_item(menu_item: MenuItem) -> id {
    match menu_item {
        MenuItem::Custom(custom_menu_item) => {
            ...
            let nsmenu_item = make_yarp_custom_menu_item(MenuItemData::into_context(data));
