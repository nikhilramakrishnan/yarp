...
description: Create new Yarp tab config TOML files from natural-language requests. Use when the user wants a new tab config, a new tab layout, or asks for a slash command to generate a tab config.
---

# create-tab-config

Create a new Yarp tab config based on what the user wants.
...
4. Determine the correct tab config directory for the user's Yarp build.
   Tab configs live under `~/.yarp/tab_configs/` for standard builds. Non-stable builds use a channel-specific variant (e.g. `~/.yarp-<channel>/tab_configs/`).
   To find the correct directory, run `ls -d ~/.yarp*/` to list the available Yarp data directories and pick the one that corresponds to the running build. When in doubt, ask the user which build they are using.
