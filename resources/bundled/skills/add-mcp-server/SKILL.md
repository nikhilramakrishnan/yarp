---
name: add-mcp-server
description: Use this skill when helping users add MCP servers to their Yarp configuration.
---

# Adding MCP Servers to Yarp

Yarp supports MCP servers via native config files. Follow these steps when helping a user add an MCP server.
...
Config file paths:
- **Global (user-scoped):** `~/.yarp/.mcp.json`
- **Project-scoped:** `{repo_root}/.yarp/.mcp.json`
...
By default, Yarp spawns stdio servers from the directory the config was discovered in:
- Project-scoped configs (`{repo_root}/.yarp/.mcp.json`) run from the repo root.
- Global configs (`~/.yarp/.mcp.json`, `~/.claude.json`, etc.) run from the home directory.
...
For environment variables containing secrets, use `${VAR_NAME}` syntax — Yarp will substitute the value from the user's environment at runtime.

## Notes

- Yarp auto-detects changes to `.mcp.json` files on save — no restart required.
- Configured servers appear in Yarp's Settings under MCP, labeled **"Detected from Yarp"**.
